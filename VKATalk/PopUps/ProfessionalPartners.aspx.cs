...
namespace VKATalk.PopUps
{
    ...
    public partial class ProfessionalPartners : System.Web.UI.Page
    {
        int _userId = 1;
        ...
        {
            DataTable dt = new ProfessionalBL().GetProfessionalNameAutoFiller("ProfessionalPartner", prefixText);
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            ...
            {
                int status = 0;
                //DataSet ds = new ProfessionalBL().GetProfessionalTillDateValidation(_professionalId, "ProfessionalPartners", this.btnSave.Text);
                DataSet ds = null;

				//if (btnSave.Text.Equals("Add"))
				//{
				//    ds = new ProfessionalBL().GetProfessionalTillDateValidationProfile(_professionalId, "ProfessionalPartner", txtbxFromDate.Text, btnSave.Text, Convert.ToInt32(hdnfieldProfessionalnameid.Value));
				//}
				//else
				//{
				//    ds = new ProfessionalBL().GetProfessionalTillDateValidationProfile((int)ViewState["ProfessionalRecordID"], "ProfessionalPartner", txtbxFromDate.Text, btnSave.Text, Convert.ToInt32(hdnfieldProfessionalnameid.Value));
				//}
                ...
                else
                {
                    if (btnSave.Text.Equals("Add"))
                    {
                        status = new ProfessionalBL().ProfessionalPartners(
                            _professionalId,
                            Convert.ToInt32(hdnfieldProfessionalnameid.Value),
                            txtbxFromDate.Text,
                            txtbxTillDate.Text,
                            txtbxPercentage.Text,
                            txtbxComment.Text,
                            _userId,
                            "Insert");
        ...
        {
            var dt = new ProfessionalBL().GetProfessionalNameAutoFiller("ProfessionalPartners", prefixText);
        ...
        {
                var ds = new ProfessionalBL().GetProfessionalNameWithCombination(_professionalId, "ProfessionalPartners");
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    var status = new ProfessionalBL().ProfessionalPartners((int)ViewState["ProfessionalRecordID"], 0, "__-__-____", "__-__-____", "", "", _userId, "Delete");
                    if (status > 0)
                    {
                        ClearAllSelection(this);
                        BindData();
        ...
        protected void GvHorseProfile_OnSelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                if (dataKey != null)
                {
                    ...
                    {
                        txtbxPercentage.Text = row.Cells[1].Text;
                    }
        ...
        protected void btnFileUpload_Click(object sender, EventArgs e)
        {
            try
            {
                if (flupload.HasFile)
                {
                    ...
                    if (Path.GetFileNameWithoutExtension(flupload.PostedFile.FileName).Equals("Professional_Partners"))
                    {
        ...
        private void Import_To_Grid(string FilePath, string Extension)
        {
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    var dtErrorResult = new ProfessionalBL().Import30(dt, "ProfessionalPartners");
                    if (dtErrorResult.Rows.Count > 0)
                    {
                        ...
                        {
                            ExcelWorksheet ws = xp.Workbook.Worksheets.Add("Professional_Partners");
                            int rowstart = 1;
                            int colstart = 1;
                            int rowend = rowstart;
                            ...
                            Response.AddHeader("content-disposition", "attachment;filename=Professional_Partners.xlsx");
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                using (DataSet ds = new ProfessionalBL().GetExport(_professionalId, "ProfessionalPartners"))
                {
