...
namespace VKATalk.PopUps
{
    ...
    public partial class ProspectusMasterRaceHistory : System.Web.UI.Page
    {
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (btnSave.Text.Equals("Add"))
                        {
                            status = new ProspectusBL().HandicapRaceHistory(
                                _masterraceId,
                                txtbxSrNo.Text,
                                txtbxHistory.Text,
                                txtbxComment.Text,
        ...
        public static List<string> AddHistory(string prefixText, int count)
        {
            DataTable dt = new ProspectusBL().GetprospectusAutoFiller("MasterRaceHistory", prefixText);
        ...
        {
            DataTable dt = new ProspectusBL().GetprospectusAutoFiller("MasterRaceComments", prefixText);
            List<string> commentList = new List<string>();
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                commentList.Add(dt.Rows[i][0].ToString());
        ...
        private void BindData()
        {
            try
            {
                ...
                DataSet ds = new ProspectusBL().GetProspectusNameWithCombination(_masterraceId, "ProspectusMasterHisotry");
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                if (!ViewState["MasterRaceID"].Equals(""))
                {
                    ...
                    var status = new ProspectusBL().HandicapRaceHistory((int)ViewState["MasterRaceID"],"", "", txtbxComment.Text, 1, "Delete");
        ...
        protected void GvHorseStatus_OnSelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                if (dataKey != null)
                {
                    ...
                    txtbxSrNo.Text = hdnval.Value;
                   
                    if (!row.Cells[1].Text.Contains("&nbsp;"))
                    {
                        txtbxHistory.Text= row.Cells[1].Text;
                    }
                   
                    if (row.Cells[2].Text.Contains("&quot;"))
                    {
                        txtbxComment.Text = row.Cells[2].Text.Equals("&nbsp;") ? "" : row.Cells[2].Text.Contains("&quot;") ? row.Cells[2].Text.Replace("&quot;", "\"") : row.Cells[2].Text;
                    }
                    else
                    {
                        txtbxComment.Text = row.Cells[2].Text.Equals("&nbsp;") ? "" : row.Cells[2].Text.Contains("&#39;") ? row.Cells[2].Text.Replace("&#39;", "'") : row.Cells[2].Text;
        ...
        protected void btnFileUpload_Click(object sender, EventArgs e)
        {
            try
            {
                if (flupload.HasFile)
                {
                    ...
                    if (Path.GetFileNameWithoutExtension(flupload.PostedFile.FileName).Equals("Prospectus_Master_RaceHistory"))
                    {
        ...
        private void Import_To_Grid(string FilePath, string Extension)
        {
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    var dtErrorResult = new ProspectusBL().Import30(dt, "MasterRaceHistory", _masterraceId);
                    if (dtErrorResult.Rows.Count > 0)
                    {
                       // Page.ClientScript.RegisterStartupScript(this.GetType(), "Popup", "alert('Issue in few record. Please check the XL sheet');", true);
                        ...
                        {
                            ExcelWorksheet ws = xp.Workbook.Worksheets.Add("MasterRaceHistory");
                            int rowstart = 1;
                            int colstart = 1;
                            int rowend = rowstart;
                            ...
                            Response.AddHeader("content-disposition", "attachment;filename=Prospectus_Master_RaceHistory.xlsx");
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                using (DataSet ds = new ProspectusBL().GetExport(_masterraceId, "MasterRaceHistory"))
                {
