...
namespace VKATalk.PopUps
{
    ...
    public partial class ProfessionalBaseCenter : System.Web.UI.Page
    {
        int _userId = 1;
                ...
                if (!IsPostBack)
                {
                    try
                    {
                        ...
                        if (_professionalId != 0)
                        {
                            ...
                            BindDropDown(this.drpdwnProfile, "MasterCenter", "CenterName", "CenterID");
                            drpdwnProfile.Items.Insert(0, new ListItem("-- Please select --", "-1"));
                            //if (drpdwnProfile.Items.FindByText("Unknown") != null)
                            //{
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    ds = new ProfessionalBL().GetProfessionalTillDateValidation(_professionalId, "ProfessionalBaseCenter", txtbxFromDate.Text, btnSave.Text);
                }
                else
                {
                    ds = new ProfessionalBL().GetProfessionalTillDateValidation((int)ViewState["ProfessionalRecordID"], "ProfessionalBaseCenter", txtbxFromDate.Text, btnSave.Text);
                }
                ...
                else
                {
                    if (btnSave.Text.Equals("Add"))
                    {
                        status = new ProfessionalBL().ProfessionalBaseCenter(
                            _professionalId,
        ...
        {
            var dt = new ProfessionalBL().GetProfessionalNameAutoFiller("ProfessionalBaseCenterComments", prefixText);
		...
		[System.Web.Script.Services.ScriptMethod()]
		[System.Web.Services.WebMethod]
		public static List<string> AddBaseCenterReasonofChange(string prefixText, int count)
		{
			var dt = new ProfessionalBL().GetProfessionalNameAutoFiller("ProfessionalBaseCenterReasonOfChange", prefixText);
			var commentList = new List<string>();
			for (var i = 0; i < dt.Rows.Count; i++)
			{
				commentList.Add(dt.Rows[i][0].ToString());
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    var status = new ProfessionalBL().ProfessionalBaseCenter((int)ViewState["ProfessionalRecordID"], 0, "__-__-____", "__-__-____","","", _userId, "Delete");
                    if (status > 0)
                    {
                        ClearAllSelection(this);
                        BindData();
        ...
        protected void btnFileUpload_Click(object sender, EventArgs e)
        {
            try
            {
                if (flupload.HasFile)
                {
                    ...
                    if (Path.GetFileNameWithoutExtension(flupload.PostedFile.FileName).Equals("Professional_BaseCenter"))
                    {
        ...
        private void Import_To_Grid(string FilePath, string Extension)
        {
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    var dtErrorResult = new ProfessionalBL().Import30(dt, "ProfessionalBaseCenter");
                    if (dtErrorResult.Rows.Count > 0)
                    {
                        ...
                        {
                            ExcelWorksheet ws = xp.Workbook.Worksheets.Add("Professional_BaseCenter");
                            int rowstart = 1;
                            int colstart = 1;
                            int rowend = rowstart;
                            ...
                            Response.AddHeader("content-disposition", "attachment;filename=Professional_BaseCenter.xlsx");
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            try
            {
              //  using (DataSet ds = new ProfessionalBL().GetProfessionalNameWithCombination(_professionalId, "ProfessionalBaseCenter"))
                using (DataSet ds = new ProfessionalBL().GetExport(_professionalId, "ProfessionalBaseCenter"))
                {
