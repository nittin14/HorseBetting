...
namespace VKATalk.PopUps
{
    ...
    public partial class ProspectusMasterInterestedProfessionalBackground : System.Web.UI.Page
    {
            ...
            if (!IsPostBack)
            {
                try
                {
                    if (_masterraceId != 0)
                    {
                        ...
                        BindDropDown(drpdwnPBackground, "ProfessionalBackground", "ProfessionalBackground", "ProfessionalBackgroundID");
                        drpdwnPBackground.Items.Insert(0, new ListItem("-- Please select --", "-1"));
                        BindData();
                    }
                }
                catch (Exception ex)
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                //{
                //    ds = new ProspectusBL().GetProspectusTillDateValidation(_masterraceId, "ProspectusProfessionalBackground", drpdwnFromYear.SelectedItem.Text, btnSave.Text);
                //}
                //else
                //{
                //    ds = new ProspectusBL().GetProspectusTillDateValidation((int)ViewState["MasterRaceID"], "ProspectusProfessionalBackground", drpdwnFromYear.SelectedItem.Text, btnSave.Text);
                //}
                ...
                //{
                    if (btnSave.Text.Equals("Add"))
                    {
                        status = new ProspectusBL().MasterProfessionalBackground(
                            _masterraceId,
                            Convert.ToInt32(drpdwnPBackground.SelectedItem.Value),
        ...
        {
            DataTable dt = new ProspectusBL().GetprospectusAutoFiller("ProspectusMasterAgeCondition", prefixText);
        ...
        [System.Web.Script.Services.ScriptMethod()]
        [System.Web.Services.WebMethod]
        public static List<string> OtherDetails(string prefixText, int count)
        {
            DataTable dt = new ProspectusBL().GetprospectusAutoFiller("BackgroundOtherDetails", prefixText);
            List<string> commentList = new List<string>();
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                commentList.Add(dt.Rows[i][0].ToString());
        ...
        private void BindData()
        {
            try
            {
                ...
                DataSet ds = new ProspectusBL().GetProspectusNameWithCombination(_masterraceId, "ProspectusMasterProfessionalBackground");
        ...
        protected void GvHorseStatus_OnSelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                if (dataKey != null)
                {
                    ...
                    drpdwnPBackground.Items.FindByText(hdnval.Value).Selected = true;
        ...
        protected void btnFileUpload_Click(object sender, EventArgs e)
        {
            try
            {
                if (flupload.HasFile)
                {
                    ...
                    if (Path.GetFileNameWithoutExtension(flupload.PostedFile.FileName).Equals("Prospectus_Master_InterestedProfessionalBackground"))
                    {
        ...
        private void Import_To_Grid(string FilePath, string Extension)
        {
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    var dtErrorResult = new ProspectusBL().Import30(dt, "ProspectusProfessionalBackground", _masterraceId);
                    if (dtErrorResult.Rows.Count > 0)
                    {
                        ...
                        using (ExcelPackage xp = new ExcelPackage())
                        {
                            ...
                            ExcelWorksheet ws = xp.Workbook.Worksheets.Add("ProspectusProfessionalBackground");
                            ...
                            Response.AddHeader("content-disposition", "attachment;filename=Prospectus_Master_InterestedProfessionalBackground.xlsx");
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                using (DataSet ds = new ProspectusBL().GetExport(_masterraceId, "ProspectusProfessionalBackground"))
                {
