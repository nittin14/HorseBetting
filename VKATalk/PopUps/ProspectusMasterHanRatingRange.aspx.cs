...
namespace VKATalk.PopUps
{
    ...
    public partial class ProspectusMasterHanRatingRange : System.Web.UI.Page
    {
            ...
            if (!IsPostBack)
            {
                try
                {
                    if (_masterraceId != 0)
                    {
						...
						BindDropDown(drpdwnClass, "HomeClass", "ClassType", "ClassTypeID");
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            ...
            {
                int status = 0;
                //DataSet ds = new ProspectusBL().GetProspectusTillDateValidation(_masterraceId, "MasterRatingRange", this.btnSave.Text);
                DataSet ds = null;
                if (btnSave.Text.Equals("Add"))
                {
                    ds = new ProspectusBL().GetProspectusTillDateValidation(_masterraceId, "MasterRatingRange", drpdwnFromYear.SelectedItem.Text, btnSave.Text);
                }
                else
                {
                    ErrorHandling.CheckEachSteps("btnSave_Click:" + ViewState["MasterRaceID"] + "," + drpdwnFromYear.SelectedItem.Text);
                    ds = new ProspectusBL().GetProspectusTillDateValidation((int)ViewState["MasterRaceID"], "MasterRatingRange", drpdwnFromYear.SelectedItem.Text, btnSave.Text);
                }
                ...
                else
                {
                    if (btnSave.Text.Equals("Add"))
                    {
                        status = new ProspectusBL().MasterRatingRange(
                            _masterraceId,
                            Convert.ToInt32(drpdwnHandicapRatingRange.SelectedItem.Value),
                            Convert.ToInt32(drpdwnClass.SelectedItem.Value),
        ...
        {
            DataTable dt = new ProspectusBL().GetprospectusAutoFiller("ProspectusMasterHandicapRatingRange", prefixText);
        ...
        private void BindData()
        {
            try
            {
                DataSet ds = new ProspectusBL().GetProspectusNameWithCombination(_masterraceId, "MasterHandicapRatingRange");
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    var status = new ProspectusBL().MasterRatingRange((int)ViewState["MasterRaceID"], 0,0,0,0,txtbxComment.Text, 1, "Delete","__-__-____");
                    ClearAllSelection(this);
                    BindData();
                    btnSave.Text = "Add";
        ...
        protected void GvHorseStatus_OnSelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                if (dataKey != null)
                {
                    ...
                    drpdwnHandicapRatingRange.Items.FindByText(hdnval.Value).Selected = true;
                    drpdwnClass.Items.FindByText(row.Cells[1].Text).Selected = true;
        ...
        protected void btnFileUpload_Click(object sender, EventArgs e)
        {
            try
            {
                if (flupload.HasFile)
                {
                    ...
                    if (Path.GetFileNameWithoutExtension(flupload.PostedFile.FileName).Equals("Prospectus_Master_HandicapRatingRange"))
                    {
        ...
        private void Import_To_Grid(string FilePath, string Extension)
        {
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    var dtErrorResult = new ProspectusBL().Import30(dt, "ProspectMasterHandicapRatingRange", _masterraceId);
                    if (dtErrorResult.Rows.Count > 0)
                    {
                        using (ExcelPackage xp = new ExcelPackage())
                        {

                            ExcelWorksheet ws = xp.Workbook.Worksheets.Add("ProspectMasterHandicapRatingRange");
                            ...
                            Response.AddHeader("content-disposition", "attachment;filename=Prospectus_Master_HandicapRatingRange.xlsx");
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                //using (DataSet ds = new ProspectusBL().GetProspectusNameWithCombination(_masterraceId, "MasterHandicapRatingRange"))
                using (DataSet ds = new ProspectusBL().GetExport(_masterraceId, "MasterHandicapRatingRange"))
                {
