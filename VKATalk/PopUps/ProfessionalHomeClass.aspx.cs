...
namespace VKATalk.PopUps
{
    ...
    public partial class ProfessionalHomeClass : System.Web.UI.Page
    {
        int _userId = 1;
                ...
                if (!IsPostBack)
                {
                    try
                    {
                        ...
                        if (_professionalId != 0)
                        {
                            ...
                            BindData();
                            //BindDropDown(drpdwnHomeDistance, "Class", "Class", "ClassID");
                            BindDropDown(drpdwnHomeDistance, "ProspectusMasterBunch", "Bunch", "BunchMID");
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            ...
            {
                int status = 0;
                //DataSet ds = new ProfessionalBL().GetProfessionalTillDateValidation(_professionalId, "HomeClass", this.btnSave.Text);
                DataSet ds = null;
                //if (btnSave.Text.Equals("Add"))
                //{
                //    ds = new ProfessionalBL().GetProfessionalTillDateValidation(_professionalId, "HomeClass", txtbxFromDate.Text, btnSave.Text);
                //}
                //else
                //{
                //    ds = new ProfessionalBL().GetProfessionalTillDateValidation((int)ViewState["ProfessionalRecordID"], "HomeClass", txtbxFromDate.Text, btnSave.Text);
                //}

                if (btnSave.Text.Equals("Add"))
                {
                    ds = new ProfessionalBL().GetProfessionalTillDateValidationProfile(_professionalId, "ProfessionalHomeClass", txtbxFromDate.Text, btnSave.Text, Convert.ToInt32(drpdwnHomeDistance.SelectedItem.Value));
                }
                else
                {
                    ds = new ProfessionalBL().GetProfessionalTillDateValidationProfile((int)ViewState["ProfessionalRecordID"], "ProfessionalHomeClass", txtbxFromDate.Text, btnSave.Text, Convert.ToInt32(drpdwnHomeDistance.SelectedItem.Value));
                }
                ...
                else
                {
                    if (btnSave.Text.Equals("Add"))
                    {
                        status = new ProfessionalBL().HomeClass(
                            _professionalId,
        ...
        {
            DataTable dt = new ProfessionalBL().GetProfessionalNameAutoFiller("HomeClass", prefixText);
        ...
        {
                DataSet ds = new ProfessionalBL().GetProfessionalNameWithCombination(_professionalId, "HomeClass");
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    var status = new ProfessionalBL().HomeClass((int)ViewState["ProfessionalRecordID"], 0, "__-__-____", "__-__-____", 0, "", this._userId, "Delete");
                    if (status > 0)
                    {
                        ClearAllSelection(this);
                        BindData();
        ...
        protected void btnFileUpload_Click(object sender, EventArgs e)
        {
            try
            {
                if (flupload.HasFile)
                {
                    ...
                    if (Path.GetFileNameWithoutExtension(flupload.PostedFile.FileName).Equals("Professional_HomeBunch"))
                    {
        ...
        private void Import_To_Grid(string FilePath, string Extension)
        {
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    var dtErrorResult = new ProfessionalBL().Import30(dt, "ProfessionalHomeClass");
                    if (dtErrorResult.Rows.Count > 0)
                    {
                        ...
                        {
                            ExcelWorksheet ws = xp.Workbook.Worksheets.Add("Professional_HomeBunch");
                            int rowstart = 1;
                            int colstart = 1;
                            int rowend = rowstart;
                            ...
                            Response.AddHeader("content-disposition", "attachment;filename=Professional_HomeBunch.xlsx");
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                using (DataSet ds = new ProfessionalBL().GetExport(_professionalId, "HomeClass"))
                {
