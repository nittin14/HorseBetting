...
namespace VKATalk.PopUps
{
    ...
    public partial class ProfessionalOwnerColor : System.Web.UI.Page
    {
        int _userId = 1;
                ...
                if (!IsPostBack)
                {
                    try
                    {
                        ...
                        if (_professionalId != 0)
                        {
                            ...
                            lblProfessionalFirst.Text = reqQueryValue;

                            //string[] horseName = reqQueryValue.Split(',');
                            //lblProfessionalFirst.Text = horseName[1];
                            //lblProfessionalNameSecond.Text = horseName[2];
                            BindDropDown(this.drpdwnCenter, "MasterCenter", "CenterName", "CenterID");
                            drpdwnCenter.Items.Insert(0, new ListItem("-- Please select --", "-1"));
                            //BindDropDown(this.drpdwnOwnerColor, "MasterOwnerColor", "OwnerColor", "OwnerColorID");
                            //drpdwnOwnerColor.Items.Insert(0, new ListItem("-- Please select --", "-1"));
                            BindData();
                        }
                    }
                    catch (Exception ex)
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                //{
                //    ds = new ProfessionalBL().GetProfessionalTillDateValidation(_professionalId, "ProfessionalOwnerColor", txtbxFromDate.Text, btnSave.Text);
                //}
                //else
                //{
                //    ds = new ProfessionalBL().GetProfessionalTillDateValidation((int)ViewState["ProfessionalRecordID"], "ProfessionalOwnerColor", txtbxFromDate.Text, btnSave.Text);
                //}
                ...
                else
                {
                    if (btnSave.Text.Equals("Add"))
                    {
                        status = new ProfessionalBL().ProfessionalOwnerColor(
                            _professionalId,
                            Convert.ToInt32(hdnfieldProfessionalOwnerColorID.Value),
        ...
        {
            DataTable dt = new ProfessionalBL().GetProfessionalNameAutoFiller("ProfessionalOwnerColor", prefixText);
        ...
        [System.Web.Script.Services.ScriptMethod()]
        [System.Web.Services.WebMethod]
        public static List<string> AddProfessionalOwnerColorList(string prefixText, int count)
        {
            //DataTable dt = new ProfessionalBL().GetProfessionalNameAutoFiller("ProfessionalOwnerColor", prefixText);
            //List<string> commentList = new List<string>();
            //for (int i = 0; i < dt.Rows.Count; i++)
        ...
        {
                DataSet ds = new ProfessionalBL().GetProfessionalNameWithCombination(_professionalId, "ProfessionalOwnerColor");
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    var status = new ProfessionalBL().ProfessionalOwnerColor((int)ViewState["ProfessionalRecordID"], 0, 0, "__-__-____", "__-__-____", "", _userId, "Delete");
                    if (status > 0)
                    {
                        ClearAllSelection(this);
                        BindData();
        ...
        protected void GvHorseProfile_OnSelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                btnSave.Text = "Update";
                GridViewRow row = GvHorseProfile.SelectedRow;
                HiddenField hdnval = (HiddenField)row.FindControl("hdnfieldStatus");
                HiddenField hdnfieldOwnerColorID = (HiddenField)row.FindControl("hdnfieldOwnerColorID");
                var dataKey = GvHorseProfile.DataKeys[row.RowIndex];
                if (dataKey != null)
                {
                    ClearAllSelection(this);
                    ViewState["ProfessionalRecordID"] = dataKey.Value;
                    //drpdwnOwnerColor.Items.FindByText(hdnval.Value).Selected = true;
                    txtbxOwnerColor.Text = hdnval.Value;
                    hdnfieldProfessionalOwnerColorID.Value = hdnfieldOwnerColorID.Value;
                    drpdwnCenter.Items.FindByText(row.Cells[1].Text).Selected = true;
        ...
        protected void btnFileUpload_Click(object sender, EventArgs e)
        {
            try
            {
                if (flupload.HasFile)
                {
                    ...
                    if (Path.GetFileNameWithoutExtension(flupload.PostedFile.FileName).Equals("Professional_OwnerColor"))
                    {
        ...
        private void Import_To_Grid(string FilePath, string Extension)
        {
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    var dtErrorResult = new ProfessionalBL().Import30(dt, "ProfessionalOwnerColor");
                    if (dtErrorResult.Rows.Count > 0)
                    {
                        ...
                        {
                            ExcelWorksheet ws = xp.Workbook.Worksheets.Add("Professional_OwnerColor");
                            int rowstart = 1;
                            int colstart = 1;
                            int rowend = rowstart;
                            ...
                            Response.AddHeader("content-disposition", "attachment;filename=Professional_OwnerColor.xlsx");
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            try
            {
 //               using (DataSet ds = new ProfessionalBL().GetProfessionalNameWithCombination(_professionalId, "ProfessionalOwnerColor"))
                using (DataSet ds = new ProfessionalBL().GetExport(_professionalId, "ProfessionalOwnerColor"))
                {
                    DataTable dt = ds.Tables[0];
                    if (dt.Rows.Count > 0)
                    {
                        //dt.Columns.Remove("ProfessionalOwnerColorID");
                        using (ExcelPackage xp = new ExcelPackage())
                        {
                            ExcelWorksheet ws = xp.Workbook.Worksheets.Add("Professional_OwnerColor");
                            int rowstart = 1;
