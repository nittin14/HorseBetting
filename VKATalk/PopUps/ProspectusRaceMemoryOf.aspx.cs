...
namespace VKATalk.PopUps
{
    ...
    public partial class ProspectusRaceMemoryOf : System.Web.UI.Page
    {
        //MasterHorseBL Bl = new MasterHorseBL();
        int _masterraceId = 0;
        static string memoirType = string.Empty;
            ...
            if (!IsPostBack)
            {
                try
                {
                    if (_masterraceId != 0)
                    {
                        ...
                        try
                        {
                            var count = RaceName.Length;
                            //var count1 = RaceName.get;
                            lblMasterRaceNameSecond.Text = RaceName[1];
                        }
                        catch
                        {
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    ds = new ProspectusBL().GetProspectusTillDateValidation(_masterraceId, "MemoryOf", drpdwnFromYear.SelectedItem.Text, btnSave.Text);
                }
                else
                {
                    ds = new ProspectusBL().GetProspectusTillDateValidation((int)ViewState["MasterRaceID"], "MemoryOf", drpdwnFromYear.SelectedItem.Text, btnSave.Text);
                }
                ...
                else
                {
                    if (btnSave.Text.Equals("Add"))
                    {
                        status = new ProspectusBL().InsertRaceMemoryOf(
                            _masterraceId,
                            Convert.ToInt32(drpdwMemoirType.SelectedItem.Value),
                            Convert.ToInt32(hdnfieldMemoirNameID.Value),
        ...
        {
            DataTable dt = new ProspectusBL().GetprospectusAutoFiller("MemoryOfCommentList", prefixText);
        ...
        {
            DataTable dt = new ProspectusBL().GetprospectusAutoFiller("MemoryOfOtherList", prefixText);
            List<string> commentList = new List<string>();
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                commentList.Add(dt.Rows[i][0].ToString());
        ...
        //{
        //    DataTable dt = new ProspectusBL().GetprospectusAutoFiller("AddMemoirTypeList", prefixText);
        //    List<string> commentList = new List<string>();
        //    for (int i = 0; i < dt.Rows.Count; i++)
        //    {
        //        commentList.Add(dt.Rows[i][0].ToString());
        ...
        [System.Web.Script.Services.ScriptMethod()]
        [System.Web.Services.WebMethod]
        public static List<string> AddMemoirNameList(string prefixText, int count, string contextKey)
        {
            DataTable dt = new ProspectusBL().GetprospectusAutoFillerWithParameters("AddMemoirNameList", prefixText,contextKey);
            List<string> commentList = new List<string>();
            for (int i = 0; i < dt.Rows.Count; i++)
            {
        ...
        private void BindData()
        {
            try
            {
                ...
                DataSet ds = new ProspectusBL().GetProspectusNameWithCombination(_masterraceId, "MemoryOf");
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                if (!ViewState["MasterRaceID"].Equals(""))
                {
                    ...
                    var status = new ProspectusBL().InsertRaceMemoryOf((int)ViewState["MasterRaceID"], 0,0,0, 0,"",txtbxComment.Text, 1, "Delete");
        ...
        protected void drpdwMemoirType_SelectIndexChange(object sender, EventArgs e)
        {
            try
            {
                hdnfieldMemoirNameID.Value = string.Empty;
                txtbxMemoirName.Text = string.Empty;
                txtbxMemoirName.Focus();
            }
            catch (Exception ex)
            {
        ...
        protected void GvHorseStatus_OnSelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                HiddenField hdnvalmemoirid = (HiddenField)row.FindControl("hdnfieldMemoirID");
                HiddenField hdnvalmemoirnamepid = (HiddenField)row.FindControl("hdnfieldMemoirNamePID");
                var dataKey = GvProspectus.DataKeys[row.RowIndex];
                if (dataKey != null)
                {
                    ClearAllSelection(this);
                    ViewState["MasterRaceID"] = dataKey.Value;
                    drpdwMemoirType.Items.FindByText(hdnval.Value).Selected = true;
                    txtbxMemoirName.Text = row.Cells[1].Text;
                    hdnfieldMemoirNameID.Value = hdnvalmemoirnamepid.Value;
                    ...
                    {
                        txtbxOtherDetails.Text = row.Cells[4].Text.Equals("&nbsp;") ? "" : row.Cells[4].Text.Contains("&quot;") ? row.Cells[4].Text.Replace("&quot;", "\"") : row.Cells[4].Text;
                    }
                    else
                    {
                        txtbxOtherDetails.Text = row.Cells[4].Text.Equals("&nbsp;") ? "" : row.Cells[4].Text.Contains("&#39;") ? row.Cells[4].Text.Replace("&#39;", "'") : row.Cells[4].Text;
        ...
        protected void btnFileUpload_Click(object sender, EventArgs e)
        {
            try
            {
                if (flupload.HasFile)
                {
                    ...
                    if (Path.GetFileNameWithoutExtension(flupload.PostedFile.FileName).Equals("Prospectus_Master_RaceInMemoryOf"))
                    {
        ...
        private void Import_To_Grid(string FilePath, string Extension)
        {
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    var dtErrorResult = new ProspectusBL().Import30(dt, "RaceInMemoryOf", _masterraceId);
                    if (dtErrorResult.Rows.Count > 0)
                    {
                        using (ExcelPackage xp = new ExcelPackage())
                        {
                            ExcelWorksheet ws = xp.Workbook.Worksheets.Add("RaceInMemoryOf");
                            int rowstart = 1;
                            int colstart = 1;
                            int rowend = rowstart;
                            ...
                            Response.AddHeader("content-disposition", "attachment;filename=Prospectus_Master_RaceInMemoryOf.xlsx");
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                //using (DataSet ds = new ProspectusBL().GetProspectusNameWithCombination(_masterraceId, "MemoryOf"))
                using (DataSet ds = new ProspectusBL().GetExport(_masterraceId, "MemoryOf"))
                {
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        using (ExcelPackage xp = new ExcelPackage())
                        {
                            ...
                            ExcelWorksheet ws = xp.Workbook.Worksheets.Add("MemoryOf");
