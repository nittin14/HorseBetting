...
namespace VKATalk.PopUps
{
    ...
    public partial class HorseMyObservations : System.Web.UI.Page
    {
        int _horseId = 0;
        int _userId = 1;
     ...
     //               }
		...
		public void BindData()
        {
                ...
                DataTable dt = Bl.GetHorseDetail(Convert.ToString(_horseId), "HorseObservation");
                if (dt.Rows.Count > 0)
                {
        ...
        [System.Web.Script.Services.ScriptMethod()]
        [System.Web.Services.WebMethod]
        public static List<string> AddObservation(string prefixText, int count)
        {
            DataTable dt = new MasterHorseBL().GetHorseNameAutoFiller("HorseObservation", prefixText);
        ...
        [System.Web.Script.Services.ScriptMethod()]
        [System.Web.Services.WebMethod]
        public static List<string> AddAimedList(string prefixText, int count)
        {
            DataTable dt = new MasterHorseBL().GetHorseNameAutoFiller("HorseObservationAimed", prefixText);
            List<string> currentMissionComments = new List<string>();
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                currentMissionComments.Add(dt.Rows[i][0].ToString());
        ...
        {
            DataTable dt = new MasterHorseBL().GetHorseNameAutoFiller("HorseObservationComments", prefixText);
            List<string> currentMissionComments = new List<string>();
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                currentMissionComments.Add(dt.Rows[i][0].ToString());
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    {
                        if (dt.Rows[count][1].ToString().Equals(rdbtn.SelectedItem.Text))
                        {
                            check = 1;
                        }
                    }
                ...
                if (check == 1)
                {
                    ...
                    {
                        ds = new MasterHorseBL().GetHorseTillDateValidation(_horseId, "HorseObservation", txtbxFromDate.Text, btnSave.Text);
                    }
                    else
                    {
                        ds = new MasterHorseBL().GetHorseTillDateValidation((int)ViewState["horseStatusID"], "HorseObservation", txtbxFromDate.Text, btnSave.Text);
                    }
                    checkTillDate = Convert.ToInt64(ds.Tables[0].Rows[0][0]);
                }
                ...
                else
                {
                    if (btnSave.Text.Equals("Add"))
                    {
                        status = new MasterHorseBL().HorseMyObservation(
                            _horseId,
                            txtbxObservations.Text,
                            this.rdbtn.SelectedItem.Text,
                            txtbxFromDate.Text,
                            txtbxTillDate.Text,
                            TxtbxReason.Text,
                            txtbxComment.Text,
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    new MasterHorseBL().HorseMyObservation((int)ViewState["horseStatusID"],"","", txtbxFromDate.Text, txtbxTillDate.Text,"",
        ...
        protected void GvGlobal_OnSelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                if (dataKey != null)
                {
                    ...
                    txtbxObservations.Text = row.Cells[1].Text;
					if (!row.Cells[2].Text.Contains("&nbsp;"))
					{
						TxtbxReason.Text = row.Cells[2].Text;
					}
        ...
        public void ClearAllSelection(Control parent)
        {
			...
			rdbtn.Items.FindByText("Short Term").Selected = true;
        ...
        protected void btnFileUpload_Click(object sender, EventArgs e)
        {
            try
            {
                if (flupload.HasFile)
                {
                    ...
                    if (Path.GetFileNameWithoutExtension(flupload.PostedFile.FileName).Equals("Horse_MyObservation"))
                    {
        ...
        private void Import_To_Grid(string FilePath, string Extension)
        {
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    var dtErrorResult = new MasterHorseBL().Import30(dt, "HorseMyObservation");
                    if (dtErrorResult.Rows.Count > 0)
                    {
                        ...
                        //{
                        //    ExcelWorksheet ws = xp.Workbook.Worksheets.Add("Horse_MyObservation");
                        //    int rowstart = 1;
                        //    int colstart = 1;
                        //    int rowend = rowstart;
                        ...
                        //    Response.AddHeader("content-disposition", "attachment;filename=Horse_MyObservations.xlsx");
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            try
            {
                //using (DataSet ds = new MasterHorseBL().GetHorseNameWithCombination(_horseId, "HorseMyObservation"))
                using (DataSet ds = new MasterHorseBL().GetExport(_horseId, "HorseMyObservation"))
                {
                    DataTable dt = ds.Tables[0];
                    if (dt.Rows.Count > 0)
                    {
                        //dt.Columns.Remove("MyObservationsID");
                        using (ExcelPackage xp = new ExcelPackage())
                        {
                            ExcelWorksheet ws = xp.Workbook.Worksheets.Add("Horse_MyObservation");
                            int rowstart = 1;
                            ...
                            Response.AddHeader("content-disposition", "attachment;filename=Horse_MyObservation.xlsx");
                            Response.ContentType = "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet";
                            Response.BinaryWrite(xp.GetAsByteArray());
                            Response.End();
                        }
