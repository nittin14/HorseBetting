...
namespace VKATalk.PopUps
{
    ...
    public partial class ProspectusMasterBunchCondition : System.Web.UI.Page
    {
            ...
            if (!IsPostBack)
            {
                try
                {
                    if (_masterraceId != 0)
                    {
						...
						BindDropDown(drpdwnBunchCondition, "ProspectusMasterBunchCondition", "BunchCondition", "BunchConditionID");
						drpdwnBunchCondition.Items.Insert(0, new ListItem("-- Please select --", "-1"));

						//BindDropDown(drpdwnPermanentCondition, "AgeCondition", "AgeCondition", "AgeConditionID");
						//drpdwnPermanentCondition.Items.Insert(0, new ListItem("-- Please select --", "-1"));
						BindData();
                    }
                }
                catch (Exception ex)
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            ...
            {
                int status = 0;
                //DataSet ds = new ProspectusBL().GetProspectusTillDateValidation(_masterraceId, "MasterAgeCondition", this.btnSave.Text);
                DataSet ds = null;
                if (btnSave.Text.Equals("Add"))
                {
                    ds = new ProspectusBL().GetProspectusTillDateValidation(_masterraceId, "ProspectusMasterBunchCondition", drpdwnFromYear.SelectedItem.Text, btnSave.Text);
                }
                else
                {
                    ds = new ProspectusBL().GetProspectusTillDateValidation((int)ViewState["MasterRaceID"], "ProspectusMasterBunchCondition", drpdwnFromYear.SelectedItem.Text, btnSave.Text);
                }
                ...
                else
                {
                    if (btnSave.Text.Equals("Add"))
                    {
                        status = new ProspectusBL().MasterBunchCondition(
                            _masterraceId,
                            Convert.ToInt32(drpdwnBunchCondition.SelectedItem.Value),
        ...
        {
            DataTable dt = new ProspectusBL().GetprospectusAutoFiller("ProspectusMasterBunchConditionComments", prefixText);
        ...
        {
            DataTable dt = new ProspectusBL().GetprospectusAutoFiller("ProspectusMasterBunchCondition", prefixText);
        ...
        private void BindData()
        {
            try
            {
                ...
                DataSet ds = new ProspectusBL().GetProspectusNameWithCombination(_masterraceId, "ProspectusMasterBunchCondition");
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                   // var status = new ProspectusBL().PermanentCondition((int)ViewState["MasterRaceID"], 0, 0, 0, txtbxComment.Text, 1, "Delete");
                    var status = new ProspectusBL().MasterBunchCondition(
                            (int)ViewState["MasterRaceID"],
                            0,0,0,
                            "",
                            1,
        ...
        protected void GvHorseStatus_OnSelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                //hdnfieldBunchCondition.Value = hdnval.Value;
                HiddenField hdnval1 = (HiddenField)row.FindControl("hdnfieldbunchconditionid");
                //hdnfieldBunchCondition.Value = hdnval1.Value;
                var dataKey = GvProspectus.DataKeys[row.RowIndex];
                if (dataKey != null)
                {
                    ClearAllSelection(this);
                    ViewState["MasterRaceID"] = dataKey.Value;
					//drpdwnPermanentCondition.Items.FindByText(hdnval.Value).Selected = true;
					drpdwnBunchCondition.Items.FindByText(hdnval.Value).Selected = true;
					//txtbxBunchCondition.Text = hdnval.Value;
        ...
        protected void btnFileUpload_Click(object sender, EventArgs e)
        {
            try
            {
                if (flupload.HasFile)
                {
                    ...
                    if (Path.GetFileNameWithoutExtension(flupload.PostedFile.FileName).Equals("Prospectus_Master_BunchCondition"))
                    {
        ...
        private void Import_To_Grid(string FilePath, string Extension)
        {
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    var dtErrorResult = new ProspectusBL().Import30(dt, "ProspetusMasterBunchCondition", _masterraceId);
                    if (dtErrorResult.Rows.Count > 0)
                    {
                        using (ExcelPackage xp = new ExcelPackage())
                        {
                            ExcelWorksheet ws = xp.Workbook.Worksheets.Add("ProspetusMasterBunchCondition");
                            int rowstart = 1;
                            int colstart = 1;
                            int rowend = rowstart;
                            ...
                            Response.AddHeader("content-disposition", "attachment;filename=Prospectus_Master_BunchCondition.xlsx");
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            try
            {
                using (DataSet ds = new ProspectusBL().GetExport(_masterraceId, "ProspetusMasterBunchCondition"))
                {
