...
namespace VKATalk.PopUps
{
    ...
    public partial class ProfessionalAntiPerson : System.Web.UI.Page
    {
        int _userId = 1;
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
				...
				//{
				//    ds = new ProfessionalBL().GetProfessionalTillDateValidationProfile(_professionalId, "ProfessionalAntiPerson", txtbxFromDate.Text, btnSave.Text, Convert.ToInt32(hdnfieldAntiPersonID.Value));
				//}
				//else
				//{
                ...
                else
                {
                    if (btnSave.Text.Equals("Add"))
                    {
                        status = new ProfessionalBL().ProfessionalAntiPerson(
                            _professionalId,
                            Convert.ToInt32(hdnfieldAntiPersonID.Value),
                            txtbxFromDate.Text,
                            txtbxTillDate.Text,
                            txtbxReason.Text,
                            txtbxComment.Text,
                    ...
                    if (status == 1)
                    {
                        //if (btnSave.Text.Equals("Add"))
                        //{
                        //    hdnfieldProfessionalName.Value = drpdwnProfile.SelectedItem.Text;
                        //}
                        var message = "Record added successfully.";
                        ClientScript.RegisterStartupScript(this.GetType(), "Popup", "ShowPopup('" + message + "');", true);
                        BindData();
                        ClearAllSelection(this);
                      //  drpdwnProfile.Items.FindByText("Unknown").Selected = true;
                    }
                    else if (status == 2)
                    {
        ...
        {
            var dt = new ProfessionalBL().GetProfessionalNameAutoFiller("ProfessionalAntiComments", prefixText);
        ...
        [System.Web.Script.Services.ScriptMethod()]
        [System.Web.Services.WebMethod]
        public static List<string> AddReasonforAnti(string prefixText, int count)
        {
            var dt = new ProfessionalBL().GetProfessionalNameAutoFiller("ProfessionalAntiReason", prefixText);
            var commentList = new List<string>();
            for (var i = 0; i < dt.Rows.Count; i++)
            {
                commentList.Add(dt.Rows[i][0].ToString());
        ...
        {
               var ds = new ProfessionalBL().GetProfessionalNameWithCombination(_professionalId, "ProfessionalAntiPerson");
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    var status = new ProfessionalBL().ProfessionalAntiPerson((int)ViewState["ProfessionalRecordID"], 0, "__-__-____", "__-__-____", "", "", _userId, "Delete");
                    if (status > 0)
                    {
                        ClearAllSelection(this);
                        BindData();
        ...
        protected void GvHorseProfile_OnSelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                var hdnvalprofessionalnameid = (HiddenField)row.FindControl("hdnfieldgridviewpersonid");
                hdnfieldAntiPersonID.Value = hdnvalprofessionalnameid.Value;
                ...
                if (dataKey != null)
                {
                    ...
                    txtbxPersonName.Text = hdnval.Value;
        ...
        protected void btnFileUpload_Click(object sender, EventArgs e)
        {
            try
            {
                if (flupload.HasFile)
                {
                    ...
                    if (Path.GetFileNameWithoutExtension(flupload.PostedFile.FileName).Equals("Professional_AntiPerson"))
                    {
        ...
        private void Import_To_Grid(string FilePath, string Extension)
        {
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    var dtErrorResult = new ProfessionalBL().Import30(dt, "ProfessionalAntiPerson");
                    if (dtErrorResult.Rows.Count > 0)
                    {
                        ...
                        {
                            ExcelWorksheet ws = xp.Workbook.Worksheets.Add("Professional_AntiPerson");
                            int rowstart = 1;
                            int colstart = 1;
                            int rowend = rowstart;
                            ...
                            Response.AddHeader("content-disposition", "attachment;filename=Professional_AntiPerson.xlsx");
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            try
            {
              //  using (DataSet ds = new ProfessionalBL().GetProfessionalNameWithCombination(_professionalId, "ProfessionalBaseCenter"))
                using (DataSet ds = new ProfessionalBL().GetExport(_professionalId, "ProfessionalAntiPerson"))
                {
