...
namespace VKATalk.PopUps
{
    ...
    public partial class DirectGate : System.Web.UI.Page
    {
        int _horseId = 0;
        int _userId = 1;
        ...
        public void BindData()
        {
            try
            {
                ...
                DataTable dt = Bl.GetHorseDetail(Convert.ToString(_horseId), "HorseDirectGate");
                if (dt.Rows.Count > 0)
                {
        ...
        {
            DataTable dt = new MasterHorseBL().GetHorseNameAutoFiller("HorseDirectGate", prefixText);
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                int status = 0;
                DataSet ds = null;
                if (!chkbxDirectGate.Checked)
                {
                    string message = "Please select Direct Gate.";
                    ClientScript.RegisterStartupScript(this.GetType(), "Popup", "ShowPopup('" + message + "');", true);
                }
                else
                {
                    ...
                    {
                        ds = new MasterHorseBL().GetHorseTillDateValidation(_horseId, "HorseDirectGate", txtbxFromDate.Text, btnSave.Text);
                    }
                    else
                    {
                        ds = new MasterHorseBL().GetHorseTillDateValidation((int)ViewState["horseStatusID"], "HorseDirectGate", txtbxFromDate.Text, btnSave.Text);
                    }
                    ...
                    else
                    {
                        if (btnSave.Text.Equals("Add"))
                        {
                            status = new MasterHorseBL().HorseDirectGate(
                                _horseId,
                                Convert.ToInt32(this.chkbxDirectGate.Checked),
                                txtbxFromDate.Text,
                                txtbxTillDate.Text,
                                txtbxComment.Text,
                                1,
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    new MasterHorseBL().HorseDirectGate((int)ViewState["horseStatusID"], 0, txtbxFromDate.Text, txtbxTillDate.Text,
        ...
        protected void GvGlobal_OnSelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                if (dataKey != null)
                {
                    ...
                    if (hdnval.Value.Equals("Direct Gate"))
                        chkbxDirectGate.Checked = true;
                    else
                        chkbxDirectGate.Checked = false;
        ...
        protected void btnFileUpload_Click(object sender, EventArgs e)
        {
            try
            {
                if (flupload.HasFile)
                {
                    ...
                    if (Path.GetFileNameWithoutExtension(flupload.PostedFile.FileName).Equals("Horse_DirectGate"))
                    {
        ...
        private void Import_To_Grid(string FilePath, string Extension)
        {
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                   // var dtErrorResult = new MasterHorseBL().UploadExcelRecordBulkMinimumColumns(dt, "HorseDirectGate");
                    var dtErrorResult = new MasterHorseBL().Import30(dt, "HorseDirectGate");
                    if (dtErrorResult.Rows.Count > 0)
                    {
                        ...
                        {
                            ExcelWorksheet ws = xp.Workbook.Worksheets.Add("Horse DirectGate");
                            int rowstart = 1;
                            int colstart = 1;
                            int rowend = rowstart;
                            ...
                            Response.AddHeader("content-disposition", "attachment;filename=Horse_DirectGate.xlsx");
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            try
            {
                //using (DataSet ds = new MasterHorseBL().GetHorseNameWithCombination(_horseId, "HorseDirectGate"))
                using (DataSet ds = new MasterHorseBL().GetExport(_horseId, "HorseDirectGate"))
                {
                    DataTable dt = ds.Tables[0];
                    if (dt.Rows.Count > 0)
                    {
                       // dt.Columns.Remove("DirectGateID");
                        using (ExcelPackage xp = new ExcelPackage())
                        {
                            ExcelWorksheet ws = xp.Workbook.Worksheets.Add("HorseDirectGate");
                            int rowstart = 1;
                            int colstart = 1;
                            int rowend = rowstart;
