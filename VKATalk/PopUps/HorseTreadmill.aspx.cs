...
namespace VKATalk.PopUps
{
    ...
    public partial class HorseTreadmill : System.Web.UI.Page
    {
        int _horseId = 0;
        int _userId = 1;
        ...
        public static List<string> AddHorseList(string prefixText, int count)
       {
        ...
        public void BindData()
        {
                ...
                DataTable dt = Bl.GetHorseDetail(hdnfieldHorseNameID.Value, "HorseTreadmill");
                if (dt.Rows.Count > 0)
                {
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    {
                        status = new MasterHorseBL().HorseTreadmill(
                            Convert.ToInt32(hdnfieldHorseNameID.Value)
                            , txtbxTreadmillDate.Text
                            ,txtbxValueA.Text
                            ,txtbxValueB.Text
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    new MasterHorseBL().HorseTreadmill(
                            Convert.ToInt32(hdnfieldHorseNameID.Value)
                            , txtbxTreadmillDate.Text
                            , txtbxValueA.Text
                            , txtbxValueB.Text
                ...
                if (dataKey != null)
                {
                 //   ClearAllSelection(this);
                  //  HiddenField hdnfieldHorseNameIDG = (HiddenField)row.FindControl("hdnfieldHorseNameID");
                    HiddenField hdnfieldSwimmingDate = (HiddenField)row.FindControl("hdnfieldTreadmillDate");
                    ...
                    txtbxTreadmillDate.Text = hdnfieldSwimmingDate.Value;
                    txtbxValueA.Text = row.Cells[1].Text;
                    txtbxValueB.Text = row.Cells[2].Text;
                    drpdnWorkoutRating.ClearSelection();
                    this.drpdnWorkoutRating.Items.FindByText(row.Cells[3].Text).Selected = true;
                    if(row.Cells[4].Text.Equals("Show"))
                    {
                        chkbxIsShow.Checked = true;
                    }
                    else
        ...
        public void ClearAllSelection(Control parent)
        {
            ...
            {
                txtbxValueA.Text = string.Empty;
                txtbxValueB.Text = string.Empty;
                drpdnWorkoutRating.ClearSelection();
                chkbxIsShow.Checked = true;
            }
            else if (chkbxfix.Checked.Equals(true))
            {
                txtbxTreadmillDate.Text = string.Empty;
                txtbxValueA.Text = string.Empty;
                txtbxValueB.Text = string.Empty;
                drpdnWorkoutRating.ClearSelection();
                chkbxIsShow.Checked = true;
        ...
        protected void btnFileUpload_Click(object sender, EventArgs e)
        {
            try
            {
                if (flupload.HasFile)
                {
                    ...
                    if (Path.GetFileNameWithoutExtension(flupload.PostedFile.FileName).Equals("Horse_Treadmill"))
                    {
        ...
        private void Import_To_Grid(string FilePath, string Extension)
        {
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    var dtErrorResult = new MasterHorseBL().Import30(dt, "Horse_Treadmill");
                    if (dtErrorResult.Rows.Count > 0)
                    {
                        ...
                        {
                            ExcelWorksheet ws = xp.Workbook.Worksheets.Add("Horse Treadmill");
                            int rowstart = 1;
                            int colstart = 1;
                            int rowend = rowstart;
                            ...
                            Response.AddHeader("content-disposition", "attachment;filename=Horse_Treadmill.xlsx");
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                using (DataSet ds = new MasterHorseBL().GetExport(_horseId, "Horse_Treadmill"))
                {
