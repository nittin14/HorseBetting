...
namespace VKATalk.PopUps
{
    ...
    public partial class ProfessionalJockeyAllowanceStage : System.Web.UI.Page
    {
        int _userId = 1;
        //const string professionialname =string.Empty;
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                if (!IsPostBack)
                {
                    ...
                    try
                    {
                        BindDropDown(drpdwnJockeyAllowanceStage, "JockeyAllowanceStage", "JockeyAllowanceStage", "JockeyAllowanceStagePMID");
                        drpdwnJockeyAllowanceStage.Items.Insert(0, new ListItem("-- Please select --", "-1"));
                        BindData();
                    }
                    catch (Exception ex)
                    {
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    {
                        status = new ProfessionalBL().JockeyAllowanceStage(
                            Convert.ToInt32(hdnfieldProfessionalNameID.Value),
                            Convert.ToInt32(drpdwnJockeyAllowanceStage.SelectedItem.Value),
                            txtbxStagestartDate.Text,
                            txtbxStartDayRaceNo.Text,
                            txtbxEndDayRaceNo.Text,
                            txtbxStageendDate.Text,
                            1,
        ...
        {
                DataSet ds = new ProfessionalBL().GetProfessionalNameWithCombination(Convert.ToInt32(hdnfieldProfessionalNameID.Value), "Professional_JockeyAllowanceStage");
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    var status = new ProfessionalBL().JockeyAllowanceStage(
                            (int)ViewState["ProfessionalRecordID"],
                             Convert.ToInt32(drpdwnJockeyAllowanceStage.SelectedItem.Value),
                            txtbxStagestartDate.Text,
                            txtbxStagestartDate.Text,
        ...
        protected void GvHomeDistance_OnSelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
				...
				if (dataKey != null)
				{
                    ...
                    drpdwnJockeyAllowanceStage.Items.FindByText(hdnval.Value).Selected = true;
                    if (!row.Cells[1].Text.Contains("&nbsp;"))
                    {
                        txtbxStagestartDate.Text = row.Cells[1].Text;
                    }
                    if (!row.Cells[2].Text.Contains("&nbsp;"))
                    {
                        txtbxStartDayRaceNo.Text = row.Cells[2].Text;
                    }
                    if (!row.Cells[3].Text.Contains("&nbsp;"))
                    {
                        txtbxEndDayRaceNo.Text = row.Cells[3].Text;
                    }
                    if (!row.Cells[4].Text.Contains("&nbsp;"))
                    {
                        txtbxStageendDate.Text = row.Cells[4].Text;
                    }
					if (row.Cells[5].Text.Contains("&quot;"))
					{
						txtbxCommentNew.Text = row.Cells[5].Text.Equals("&nbsp;") ? "" : row.Cells[5].Text.Contains("&quot;") ? row.Cells[5].Text.Replace("&quot;", "\"") : row.Cells[5].Text;
					}
					else
					{
						txtbxCommentNew.Text = row.Cells[5].Text.Equals("&nbsp;") ? "" : row.Cells[5].Text.Contains("&#39;") ? row.Cells[5].Text.Replace("&#39;", "'") : row.Cells[5].Text;
        ...
        protected void btnFileUpload_Click(object sender, EventArgs e)
        {
            try
            {
                if (flupload.HasFile)
                {
                    ...
                    if (Path.GetFileNameWithoutExtension(flupload.PostedFile.FileName).Equals("Professional_JockeyAllowanceStage"))
                    {
        ...
        private void Import_To_Grid(string FilePath, string Extension)
        {
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    var dtErrorResult = new ProfessionalBL().Import30(dt, "Professional_JockeyAllowanceStage");
                    if (dtErrorResult.Rows.Count > 0)
                    {
                        ...
                        {
                            ExcelWorksheet ws = xp.Workbook.Worksheets.Add("Professional_JockeyAllowanceStage");
                            int rowstart = 1;
                            int colstart = 1;
                            int rowend = rowstart;
                            ...
                            Response.AddHeader("content-disposition", "attachment;filename=Professional_JockeyAllowanceStage.xlsx");
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            try
            {
                using (DataSet ds = new ProfessionalBL().GetExport(Convert.ToInt32(hdnfieldProfessionalNameID.Value), "Professional_JockeyAllowanceStage"))
                {
