...
namespace VKATalk.PopUps
{
    ...
    public partial class StandingNation : System.Web.UI.Page
    {
        int _horseId = 0;
        int _userId = 1;
            ...
            try
            {
                ...
                if (!IsPostBack)
                {
                    if (_horseId != 0)
                    {
                        ...
                        BindDropDown(this.drpdwnGender, "MasterNation", "Nation", "NationID");
                        drpdwnGender.Items.Insert(0, new ListItem("-- Please select --", "-1"));
                        drpdwnGender.Items.FindByText("India").Selected = true;
                        BindData();
                    }
                }
            }
        ...
        public void BindData()
        {
                ...
                DataTable dt = Bl.GetHorseDetail(Convert.ToString(_horseId), "MasterNation");
                if (dt.Rows.Count > 0)
                {
                    this.GvStandingNation.DataSource = dt;
                    this.GvStandingNation.DataBind();
                }
                else
                {
                    this.GvStandingNation.DataSource = new DataTable();
                    this.GvStandingNation.DataBind();
                }
        }
        ...
        {
            DataTable dt = new MasterHorseBL().GetHorseNameAutoFiller("HorseStandingNation", prefixText);
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                //{
                    ...
                    {
                        ds = new MasterHorseBL().GetHorseTillDateValidation(_horseId, "HorseStandingNation", txtbxFromDate.Text, btnSave.Text);
                    }
                    else
                    {
                        ds = new MasterHorseBL().GetHorseTillDateValidation((int)ViewState["horseStatusID"], "HorseStandingNation", txtbxFromDate.Text, btnSave.Text);
                    }
                    ...
                    else
                    {
                        if (btnSave.Text.Equals("Add"))
                        {
                            status = new MasterHorseBL().HorseStandingNation(
                                _horseId,
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    new MasterHorseBL().HorseStandingNation((int)ViewState["horseStatusID"], Convert.ToInt32(this.drpdwnGender.SelectedItem.Value), txtbxFromDate.Text, txtbxTillDate.Text,
        ...
        protected void GvStandingNation_OnSelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                btnSave.Text = "Update";
                GridViewRow row = this.GvStandingNation.SelectedRow;
                HiddenField hdnval = (HiddenField)row.FindControl("hdnfieldStatus");
                var dataKey = GvStandingNation.DataKeys[row.RowIndex];
                if (dataKey != null)
                {
                    ClearAllSelection(this);
        ...
        protected void btnFileUpload_Click(object sender, EventArgs e)
        {
            try
            {
                if (flupload.HasFile)
                {
                    ...
                    if (Path.GetFileNameWithoutExtension(flupload.PostedFile.FileName).Equals("Horse_StandingNation"))
                    {
        ...
        private void Import_To_Grid(string FilePath, string Extension)
        {
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    var dtErrorResult = new MasterHorseBL().Import30(dt, "HorseStandingNation");
                    if (dtErrorResult.Rows.Count > 0)
                    {
                        ...
                        {
                            ExcelWorksheet ws = xp.Workbook.Worksheets.Add("Horse_StandingNation");
                            int rowstart = 1;
                            int colstart = 1;
                            int rowend = rowstart;
                            ...
                            Response.AddHeader("content-disposition", "attachment;filename=Horse_StandingNation.xlsx");
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            try
            {
                //using (DataSet ds = new MasterHorseBL().GetHorseNameWithCombination(_horseId, "HorseStandingNation"))
                using (DataSet ds = new MasterHorseBL().GetExport(_horseId, "HorseStandingNation"))
                {
                    DataTable dt = ds.Tables[0];
                    if (dt.Rows.Count > 0)
                    {
                        //dt.Columns.Remove("StandingNationID");
                        using (ExcelPackage xp = new ExcelPackage())
                        {
                            ExcelWorksheet ws = xp.Workbook.Worksheets.Add("Horse_StandingNation");
                            int rowstart = 1;
