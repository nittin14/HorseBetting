...
namespace VKATalk.PopUps
{
    ...
    public partial class HorseBit : System.Web.UI.Page
    {
        int _horseId = 0;
        int _userId = 1;
        ...
        public void GetHorseInformation()
        {
            ...
            BindDropDown(this.drpdwnGender, "HorseBit", "Bit", "BitID");
        ...
        public void BindData()
        {
            ...
            DataTable dt = Bl.GetHorseDetail(Convert.ToString(_horseId), "HorseBit");
            if (dt.Rows.Count > 0)
            {
        ...
        {
            DataTable dt = new MasterHorseBL().GetHorseNameAutoFiller("HorseBit", prefixText);
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                //{
                //    ds = new MasterHorseBL().GetHorseTillDateValidationMultiple(_horseId, "HorseBit", txtbxFromDate.Text, btnSave.Text, drpdwnGender.SelectedItem.Value, string.Empty);
                //}
                //else
                //{
                //    ds = new MasterHorseBL().GetHorseTillDateValidationMultiple((int)ViewState["horseStatusID"], "HorseBit", txtbxFromDate.Text, btnSave.Text, drpdwnGender.SelectedItem.Value, string.Empty);
                //}
                ...
                else
                {
                    if (btnSave.Text.Equals("Add"))
                    {
                        status = new MasterHorseBL().HorseBit(
                            _horseId,
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    new MasterHorseBL().HorseBit((int)ViewState["horseStatusID"], Convert.ToInt32(this.drpdwnGender.SelectedItem.Value), txtbxFromDate.Text, txtbxTillDate.Text,
        ...
        protected void btnFileUpload_Click(object sender, EventArgs e)
        {
            try
            {
                if (flupload.HasFile)
                {
                    ...
                    if (Path.GetFileNameWithoutExtension(flupload.PostedFile.FileName).Equals("Horse_Bit"))
                    {
        ...
        private void Import_To_Grid(string FilePath, string Extension)
        {
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    var dtErrorResult = new MasterHorseBL().Import30(dt, "HorseBit");
                    if (dtErrorResult.Rows.Count > 0)
                    {
                        ...
                        {
                            ExcelWorksheet ws = xp.Workbook.Worksheets.Add("Horse Bit");
                            int rowstart = 1;
                            int colstart = 1;
                            int rowend = rowstart;
                            ...
                            Response.AddHeader("content-disposition", "attachment;filename=Horse_Bit.xlsx");
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            try
            {
                //using (DataSet ds = new MasterHorseBL().GetHorseNameWithCombination(_horseId, "HorseBit"))
                using (DataSet ds = new MasterHorseBL().GetExport(_horseId, "HorseBit"))
                {
                    DataTable dt = ds.Tables[0];
                    if (dt.Rows.Count > 0)
                    {
                        // dt.Columns.Remove("HorseBitID");
                        using (ExcelPackage xp = new ExcelPackage())
                        {
                            ExcelWorksheet ws = xp.Workbook.Worksheets.Add("HorseBit");
                            int rowstart = 1;
                            int colstart = 1;
                            int rowend = rowstart;
