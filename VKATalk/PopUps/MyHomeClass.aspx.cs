...
namespace VKATalk.PopUps
{
    ...
    public partial class MyHomeClass: System.Web.UI.Page
    {
        int _userId = 1;
        int _horseId = 0;
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    ds = new MasterHorseBL().GetHorseTillDateValidationMultiple(_horseId, "MyHomeClass", txtbxFromDate.Text, btnSave.Text, drpdwnHomeDistance.SelectedItem.Value, drpdwnSupportType.SelectedItem.Value);
                }
                else
                {
                    ds = new MasterHorseBL().GetHorseTillDateValidationMultiple((int)ViewState["horseRecordID"], "MyHomeClass", txtbxFromDate.Text, btnSave.Text, drpdwnHomeDistance.SelectedItem.Value, drpdwnSupportType.SelectedItem.Value);
                }
                ...
                else
                {
                    if (btnSave.Text.Equals("Add"))
                    {
                        status = new MasterHorseBL().MyHomeClass(
                            _horseId,
        ...
        {
            DataTable dt = new MasterHorseBL().GetHorseNameAutoFiller("MyHomeClass", prefixText);
        ...
        {
                DataSet ds = new MasterHorseBL().GetHorseNameWithCombination(_horseId, "MyHomeClass");
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                if (!ViewState["horseRecordID"].Equals(""))
                {
                    ...
                    var status = new MasterHorseBL().MyHomeClass((int)ViewState["horseRecordID"],0,0,"","__-__-____","__-__-____","",this._userId,"Delete");
                    if (status > 0)
                    {
                        ClearAllSelection(this);
                        BindData();
        ...
        protected void btnFileUpload_Click(object sender, EventArgs e)
        {
            try
            {
                if (flupload.HasFile)
                {
                    ...
                    if (Path.GetFileNameWithoutExtension(flupload.PostedFile.FileName).Equals("Horse_MyHomeClass"))
                    {
        ...
        private void Import_To_Grid(string FilePath, string Extension)
        {
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    var dtErrorResult = new MasterHorseBL().Import30(dt, "HorseMyHomeClass");
                    if (dtErrorResult.Rows.Count > 0)
                    {
                        ...
                        {
                            ExcelWorksheet ws = xp.Workbook.Worksheets.Add("Horse_MyHomeClass");
                            int rowstart = 1;
                            int colstart = 1;
                            int rowend = rowstart;
                            ...
                            Response.AddHeader("content-disposition", "attachment;filename=Horse_MyHomeClass.xlsx");
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            try
            {
                //using (DataSet ds = new MasterHorseBL().GetHorseNameWithCombination(_horseId, "HorseMyHomeClass"))
                using (DataSet ds = new MasterHorseBL().GetExport(_horseId, "HorseMyHomeClass"))
                {
                    DataTable dt = ds.Tables[0];
                    if (dt.Rows.Count > 0)
                    {
                       // dt.Columns.Remove("MyHomeClassID");
                        using (ExcelPackage xp = new ExcelPackage())
                        {
                            ExcelWorksheet ws = xp.Workbook.Worksheets.Add("Horse_MyHomeClass");
                            int rowstart = 1;
