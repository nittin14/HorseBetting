...
namespace VKATalk.PopUps
{
    ...
    public partial class TrainerRecord : System.Web.UI.Page
    {
        int _horseId = 0;
        int _userId = 1;
        ...
        {
            DataTable dt = new MasterHorseBL().GetHorseNameAutoFiller("ProfessionalNameTrainer", prefixText);
        ...
        public void BindData()
        {
                ...
                DataTable dt = Bl.GetHorseDetail(Convert.ToString(_horseId), "TrainerRecord");
                if (dt.Rows.Count > 0)
                {
        ...
        {
            DataTable dt = new MasterHorseBL().GetHorseNameAutoFiller("TrainerRecordComment", prefixText);
        ...
        {
            DataTable dt = new MasterHorseBL().GetHorseNameAutoFiller("TrainerRecordReasonofChange", prefixText);
            List<string> currentMissionComments = new List<string>();
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                currentMissionComments.Add(dt.Rows[i][0].ToString());
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    {
                        ds = new MasterHorseBL().GetHorseTillDateValidation(_horseId, "HorseTrainerRecord", txtbxFromDate.Text, btnSave.Text);
                    }
                    else
                    {
                        ds = new MasterHorseBL().GetHorseTillDateValidation((int)ViewState["horseStatusID"], "HorseTrainerRecord", txtbxFromDate.Text, btnSave.Text);
                    }
                    ...
                    else
                    {
						...
						if (btnSave.Text.Equals("Add"))
                        {
                            status = new MasterHorseBL().HorseTrainerRecord(
                                _horseId,
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    new MasterHorseBL().HorseTrainerRecord((int)ViewState["horseStatusID"], 0, txtbxFromDate.Text, txtbxTillDate.Text,
        ...
        protected void btnFileUpload_Click(object sender, EventArgs e)
        {
            try
            {
                if (flupload.HasFile)
                {
                    ...
                    if (Path.GetFileNameWithoutExtension(flupload.PostedFile.FileName).Equals("Horse_Trainer"))
                    {
        ...
        private void Import_To_Grid(string FilePath, string Extension)
        {
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    var dtErrorResult = new MasterHorseBL().Import30(dt, "HorseTrainerRecord");
                    if (dtErrorResult.Rows.Count > 0)
                    {
                        ...
                        {
                            ExcelWorksheet ws = xp.Workbook.Worksheets.Add("Horse_Trainer");
                            int rowstart = 1;
                            int colstart = 1;
                            int rowend = rowstart;
                            ...
                            Response.AddHeader("content-disposition", "attachment;filename=Horse_Trainer.xlsx");
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            try
            {
                //using (DataSet ds = new MasterHorseBL().GetHorseNameWithCombination(_horseId, "HorseTrainerRecord"))
                using (DataSet ds = new MasterHorseBL().GetExport(_horseId, "HorseTrainerRecord"))
                {
                    DataTable dt = ds.Tables[0];
                    if (dt.Rows.Count > 0)
                    {
                    //    dt.Columns.Remove("TrainerRecordID");
                        using (ExcelPackage xp = new ExcelPackage())
                        {
                            ExcelWorksheet ws = xp.Workbook.Worksheets.Add("Horse_Trainer");
                            int rowstart = 1;
