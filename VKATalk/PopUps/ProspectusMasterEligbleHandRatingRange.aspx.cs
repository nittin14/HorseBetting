...
namespace VKATalk.PopUps
{
    ...
    public partial class ProspectusMasterEligbleHandRatingRange : System.Web.UI.Page
    {
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            ...
            {
                int status = 0;
                //DataSet ds = new ProspectusBL().GetProspectusTillDateValidation(_masterraceId, "MasterEligbleRatingRange", this.btnSave.Text);
                DataSet ds = null;
                if (btnSave.Text.Equals("Add"))
                {
                    ds = new ProspectusBL().GetProspectusTillDateValidation(_masterraceId, "MasterEligbleRatingRange", drpdwnFromYear.SelectedItem.Text, btnSave.Text);
                }
                else
                {
                    ds = new ProspectusBL().GetProspectusTillDateValidation((int)ViewState["MasterRaceID"], "MasterEligbleRatingRange", drpdwnFromYear.SelectedItem.Text, btnSave.Text);
                }
                ...
                else
                {
                    if (btnSave.Text.Equals("Add"))
                    {
                        status = new ProspectusBL().MasterEligbleRatingRange(
                            _masterraceId,
        ...
        {
            DataTable dt = new ProspectusBL().GetprospectusAutoFiller("ProspectusMasterEligbleHandicapRatingRange", prefixText);
        ...
        private void BindData()
        {
            try
            {
                ...
                DataSet ds = new ProspectusBL().GetProspectusNameWithCombination(_masterraceId, "MasterEligbleHandicapRatingRange");
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                if (!ViewState["MasterRaceID"].Equals(""))
                {
                    ...
                    var status = new ProspectusBL().MasterEligbleRatingRange((int)ViewState["MasterRaceID"], 0,0,0,txtbxComment.Text, 1, "Delete","__-__-____");
        ...
        protected void btnFileUpload_Click(object sender, EventArgs e)
        {
            try
            {
                if (flupload.HasFile)
                {
                    ...
                    if (Path.GetFileNameWithoutExtension(flupload.PostedFile.FileName).Equals("Prospectus_Master_EligbleHandicapRatingRange"))
                    {
        ...
        private void Import_To_Grid(string FilePath, string Extension)
        {
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    var dtErrorResult = new ProspectusBL().Import30(dt, "ProspectMasterEligbleHandicapRatingRange", _masterraceId);
                    if (dtErrorResult.Rows.Count > 0)
                    {
                       // Page.ClientScript.RegisterStartupScript(this.GetType(), "Popup", "alert('Issue in few record. Please check the XL sheet');", true);
                        using (ExcelPackage xp = new ExcelPackage())
                        {
                            ...
                            ExcelWorksheet ws = xp.Workbook.Worksheets.Add("ProspectMasterEligbleHandicapRatingRange");
                            ...
                            Response.AddHeader("content-disposition", "attachment;filename=Prospectus_Master_EligbleHandicapRatingRange.xlsx");
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                //using (DataSet ds = new ProspectusBL().GetProspectusNameWithCombination(_masterraceId, "MasterEligbleHandicapRatingRange"))
                using (DataSet ds = new ProspectusBL().GetExport(_masterraceId, "MasterEligbleHandicapRatingRange"))
                {
                    DataTable dt = ds.Tables[0];
                    if (dt.Rows.Count > 0)
                    {
                       // dt.Columns.Remove("ERRID");
                        using (ExcelPackage xp = new ExcelPackage())
                        {
