...
namespace VKATalk.PopUps
{
    ...
    public partial class ProfessionalBackground : System.Web.UI.Page
    {
        int _userId = 1;
                ...
                if (!IsPostBack)
                {
                    try
                    {
                        ...
                        if (_professionalId != 0)
                        {
							...
							BindDropDown(this.drpdwnProfBackground, "ProfessionalBackground", "ProfessionalBackground", "ProfessionalBackgroundID");
							drpdwnProfBackground.Items.Insert(0, new ListItem("-- Please select --", "-1"));
							BindData();
                        }
                    }
                    catch (Exception ex)
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
				...
				//{
				//    ds = new ProfessionalBL().GetProfessionalTillDateValidation(_professionalId, "ProfessionalBackground", txtbxFromDate.Text, btnSave.Text);
				//}
				//else
				//{
				//    ds = new ProfessionalBL().GetProfessionalTillDateValidation((int)ViewState["ProfessionalRecordID"], "ProfessionalBackground", txtbxFromDate.Text, btnSave.Text);
				//}

				//if (btnSave.Text.Equals("Add"))
				//{
				//    ds = new ProfessionalBL().GetProfessionalTillDateValidationProfile(_professionalId, "ProfessionalBackGround", txtbxFromDate.Text, btnSave.Text,Convert.ToInt32(drpdwnProfBackground.SelectedItem.Value));
				//}
				//else
				//{
				//    ds = new ProfessionalBL().GetProfessionalTillDateValidationProfile((int)ViewState["ProfessionalRecordID"], "ProfessionalBackGround", txtbxFromDate.Text, btnSave.Text, Convert.ToInt32(drpdwnProfBackground.SelectedItem.Value));
				//}
                ...
                else
                {
                    if (btnSave.Text.Equals("Add"))
                    {
                        status = new ProfessionalBL().ProfessionalBackground(
                            _professionalId,
							Convert.ToInt32(drpdwnProfBackground.SelectedItem.Value),
                              txtbxFromDate.Text,
                                txtbxTillDate.Text,
                            txtbxComment.Text,
                            _userId,
        ...
        {
            var dt = new ProfessionalBL().GetProfessionalNameAutoFiller("Religion", prefixText);
        ...
        {
            var dt = new ProfessionalBL().GetProfessionalNameAutoFiller("ProfessionalBackground", prefixText);
        ...
        {
            var ds = new ProfessionalBL().GetProfessionalNameWithCombination(_professionalId, "ProfessionalBackground");
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    var status = new ProfessionalBL().ProfessionalBackground((int)ViewState["ProfessionalRecordID"], 0,"__-__-____","__-__-____", "", _userId, "Delete");
                    if (status > 0)
                    {
                        ClearAllSelection(this);
                        BindData();
                ...
                else
                {
                    ...
                    Page.ClientScript.RegisterStartupScript(this.GetType(), "Popup", "alert('" + message + "');", true);
                }
        ...
        protected void GvHorseProfile_OnSelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                //var hdnvalprofessionalnameid = (HiddenField)row.FindControl("hdnfieldmasterprofessionalid");
                //   hdnfieldprofessionalbackground.Value = hdnvalprofessionalnameid.Value;
                var dataKey = GvHorseProfile.DataKeys[row.RowIndex];
                if (dataKey != null)
                {
                    ClearAllSelection(this);
                    ViewState["ProfessionalRecordID"] = dataKey.Value;
					drpdwnProfBackground.Items.FindByText(hdnval.Value).Selected = true;
					//Convert.ToInt32(drpdwnProfBackground.SelectedItem.Value)
        ...
        protected void btnFileUpload_Click(object sender, EventArgs e)
        {
            try
            {
                if (flupload.HasFile)
                {
                    ...
                    if (Path.GetFileNameWithoutExtension(flupload.PostedFile.FileName).Equals("Professional_Profession"))
                    {
        ...
        private void Import_To_Grid(string FilePath, string Extension)
        {
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    var dtErrorResult = new ProfessionalBL().Import30(dt, "ProfessionalBackground");
                    if (dtErrorResult.Rows.Count > 0)
                    {
                        ...
                        {
                            ExcelWorksheet ws = xp.Workbook.Worksheets.Add("Professional_Profession");
                            int rowstart = 1;
                            int colstart = 1;
                            int rowend = rowstart;
                            ...
                            Response.AddHeader("content-disposition", "attachment;filename=Professional_Profession.xlsx");
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            try
            {
              //  using (DataSet ds = new ProfessionalBL().GetProfessionalNameWithCombination(_professionalId, "ProfessionalBaseCenter"))
                using (DataSet ds = new ProfessionalBL().GetExport(_professionalId, "ProfessionalBackground"))
                {
                    DataTable dt = ds.Tables[0];
                    if (dt.Rows.Count > 0)
                    {
                     //   dt.Columns.Remove("ProfessionalBaseCenterID");
                        using (ExcelPackage xp = new ExcelPackage())
                        {
