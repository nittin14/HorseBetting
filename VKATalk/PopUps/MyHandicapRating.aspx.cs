...
namespace VKATalk.PopUps
{
    ...
    public partial class MyHandicapRating : System.Web.UI.Page
    {
        int _horseId = 0;
        int _userId = 1;
       ...
       /// <summary>
        ...
        public void BindData()
        {
                ...
                DataTable dt = Bl.GetHorseDetail(Convert.ToString(_horseId), "MyHandicapRating");
                if (dt.Rows.Count > 0)
                {
        ...
        {
            DataTable dt = new MasterHorseBL().GetHorseNameAutoFiller("MyHandicapRating", prefixText);
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                   // ds = new MasterHorseBL().GetHorseTillDateValidation(_horseId, "MyHandicapRating", txtbxFromDate.Text,btnSave.Text);
                }
                else
                {
                    //ds = new MasterHorseBL().GetHorseTillDateValidation((int)ViewState["horseStatusID"], "MyHandicapRating", txtbxFromDate.Text, btnSave.Text);
                }
                ...
                else
                {
                    if (btnSave.Text.Equals("Add"))
                    {
                        status = new MasterHorseBL().MyHandicapRating(
                            _horseId,
                            txtbxhandicapRating.Text,
                            txtbxFromDate.Text,
                            txtbxTillDate.Text,
                            txtbxComment.Text,
                            1,
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    new MasterHorseBL().MyHandicapRating((int)ViewState["horseStatusID"], "", txtbxFromDate.Text, txtbxTillDate.Text, txtbxComment.Text, 1, "Delete");
                    ClearAllSelection(this);
                    BindData();
                    btnSave.Text = "Add";
        ...
        protected void btnFileUpload_Click(object sender, EventArgs e)
        {
            try
            {
                if (flupload.HasFile)
                {
                    ...
                    if (Path.GetFileNameWithoutExtension(flupload.PostedFile.FileName).Equals("Horse_MyHandicapRating"))
                    {
        ...
        private void Import_To_Grid(string FilePath, string Extension)
        {
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    var dtErrorResult = new MasterHorseBL().Import30(dt, "HorseMyHandicapRating");
                    if (dtErrorResult.Rows.Count > 0)
                    {
                        ...
                        {
                            ExcelWorksheet ws = xp.Workbook.Worksheets.Add("Horse_MyHandicapRating");
                            int rowstart = 1;
                            int colstart = 1;
                            int rowend = rowstart;
                            ...
                            Response.AddHeader("content-disposition", "attachment;filename=Horse_MyHandicapRating.xlsx");
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            try
            {
              //  using (DataSet ds = new MasterHorseBL().GetHorseNameWithCombination(_horseId, "HorseMyHandicapRating"))
                using (DataSet ds = new MasterHorseBL().GetExport(_horseId, "HorseMyHandicapRating"))
                {
                    DataTable dt = ds.Tables[0];
                    if (dt.Rows.Count > 0)
                    {
                   //     dt.Columns.Remove("MyHandicapRatingID");
                        using (ExcelPackage xp = new ExcelPackage())
                        {
                            ExcelWorksheet ws = xp.Workbook.Worksheets.Add("Horse_MyHandicapRating");
                            int rowstart = 1;
