...
namespace VKATalk.PopUps
{
    ...
    public partial class HorseShoe : System.Web.UI.Page
    {
        int _horseId = 0;
        int _userId = 1;
		...
		public void horseinformation()
		{
			...
			BindDropDown(this.drpdwnGender, "HorseShoe", "Shoe", "ShoeMID");
        ...
        public void BindData()
        {
                ...
                DataTable dt = Bl.GetHorseDetail(Convert.ToString(_horseId), "HorseShoe");
                if (dt.Rows.Count > 0)
                {
        ...
        {
            DataTable dt = new MasterHorseBL().GetHorseNameAutoFiller("HorseShoe", prefixText);
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    ds = new MasterHorseBL().GetHorseTillDateValidation(_horseId, "HorseShoe", txtbxFromDate.Text, btnSave.Text);
                }
                else
                {
                    ds = new MasterHorseBL().GetHorseTillDateValidation((int)ViewState["horseStatusID"], "HorseShoe", txtbxFromDate.Text, btnSave.Text);
                }
                ...
                else
                {
                    if (btnSave.Text.Equals("Add"))
                    {
                        status = new MasterHorseBL().HorseShoe(
                            _horseId,
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    new MasterHorseBL().HorseShoe((int)ViewState["horseStatusID"], Convert.ToInt32(this.drpdwnGender.SelectedItem.Value), txtbxFromDate.Text, txtbxTillDate.Text,
        ...
        protected void btnFileUpload_Click(object sender, EventArgs e)
        {
            try
            {
                if (flupload.HasFile)
                {
                    ...
                    if (Path.GetFileNameWithoutExtension(flupload.PostedFile.FileName).Equals("Horse_Shoe"))
                    {
        ...
        private void Import_To_Grid(string FilePath, string Extension)
        {
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    var dtErrorResult = new MasterHorseBL().Import30(dt, "HorseShoe");
                    if (dtErrorResult.Rows.Count > 0)
                    {
                        ...
                        {
                            ExcelWorksheet ws = xp.Workbook.Worksheets.Add("Horse_Shoe");
                            int rowstart = 1;
                            int colstart = 1;
                            int rowend = rowstart;
                            ...
                            Response.AddHeader("content-disposition", "attachment;filename=Horse_Shoe.xlsx");
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            try
            {
                //using (DataSet ds = new MasterHorseBL().GetHorseNameWithCombination(_horseId, "HorseShoe"))
                using (DataSet ds = new MasterHorseBL().GetExport(_horseId, "HorseShoe"))
                {
                    DataTable dt = ds.Tables[0];
                    if (dt.Rows.Count > 0)
                    {
                     //   dt.Columns.Remove("HorseShoeID");
                        using (ExcelPackage xp = new ExcelPackage())
                        {
                            ExcelWorksheet ws = xp.Workbook.Worksheets.Add("Horse_Shoe");
                            int rowstart = 1;
