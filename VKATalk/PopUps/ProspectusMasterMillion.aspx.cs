...
namespace VKATalk.PopUps
{
    ...
    public partial class ProspectusMasterMillion : System.Web.UI.Page
    {
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            ...
            {
                int status = 0;
                //DataSet ds = new ProspectusBL().GetProspectusTillDateValidation(_masterraceId, "MasterMillion", this.btnSave.Text);
                DataSet ds = null;
                if (!chkbxMillion.Checked)
                {
                    var message = "Please select Million.";
                    Page.ClientScript.RegisterStartupScript(this.GetType(), "Popup", "ShowPopup('" + message + "');", true);
                }
                else
                {
                    ...
                    {
                        ds = new ProspectusBL().GetProspectusTillDateValidation(_masterraceId, "MasterMillion", drpdwnFromYear.SelectedItem.Text, btnSave.Text);
                    }
                    else
                    {
                        ds = new ProspectusBL().GetProspectusTillDateValidation((int)ViewState["MasterRaceID"], "MasterMillion", drpdwnFromYear.SelectedItem.Text, btnSave.Text);
                    }
                    ...
                    else
                    {
                        if (btnSave.Text.Equals("Add"))
                        {
                            status = new ProspectusBL().MasterMillion(
                                _masterraceId,
                                "Yes",
        ...
        private void BindData()
        {
            try
            {
                ...
                DataSet ds = new ProspectusBL().GetProspectusNameWithCombination(_masterraceId, "MasterMillion");
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                if (!ViewState["MasterRaceID"].Equals(""))
                {
                    ...
                    var status = new ProspectusBL().MasterMillion((int)ViewState["MasterRaceID"], "",0,0,txtbxComment.Text, 1, "Delete");
            ...
            try
            {
                ...
                if (dataKey != null)
                {
                   ...
                   // rdbtnMillion.Items.FindByText(hdnval.Value).Selected = true;
                    if (hdnval.Value.Equals("Million"))
                    {
        ...
        protected void btnFileUpload_Click(object sender, EventArgs e)
        {
            try
            {
                if (flupload.HasFile)
                {
                    ...
                    if (Path.GetFileNameWithoutExtension(flupload.PostedFile.FileName).Equals("Prospectus_Master_Million"))
                    {
        ...
        private void Import_To_Grid(string FilePath, string Extension)
        {
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    var dtErrorResult = new ProspectusBL().Import30(dt, "MasterMillion", _masterraceId);
                    if (dtErrorResult.Rows.Count > 0)
                    {
                        using (ExcelPackage xp = new ExcelPackage())
                        {

                            ExcelWorksheet ws = xp.Workbook.Worksheets.Add("MasterMillion");
                            ...
                            Response.AddHeader("content-disposition", "attachment;filename=Prospectus_Master_Million.xlsx");
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                using (DataSet ds = new ProspectusBL().GetExport(_masterraceId, "MasterMillion"))
                {
