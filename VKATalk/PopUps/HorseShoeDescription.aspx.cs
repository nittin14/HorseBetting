...
namespace VKATalk.PopUps
{
    ...
    public partial class HorseShoeDescription : System.Web.UI.Page
    {
        int _horseId = 0;
        int _userId = 1;
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                if (!IsPostBack)
                {
                    BindDropDown(this.drpdwnGender, "HorseShoeMetal", "ShoeMetal", "ShoeMetalMID");
        ...
        public void BindData()
        {
                ...
                DataTable dt = Bl.GetHorseDetail(Convert.ToString(_horseId), "HorseShoeDescription");
                if (dt.Rows.Count > 0)
                {
        ...
        {
            DataTable dt = new MasterHorseBL().GetHorseNameAutoFiller("HorseShoeDescription", prefixText);
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    ds = new MasterHorseBL().GetHorseTillDateValidation(_horseId, "HorseShoeDescription", txtbxFromDate.Text, btnSave.Text);
                }
                else
                {
                    ds = new MasterHorseBL().GetHorseTillDateValidation((int)ViewState["horseStatusID"], "HorseShoeDescription", txtbxFromDate.Text, btnSave.Text);
                }
                ...
                else
                {
                    if (btnSave.Text.Equals("Add"))
                    {
                        status = new MasterHorseBL().HorseShoeDescription(
                            _horseId,
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    new MasterHorseBL().HorseShoeDescription((int)ViewState["horseStatusID"], Convert.ToInt32(this.drpdwnGender.SelectedItem.Value), txtbxFromDate.Text, txtbxTillDate.Text,
        ...
        protected void btnFileUpload_Click(object sender, EventArgs e)
        {
            try
            {
                if (flupload.HasFile)
                {
                    ...
                    if (Path.GetFileNameWithoutExtension(flupload.PostedFile.FileName).Equals("Horse_ShoeMetal"))
                    {
        ...
        private void Import_To_Grid(string FilePath, string Extension)
        {
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    var dtErrorResult = new MasterHorseBL().Import30(dt, "HorseShoeDescription");
                    if (dtErrorResult.Rows.Count > 0)
                    {
                        ...
                        {
                            ExcelWorksheet ws = xp.Workbook.Worksheets.Add("Horse_ShoeMetal");
                            int rowstart = 1;
                            int colstart = 1;
                            int rowend = rowstart;
                            ...
                            Response.AddHeader("content-disposition", "attachment;filename=Horse_ShoeMetal.xlsx");
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            try
            {
                //using (DataSet ds = new MasterHorseBL().GetHorseNameWithCombination(_horseId, "HorseShoeDescription"))
                using (DataSet ds = new MasterHorseBL().GetExport(_horseId, "HorseShoeDescription"))
                {
                    DataTable dt = ds.Tables[0];
                    if (dt.Rows.Count > 0)
                    {
                     //   dt.Columns.Remove("HorseShoeDescriptionID");
                        using (ExcelPackage xp = new ExcelPackage())
                        {
                            ExcelWorksheet ws = xp.Workbook.Worksheets.Add("Horse_ShoeMetal");
                            int rowstart = 1;
