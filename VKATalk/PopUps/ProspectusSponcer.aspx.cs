...
namespace VKATalk.PopUps
{
    ...
    public partial class ProspectusSponcer : System.Web.UI.Page
    {
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
				...
				{
					status = new ProspectusBL().Sponcer(
						_masterraceId,
				...
				//DataSet ds = new ProspectusBL().GetProspectusTillDateValidation(_masterraceId, "Sponcer", this.btnSave.Text);
				//DataSet ds = null;
				//DataSet ds1 = new ProspectusBL().GetProspectusNameWithCombination(_masterraceId, "Sponcer");
				//var checkstatus = 0;
				//if (ds1.Tables[0].Rows.Count > 0)
				//{
				//    for (int count = 0; count <= ds1.Tables[0].Rows.Count - 1; count++)
				//    {
				//        if (ds1.Tables[0].Rows[count][1].ToString().Equals(hdnfieldSponcerNameID.Value))
				//        {
				//            checkstatus = 1;
				//        }
				//    }
				...
				//    if (btnSave.Text.Equals("Add"))
				//    {
				//        ds = new ProspectusBL().GetProspectusTillDateValidation(_masterraceId, "Sponcer", drpdwnFromYear.SelectedItem.Text, btnSave.Text);
				//    }
				//    else
				//    {
				//        ds = new ProspectusBL().GetProspectusTillDateValidation((int)ViewState["MasterRaceID"], "Sponcer", drpdwnFromYear.SelectedItem.Text, btnSave.Text);
				//    }
        ...
        public static List<string> AddCommentsList(string prefixText, int count)
        {
            ...
            DataTable dt = new ProspectusBL().GetprospectusAutoFiller("SponcerCommentList", prefixText);
        ...
        {
            DataTable dt = new ProspectusBL().GetprospectusAutoFiller("SponcerOtherList", prefixText);
            List<string> commentList = new List<string>();
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                commentList.Add(dt.Rows[i][0].ToString());
        ...
        private void BindData()
        {
            try
            {
                ...
                DataSet ds = new ProspectusBL().GetProspectusNameWithCombination(_masterraceId, "Sponcer");
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                if (!ViewState["MasterRaceID"].Equals(""))
                {
                    ...
                    var status = new ProspectusBL().Sponcer((int)ViewState["MasterRaceID"], 0,0, 0,"",txtbxComment.Text, 1, "Delete");
        ...
        protected void btnFileUpload_Click(object sender, EventArgs e)
        {
            try
            {
                if (flupload.HasFile)
                {
                    ...
                    if (Path.GetFileNameWithoutExtension(flupload.PostedFile.FileName).Equals("Prospectus_Master_SponcerOfTheRace"))
                    {
        ...
        private void Import_To_Grid(string FilePath, string Extension)
        {
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    //var dtErrorResult = new ProspectusBL().ImportExcel(dt, "Sponcer", _masterraceId);
                    var dtErrorResult = new ProspectusBL().Import30(dt, "Sponcer", _masterraceId);
                    if (dtErrorResult.Rows.Count > 0)
                    {
                        using (ExcelPackage xp = new ExcelPackage())
                        {
                            ExcelWorksheet ws = xp.Workbook.Worksheets.Add("Sponcer");
                            int rowstart = 1;
                            int colstart = 1;
                            int rowend = rowstart;
                            ...
                            Response.AddHeader("content-disposition", "attachment;filename=Prospectus_Master_SponcerOfTheRace.xlsx");
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                //using (DataSet ds = new ProspectusBL().GetProspectusNameWithCombination(_masterraceId, "Sponcer"))
                using (DataSet ds = new ProspectusBL().GetExport(_masterraceId, "Sponcer"))
                {
