...
namespace VKATalk.PopUps
{
    ...
    public partial class ProfessionalTrainerAssistant : System.Web.UI.Page
    {
        int _userId = 1;
                ...
                if (!IsPostBack)
                {
                    try
                    {
                        ...
                        if (_professionalId != 0)
                        {
                            ...
                            //BindDropDown(this.drpdwnProfile, "JockeyTrainerAssistance", "ProfessionalName", "ProfessionalNameID");
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                //DataSet ds = new ProfessionalBL().GetProfessionalTillDateValidation(_professionalId, "ProfessionalTrainerAssistant", this.btnSave.Text);
                DataSet ds = null;
                ...
                if (checkcondition == 1)
                {
					...
					//{
					//    ds = new ProfessionalBL().GetProfessionalTillDateValidation(_professionalId, "ProfessionalTrainerAssistant", txtbxFromDate.Text, btnSave.Text);
					//}
					//else
					//{
					//    ds = new ProfessionalBL().GetProfessionalTillDateValidation((int)ViewState["ProfessionalRecordID"], "ProfessionalTrainerAssistant", txtbxFromDate.Text, btnSave.Text);
					//}
                    ...
                    else
                    {
                        if (btnSave.Text.Equals("Add"))
                        {
                            status = new ProfessionalBL().ProfessionalTrainingAssistant(
                                _professionalId,
        ...
        {
            var dt = new ProfessionalBL().GetProfessionalNameAutoFiller("ProfessionalTrainerAssistance", prefixText);
        ...
        {
            var dt = new ProfessionalBL().GetProfessionalNameAutoFiller("ProfessionalTrainerAssistanceReason", prefixText);
            var commentList = new List<string>();
            for (var i = 0; i < dt.Rows.Count; i++)
            {
                commentList.Add(dt.Rows[i][0].ToString());
        ...
        {
                var ds = new ProfessionalBL().GetProfessionalNameWithCombination(_professionalId, "ProfessionalTrainerAssistant");
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    var status = new ProfessionalBL().ProfessionalTrainingAssistant((int)ViewState["ProfessionalRecordID"], 0, "__-__-____", "__-__-____", "", "", _userId, "Delete");
                    if (status > 0)
                    {
                        ClearAllSelection(this);
                        BindData();
        ...
        protected void btnFileUpload_Click(object sender, EventArgs e)
        {
            try
            {
                if (flupload.HasFile)
                {
                    ...
                    if (Path.GetFileNameWithoutExtension(flupload.PostedFile.FileName).Equals("Professional_TrainerMasterTrainer"))
                    {
        ...
        private void Import_To_Grid(string FilePath, string Extension)
        {
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    var dtErrorResult = new ProfessionalBL().Import30(dt, "ProfessionalTrainerAssistant");
                    if (dtErrorResult.Rows.Count > 0)
                    {
                        ...
                        {
                            ExcelWorksheet ws = xp.Workbook.Worksheets.Add("Professional_TrainerMasterTrainer");
                            int rowstart = 1;
                            int colstart = 1;
                            int rowend = rowstart;
                            ...
                            Response.AddHeader("content-disposition", "attachment;filename=Professional_TrainerMasterTrainer.xlsx");
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            try
            {
               // var ds = new ProfessionalBL().GetProfessionalNameWithCombination(_professionalId, "ProfessionalBaseCenter");
                using (DataSet ds = new ProfessionalBL().GetExport(_professionalId, "ProfessionalTrainerAssistant"))
                {
