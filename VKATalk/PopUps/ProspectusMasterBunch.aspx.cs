...
namespace VKATalk.PopUps
{
    ...
    public partial class ProspectusMasterBunch : System.Web.UI.Page
    {
            ...
            if (!IsPostBack)
            {
                try
                {
                    if (_masterraceId != 0)
                    {
                        ...
                        BindDropDown(drpdwnBunch, "ProspectusMasterBunch", "Bunch", "BunchMID");
                        drpdwnBunch.Items.Insert(0, new ListItem("-- Please select --", "-1"));
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (btnSave.Text.Equals("Add"))
                    {
                        ds = new ProspectusBL().GetProspectusTillDateValidation(_masterraceId, "MasterBunch", drpdwnFromYear.SelectedItem.Text, btnSave.Text);
                    }
                    else
                    {
                        ds = new ProspectusBL().GetProspectusTillDateValidation((int)ViewState["MasterRaceID"], "MasterBunch", drpdwnFromYear.SelectedItem.Text, btnSave.Text);
                    }
                ...
                else
                    {
                        if (btnSave.Text.Equals("Add"))
                        {
                            status = new ProspectusBL().MasterBunch(
                                _masterraceId,
                                drpdwnBunch.SelectedItem.Value,
                                Convert.ToInt32(drpdwnFromYear.SelectedItem.Value),
                                Convert.ToInt32(drpdwnTillYear.SelectedItem.Value),
                                CommonMethods.ConvertInSentenceCase(txtbxComment.Text),
                                1,
                                "Insert");
                        }
        ...
        {
            DataTable dt = new ProspectusBL().GetprospectusAutoFiller("ProspectusMasterMillion", prefixText);
        ...
        private void BindData()
        {
            try
            {
                ...
                DataSet ds = new ProspectusBL().GetProspectusNameWithCombination(_masterraceId, "MasterBunch");
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    var status = new ProspectusBL().MasterBunch((int)ViewState["MasterRaceID"], "",0,0,txtbxComment.Text, 1, "Delete");
        ...
        protected void GvHorseStatus_OnSelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                if (dataKey != null)
                {
                    ...
                    drpdwnBunch.Items.FindByText(hdnval.Value).Selected = true;
        ...
        protected void btnFileUpload_Click(object sender, EventArgs e)
        {
            try
            {
                if (flupload.HasFile)
                {
                    ...
                    if (Path.GetFileNameWithoutExtension(flupload.PostedFile.FileName).Equals("Prospectus_Master_Bunch"))
                    {
        ...
        private void Import_To_Grid(string FilePath, string Extension)
        {
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    var dtErrorResult = new ProspectusBL().Import30(dt, "MasterBunch", _masterraceId);
                    if (dtErrorResult.Rows.Count > 0)
                    {
                        using (ExcelPackage xp = new ExcelPackage())
                        {

                            ExcelWorksheet ws = xp.Workbook.Worksheets.Add("MasterBunch");
                            ...
                            Response.AddHeader("content-disposition", "attachment;filename=Prospectus_Master_Bunch.xlsx");
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                using (DataSet ds = new ProspectusBL().GetExport(_masterraceId, "MasterBunch"))
                {
