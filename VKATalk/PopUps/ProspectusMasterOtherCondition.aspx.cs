...
namespace VKATalk.PopUps
{
    ...
    public partial class ProspectusMasterOtherCondition : System.Web.UI.Page
    {
            ...
            if (!IsPostBack)
            {
                try
                {
                    if (_masterraceId != 0)
                    {
                        ...
                        BindDropDown(drpdwnPermanentCondition, "OtherCondition", "OtherCondition", "OtherConditionMID");
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
				...
				//{
				//    ds = new ProspectusBL().GetProspectusTillDateValidation(_masterraceId, "OtherCondition", drpdwnFromYear.SelectedItem.Text, btnSave.Text);
				//}
				//else
				//{
				//    ds = new ProspectusBL().GetProspectusTillDateValidation((int)ViewState["MasterRaceID"], "OtherCondition", drpdwnFromYear.SelectedItem.Text, btnSave.Text);
				//}
                ...
                else
                {
                    if (btnSave.Text.Equals("Add"))
                    {
                        status = new ProspectusBL().OtherCondition(
                            _masterraceId,
                            Convert.ToInt32(drpdwnPermanentCondition.SelectedItem.Value),
        ...
        {
            DataTable dt = new ProspectusBL().GetprospectusAutoFiller("MasterOtherCondition", prefixText);
        ...
        private void BindData()
        {
            try
            {
                ...
                DataSet ds = new ProspectusBL().GetProspectusNameWithCombination(_masterraceId, "MasterOtherCondition");
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    var status = new ProspectusBL().OtherCondition((int)ViewState["MasterRaceID"], 0, 0, 0, txtbxComment.Text, 1, "Delete");
                    ClearAllSelection(this);
                    BindData();
                    btnSave.Text = "Add";
        ...
        protected void btnFileUpload_Click(object sender, EventArgs e)
        {
            try
            {
                if (flupload.HasFile)
                {
                    ...
                    if (Path.GetFileNameWithoutExtension(flupload.PostedFile.FileName).Equals("Prospectus_Master_OtherCondition"))
                    {
        ...
        private void Import_To_Grid(string FilePath, string Extension)
        {
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    var dtErrorResult = new ProspectusBL().Import30(dt, "MasterOtherCondition", _masterraceId);
                    if (dtErrorResult.Rows.Count > 0)
                    {
                        ...
                        {
                            ExcelWorksheet ws = xp.Workbook.Worksheets.Add("MasterOtherCondition");
                            int rowstart = 1;
                            int colstart = 1;
                            int rowend = rowstart;
                            ...
                            Response.AddHeader("content-disposition", "attachment;filename=Prospectus_Master_OtherCondition.xlsx");
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            try
            {
                using (DataSet ds = new ProspectusBL().GetExport(_masterraceId, "MasterRaceOtherCondition"))
                {
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        using (ExcelPackage xp = new ExcelPackage())
                        {
                            ...
                            ExcelWorksheet ws = xp.Workbook.Worksheets.Add("OtherCondition");
