...
namespace VKATalk.PopUps
{
    ...
    public partial class HorseRunningStyle : System.Web.UI.Page
    {
        int _horseId = 0;
        int _userId = 1;
            ...
            try
            {
                ...
                if (!IsPostBack)
                {
                    if (_horseId != 0)
                    {
                        ...
                        BindDropDown(this.drpdwnGender, "HorseRunningStyle", "RunningStyle", "RunningStyleTypeID");
        ...
        public void BindData()
        {
                ...
                DataTable dt = Bl.GetHorseDetail(Convert.ToString(_horseId), "HorseRunningStyle");
                if (dt.Rows.Count > 0)
                {
        ...
        {
            DataTable dt = new MasterHorseBL().GetHorseNameAutoFiller("HorseRunningStyle", prefixText);
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
				...
				//{
				//    ds = new MasterHorseBL().GetHorseTillDateValidation(_horseId, "HorseRunningStyle", txtbxFromDate.Text, btnSave.Text);
				//}
				//else
				//{
				//    ds = new MasterHorseBL().GetHorseTillDateValidation((int)ViewState["horseStatusID"], "HorseRunningStyle", txtbxFromDate.Text, btnSave.Text);
				//}
                ...
                else
                {
                    if (btnSave.Text.Equals("Add"))
                    {
                        status = new MasterHorseBL().HorseRunningStyle(
                            _horseId,
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    new MasterHorseBL().HorseRunningStyle((int)ViewState["horseStatusID"],
                        0,txtbxFromDate.Text, txtbxTillDate.Text,
        ...
        protected void btnFileUpload_Click(object sender, EventArgs e)
        {
            try
            {
                if (flupload.HasFile)
                {
                    ...
                    if (Path.GetFileNameWithoutExtension(flupload.PostedFile.FileName).Equals("Horse_RunningStyle"))
                    {
        ...
        private void Import_To_Grid(string FilePath, string Extension)
        {
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    var dtErrorResult = new MasterHorseBL().Import30(dt, "HorseRunningStyle");
                    if (dtErrorResult.Rows.Count > 0)
                    {
                       // Page.ClientScript.RegisterStartupScript(this.GetType(), "Popup", "alert('Issue in few record. Please check the XL sheet');", true);
                        ...
                        {
                            ExcelWorksheet ws = xp.Workbook.Worksheets.Add("Horse RunningStyle");
                            int rowstart = 1;
                            int colstart = 1;
                            int rowend = rowstart;
                            ...
                            Response.AddHeader("content-disposition", "attachment;filename=Horse_RunningStyle.xlsx");
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            try
            {
                //using (DataSet ds = new MasterHorseBL().GetHorseNameWithCombination(_horseId, "HorseRunningStyle"))
                using (DataSet ds = new MasterHorseBL().GetExport(_horseId, "HorseRunningStyle"))
                {
                    DataTable dt = ds.Tables[0];
                    if (dt.Rows.Count > 0)
                    {
                       // dt.Columns.Remove("RunningStyleID");
                        using (ExcelPackage xp = new ExcelPackage())
                        {
                            ExcelWorksheet ws = xp.Workbook.Worksheets.Add("Horse_RunningStyle");
                            int rowstart = 1;
                            int colstart = 1;
                            int rowend = rowstart;
