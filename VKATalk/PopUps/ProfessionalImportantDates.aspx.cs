...
namespace VKATalk.PopUps
{
    ...
    public partial class ProfessionalImportantDates : System.Web.UI.Page
    {
        int _userId = 1;
                ...
                if (!IsPostBack)
                {
                    try
                    {
                        ...
                        if (_professionalId != 0)
                        {
							...
							BindDropDown(this.drpdwnOcassion, "MasterOccassion", "Ocassion", "OccasionMID");
							drpdwnOcassion.Items.Insert(0, new ListItem("-- Please select --", "-1"));

							BindDropDown(this.drpdwnRelatedTo, "ProfessionalImportantDateRelatedTo", "Relation", "RelationChildID");
							drpdwnRelatedTo.Items.Insert(0, new ListItem("-- Please select --", "-1"));
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                //{
                //    ds = new ProfessionalBL().GetProfessionalTillDateValidationProfile(_professionalId, "ProfessionalImportantDates", txtbxFromDate.Text, btnSave.Text, Convert.ToInt32(hdnfieldAntiPersonID.Value));
                //}
                //else
                //{
                //    ds = new ProfessionalBL().GetProfessionalTillDateValidationProfile((int)ViewState["ProfessionalRecordID"], "ProfessionalAntiPerson", txtbxFromDate.Text, btnSave.Text, Convert.ToInt32(hdnfieldAntiPersonID.Value));
                //}
                ...
                else
                {
                    if (btnSave.Text.Equals("Add"))
                    {
                        status = new ProfessionalBL().ProfessionalImportantDates(
                            _professionalId,
                            txtbxDates.Text,
                            drpdwnRelatedTo.SelectedItem.Value,
                            txtbxRelatedtoName.Text,
                            drpdwnOcassion.SelectedItem.Value,
        ...
        {
            DataTable dt = new ProfessionalBL().GetProfessionalNameAutoFiller("ImportantDates", prefixText);
        ...
        [System.Web.Script.Services.ScriptMethod()]
        [System.Web.Services.WebMethod]
        public static List<string> AddOcassionList(string prefixText, int count)
        {
            DataTable dt = new ProfessionalBL().GetProfessionalNameAutoFiller("AddOccasion", prefixText);
            List<string> commentList = new List<string>();
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                commentList.Add(dt.Rows[i][0].ToString());
        ...
        [System.Web.Script.Services.ScriptMethod()]
        [System.Web.Services.WebMethod]
        public static List<string> AddRelatedToList(string prefixText, int count)
        {
            DataTable dt = new ProfessionalBL().GetProfessionalNameAutoFiller("AddRelatedTo", prefixText);
            List<string> commentList = new List<string>();
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                commentList.Add(dt.Rows[i][0].ToString());
        ...
        {
                DataSet ds = new ProfessionalBL().GetProfessionalNameWithCombination(_professionalId, "ImportantDates");
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    var status = new ProfessionalBL().ProfessionalImportantDates((int)ViewState["horseRecordID"],"__-__-____",string.Empty,string.Empty,"", "", this._userId, "Delete");
                    if (status > 0)
                    {
                        ClearAllSelection(this);
                        BindData();
        ...
        protected void GvHomeDistance_OnSelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                if (dataKey != null)
                {
                    ...
                    txtbxDates.Text = hdnval.Value;
					drpdwnRelatedTo.Items.FindByText(row.Cells[2].Text).Selected = true;
					txtbxRelatedtoName.Text = row.Cells[3].Text;
					drpdwnOcassion.Items.FindByText(row.Cells[1].Text).Selected = true;
					//drpdwnOcassion.SelectedItem.Value = row.Cells[1].Text;
        ...
        protected void btnFileUpload_Click(object sender, EventArgs e)
        {
            try
            {
                if (flupload.HasFile)
                {
                    ...
                    if (Path.GetFileNameWithoutExtension(flupload.PostedFile.FileName).Equals("Professional_ImportantDates"))
                    {
        ...
        private void Import_To_Grid(string FilePath, string Extension)
        {
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    var dtErrorResult = new ProfessionalBL().Import30(dt, "ProfessionalImportantDates");
                    if (dtErrorResult.Rows.Count > 0)
                    {
                        ...
                        {
                            ExcelWorksheet ws = xp.Workbook.Worksheets.Add("Professional_ImportantDates");
                            int rowstart = 1;
                            int colstart = 1;
                            int rowend = rowstart;
                            ...
                            Response.AddHeader("content-disposition", "attachment;filename=Professional_ImportantDates.xlsx");
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                using (DataSet ds = new ProfessionalBL().GetExport(_professionalId, "ImportantDates"))
                {
