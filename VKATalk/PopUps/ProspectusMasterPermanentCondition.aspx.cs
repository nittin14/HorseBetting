...
namespace VKATalk.PopUps
{
    ...
    public partial class ProspectusMasterPermanentCondition : System.Web.UI.Page
    {
            ...
            if (!IsPostBack)
            {
                try
                {
                    if (_masterraceId != 0)
                    {
                        ...
                        BindDropDown(drpdwnPermanentCondition, "MasterCondition", "PermanentCondition", "PermanentConditionID");
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
				...
				//{
				//    ds = new ProspectusBL().GetProspectusTillDateValidation(_masterraceId, "PermanentCondition", drpdwnFromYear.SelectedItem.Text, btnSave.Text);
				//}
				//else
				//{
				//    ds = new ProspectusBL().GetProspectusTillDateValidation((int)ViewState["MasterRaceID"], "PermanentCondition", drpdwnFromYear.SelectedItem.Text, btnSave.Text);
				//}
                ...
                else
                {
                    if (btnSave.Text.Equals("Add"))
                    {
                        status = new ProspectusBL().PermanentCondition(
                            _masterraceId,
        ...
        {
            DataTable dt = new ProspectusBL().GetprospectusAutoFiller("ProspectusMasterPermanentCondition", prefixText);
        ...
        private void BindData()
        {
            try
            {
                ...
                DataSet ds = new ProspectusBL().GetProspectusNameWithCombination(_masterraceId, "ProspectusMasterPermanentCondition");
        ...
        protected void btnFileUpload_Click(object sender, EventArgs e)
        {
            try
            {
                if (flupload.HasFile)
                {
                    ...
                    if (Path.GetFileNameWithoutExtension(flupload.PostedFile.FileName).Equals("Prospectus_Master_PermanentCondition"))
                    {
        ...
        private void Import_To_Grid(string FilePath, string Extension)
        {
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    var dtErrorResult = new ProspectusBL().Import30(dt, "PermanentCondition", _masterraceId);
                    if (dtErrorResult.Rows.Count > 0)
                    {
                        ...
                        {
                            ExcelWorksheet ws = xp.Workbook.Worksheets.Add("PermanentCondition");
                            int rowstart = 1;
                            int colstart = 1;
                            int rowend = rowstart;
                            ...
                            Response.AddHeader("content-disposition", "attachment;filename=Prospectus_Master_PermanentCondition.xlsx");
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            try
            {
                using (DataSet ds = new ProspectusBL().GetExport(_masterraceId, "PermanentCondition"))
                {
