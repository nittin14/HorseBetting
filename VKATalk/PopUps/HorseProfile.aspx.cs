...
namespace VKATalk.PopUps
{
    ...
    public partial class HorseProfile : System.Web.UI.Page
    {
        int _userId = 1;
        int _horseId = 0;
		...
		protected void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    ds = new MasterHorseBL().GetHorseTillDateValidation(_horseId, "HorseProfile", txtbxFromDate.Text, btnSave.Text);
                }
                else
                {
                    ds = new MasterHorseBL().GetHorseTillDateValidation((int)ViewState["horseRecordID"], "HorseProfile", txtbxFromDate.Text, btnSave.Text);
                }
                ...
                {
                    //Page.ClientScript.RegisterStartupScript(this.GetType(), "Popup", "ShowPopup('Please Check the existing record TillDate.');", true);
                ...
                else
                {
                    if (btnSave.Text.Equals("Add"))
                    {
                        status = new MasterHorseBL().HorseProfile(
                            _horseId,
                            0,
        ...
        {
            DataTable dt = new MasterHorseBL().GetHorseNameAutoFiller("HorseProfileComments", prefixText);
        ...
        {
               DataSet ds = new MasterHorseBL().GetHorseNameWithCombination(_horseId, "HorseProfile");
        ...
        private void Import_To_Grid(string FilePath, string Extension)
        {
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    var dtErrorResult = new MasterHorseBL().Import30(dt, "HorseProfile");
                    if (dtErrorResult.Rows.Count > 0)
                    {
                        ...
                        {
                            ExcelWorksheet ws = xp.Workbook.Worksheets.Add("Horse Profile");
                            int rowstart = 1;
                            int colstart = 1;
                            int rowend = rowstart;
                            ...
                            Response.AddHeader("content-disposition", "attachment;filename=Horse_Profile.xlsx");
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            try
            {
                //using (DataSet ds = new MasterHorseBL().GetHorseNameWithCombination(_horseId, "HorseProfile"))
                using (DataSet ds = new MasterHorseBL().GetExport(_horseId, "HorseProfile"))
                {
                    DataTable dt = ds.Tables[0];
                    if (dt.Rows.Count > 0)
                    {
                       // dt.Columns.Remove("HorseProfileID");
                        using (ExcelPackage xp = new ExcelPackage())
                        {
                            ExcelWorksheet ws = xp.Workbook.Worksheets.Add("HorseProfile");
                            int rowstart = 1;
                            int colstart = 1;
                            int rowend = rowstart;
