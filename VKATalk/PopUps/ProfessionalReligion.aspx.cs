...
namespace VKATalk.PopUps
{
    ...
    public partial class ProfessionalReligion : System.Web.UI.Page
    {
        int _userId = 1;
                ...
                if (!IsPostBack)
                {
                    try
                    {
                        ...
                        if (_professionalId != 0)
                        {
                            ...
                            BindDropDown(this.drpdwnReligion, "Religion", "Religion", "ReligionID");
                            drpdwnReligion.Items.Insert(0, new ListItem("-- Please select --", "-1"));
                            BindData();
                        }
                    }
                    catch (Exception ex)
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    ds = new ProfessionalBL().GetProfessionalTillDateValidation(_professionalId, "Religion", txtbxFromDate.Text, btnSave.Text);
                }
                else
                {
                    ds = new ProfessionalBL().GetProfessionalTillDateValidation((int)ViewState["ProfessionalRecordID"], "Religion", txtbxFromDate.Text, btnSave.Text);
                }
                ...
                else
                {
                    if (btnSave.Text.Equals("Add"))
                    {
                        status = new ProfessionalBL().ProfessionalReligion(
                            _professionalId,
                            Convert.ToInt32(drpdwnReligion.SelectedItem.Value),
                            txtbxFromDate.Text,
                            txtbxTillDate.Text,
                            txtbxComment.Text,
                            _userId,
                    ...
                    if (status == 1)
                    {
                        if (btnSave.Text.Equals("Add"))
                        {
                            hdnfieldProfessionalName.Value = drpdwnReligion.SelectedItem.Text;
                        }
        ...
        {
            var ds = new ProfessionalBL().GetProfessionalNameWithCombination(_professionalId, "Religion");
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    var status = new ProfessionalBL().ProfessionalReligion((int)ViewState["ProfessionalRecordID"], 0, "__-__-____", "__-__-____","", _userId, "Delete");
                    if (status > 0)
                    {
                        ClearAllSelection(this);
                        BindData();
        ...
        protected void GvHorseProfile_OnSelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                if (dataKey != null)
                {
                    ...
                    drpdwnReligion.Items.FindByText(hdnval.Value).Selected = true;
        ...
        protected void btnFileUpload_Click(object sender, EventArgs e)
        {
            try
            {
                if (flupload.HasFile)
                {
                    ...
                    if (Path.GetFileNameWithoutExtension(flupload.PostedFile.FileName).Equals("Professional_Religion"))
                    {
        ...
        private void Import_To_Grid(string FilePath, string Extension)
        {
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    var dtErrorResult = new ProfessionalBL().Import30(dt, "ProfessionalReligion");
                    if (dtErrorResult.Rows.Count > 0)
                    {
                        ...
                        {
                            ExcelWorksheet ws = xp.Workbook.Worksheets.Add("Professional_Religion");
                            int rowstart = 1;
                            int colstart = 1;
                            int rowend = rowstart;
                            ...
                            Response.AddHeader("content-disposition", "attachment;filename=Professional_Religion.xlsx");
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            try
            {
              //  using (DataSet ds = new ProfessionalBL().GetProfessionalNameWithCombination(_professionalId, "ProfessionalBaseCenter"))
                using (DataSet ds = new ProfessionalBL().GetExport(_professionalId, "ProfessionalReligion"))
                {
