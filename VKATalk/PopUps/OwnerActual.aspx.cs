...
namespace VKATalk.PopUps
{
    ...
    public partial class OwnerActual : System.Web.UI.Page
    {
        int _horseId = 0;
        int _userId = 1;
        ...
        public void BindData()
        {
                ...
                DataTable dt = Bl.GetHorseDetail(Convert.ToString(_horseId), "OwnerActual");
                if (dt.Rows.Count > 0)
                {
        ...
        {
            DataTable dt = new MasterHorseBL().GetHorseNameAutoFiller("HorseOwnerActualComment", prefixText);
        ...
        {
            DataTable dt = new MasterHorseBL().GetHorseNameAutoFiller("HorseOwnerActualReasonofChange", prefixText);
            List<string> currentMissionComments = new List<string>();
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                currentMissionComments.Add(dt.Rows[i][0].ToString());
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    {
                        ds = new MasterHorseBL().GetHorseTillDateValidation(_horseId, "HorseOwnerActual", txtbxFromDate.Text, btnSave.Text);
                    }
                    else
                    {
                        ds = new MasterHorseBL().GetHorseTillDateValidation((int)ViewState["horseStatusID"], "HorseOwnerActual", txtbxFromDate.Text, btnSave.Text);
                    }
                    ...
                    else
                    {
						...
						if (btnSave.Text.Equals("Add"))
                        {
                            status = new MasterHorseBL().HorseOwnerActual(
                                _horseId,
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    new MasterHorseBL().HorseOwnerActual((int)ViewState["horseStatusID"], 0, txtbxFromDate.Text, txtbxTillDate.Text,
        ...
        protected void btnFileUpload_Click(object sender, EventArgs e)
        {
            try
            {
                if (flupload.HasFile)
                {
                    ...
                    if (Path.GetFileNameWithoutExtension(flupload.PostedFile.FileName).Equals("Horse_OwnerHidden"))
                    {
        ...
        private void Import_To_Grid(string FilePath, string Extension)
        {
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    var dtErrorResult = new MasterHorseBL().Import30(dt, "HorseOwnerActual");
                    if (dtErrorResult.Rows.Count > 0)
                    {
                        ...
                        {
                            ExcelWorksheet ws = xp.Workbook.Worksheets.Add("Horse_OwnerHidden");
                            int rowstart = 1;
                            int colstart = 1;
                            int rowend = rowstart;
                            ...
                            Response.AddHeader("content-disposition", "attachment;filename=Horse_OwnerHidden.xlsx");
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            try
            {
                //using (DataSet ds = new MasterHorseBL().GetHorseNameWithCombination(_horseId, "HorseOwnerActual"))
                using (DataSet ds = new MasterHorseBL().GetExport(_horseId, "HorseOwnerActual"))
                {
                    DataTable dt = ds.Tables[0];
                    if (dt.Rows.Count > 0)
                    {
                    //    dt.Columns.Remove("OwnerActualID");
                        using (ExcelPackage xp = new ExcelPackage())
                        {
                            ExcelWorksheet ws = xp.Workbook.Worksheets.Add("Horse_OwnerHidden");
                            int rowstart = 1;
