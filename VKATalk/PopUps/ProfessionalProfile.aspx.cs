    ...
    public partial class ProfessionalProfile : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                if (!IsPostBack)
                {
                        ...
                        //string[] professionalName = reqQueryValue.Split(',');
                        //    lblProfessionalNameFirst.Text = professionalName[1];
                        //    lblProfessionalNameSecond.Text = professionalName[2];
                            BindDropDown(drpdwnProfile, "ProfessionalProfile", "ProfileType", "ProfileTypeID");
                            drpdwnProfile.Items.FindByText("Owner").Selected = true;
                            BindData();
                        }
                }
            }
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    ds = new ProfessionalBL().GetProfessionalTillDateValidationProfile(_professionalId, "ProfessionalProfile", txtbxFromDate.Text, btnSave.Text, Convert.ToInt32(drpdwnProfile.SelectedItem.Value));
                }
                else
                {
                    ds = new ProfessionalBL().GetProfessionalTillDateValidationProfile((int)ViewState["ProfessionalRecordID"], "ProfessionalProfile", txtbxFromDate.Text, btnSave.Text, Convert.ToInt32(drpdwnProfile.SelectedItem.Value));
                }
                ...
                else
                {
                    if (btnSave.Text.Equals("Add"))
                    {
                        status = new ProfessionalBL().ProfessionalProfileP(
                            _professionalId,
                            Convert.ToInt32(drpdwnProfile.SelectedItem.Value),
                            txtbxFromDate.Text,
                            txtbxTillDate.Text,
                            txtbxComment.Text,
                            _userId,
                            "Insert",
                            txtbxProfileInDetail.Text);
                    }
                    else
                    {
                        status = new ProfessionalBL().ProfessionalProfileP(
        ...
        {
            DataTable dt = new ProfessionalBL().GetProfessionalNameAutoFiller("ProfessionalProfileComments", prefixText);
        ...
        [System.Web.Script.Services.ScriptMethod()]
        [System.Web.Services.WebMethod]
        public static List<string> AddProfileInDetails(string prefixText, int count)
        {
            DataTable dt = new ProfessionalBL().GetProfessionalNameAutoFiller("ProfileInDetail", prefixText);
            List<string> commentList = new List<string>();
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                commentList.Add(dt.Rows[i][0].ToString());
        ...
        {
                DataSet ds = new ProfessionalBL().GetProfessionalNameWithCombination(_professionalId, "ProfessionalProfile");
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    var status = new ProfessionalBL().ProfessionalProfileP((int)ViewState["ProfessionalRecordID"], 0, "__-__-____", "__-__-____", "", _userId, "Delete", "");
                    if (status > 0)
                    {
                        ClearAllSelection(this);
                        BindData();
        ...
        protected void GvHorseProfile_OnSelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                if (dataKey != null)
                {
                    ...
                    {
                        txtbxProfileInDetail.Text = row.Cells[1].Text.Equals("&nbsp;") ? "" : row.Cells[1].Text.Contains("&quot;") ? row.Cells[1].Text.Replace("&quot;", "\"") : row.Cells[1].Text;
                    }
                    else
                    {
                        txtbxProfileInDetail.Text = row.Cells[1].Text.Equals("&nbsp;") ? "" : row.Cells[1].Text.Contains("&#39;") ? row.Cells[1].Text.Replace("&#39;", "'") : row.Cells[1].Text;
        ...
        protected void btnFileUpload_Click(object sender, EventArgs e)
        {
            try
            {
                if (flupload.HasFile)
                {
                    ...
                    if (Path.GetFileNameWithoutExtension(flupload.PostedFile.FileName).Equals("Professional_Profile"))
                    {
        ...
        private void Import_To_Grid(string FilePath, string Extension)
        {
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    var dtErrorResult = new ProfessionalBL().Import30(dt, "ProfessionalProfile");
                    if (dtErrorResult.Rows.Count > 0)
                    {
                        ...
                        {
                            ExcelWorksheet ws = xp.Workbook.Worksheets.Add("Professional_Profile");
                            int rowstart = 1;
                            int colstart = 1;
                            int rowend = rowstart;
                            ...
                            Response.AddHeader("content-disposition", "attachment;filename=Professional_Profile.xlsx");
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            try
            {
                using (DataSet ds = new ProfessionalBL().GetExport(_professionalId, "ProfessionalProfile"))
                {
                    DataTable dt = ds.Tables[0];
                    if (dt.Rows.Count > 0)
                    {
                        //dt.Columns.Remove("ProfessionalProfileID");
                        using (ExcelPackage xp = new ExcelPackage())
                        {
                            ExcelWorksheet ws = xp.Workbook.Worksheets.Add("Professional_Profile");
                            int rowstart = 1;
