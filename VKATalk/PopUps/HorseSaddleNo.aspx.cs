...
namespace VKATalk.PopUps
{
    ...
    public partial class HorseSaddleNo : System.Web.UI.Page
    {
        int _horseId = 0;
        int _userId = 1;
            ...
            try
            {
                ...
                if (!IsPostBack)
                {
                    if (_horseId != 0)
                    {
                        ...
                        BindDropDown(this.drpdwnCommon, "MasterCenter", "CenterName", "CenterID");
                        BindDropDown(this.drpdwnCommon1, "MasterYear", "YearName", "YearID");
        ...
        public void BindData()
        {
                ...
                DataTable dt = Bl.GetHorseDetail(Convert.ToString(_horseId), "HorseSaddleNo");
                if (dt.Rows.Count > 0)
                {
        ...
        {
            DataTable dt = new MasterHorseBL().GetHorseNameAutoFiller("HorseSaddleNo", prefixText);
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
				...
				//{
				//    ds = new MasterHorseBL().GetHorseTillDateValidation(_horseId, "HorseSaddleNo", txtbxFromDate.Text, btnSave.Text);
				//}
				//else
				//{
				//    ds = new MasterHorseBL().GetHorseTillDateValidation((int)ViewState["horseStatusID"], "HorseSaddleNo", txtbxFromDate.Text, btnSave.Text);
				//}
                ...
                else
                {
                    if (btnSave.Text.Equals("Add"))
                    {
                        status = new MasterHorseBL().HorseSaddleNo(
                            _horseId,
                            Convert.ToInt32(this.drpdwnCommon.SelectedItem.Value),
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    new MasterHorseBL().HorseSaddleNo((int)ViewState["horseStatusID"],
                        Convert.ToInt32(this.drpdwnCommon.SelectedItem.Value),Convert.ToInt32(this.drpdwnCommon1.SelectedItem.Value), 
        ...
        protected void GvGlobal_OnSelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                if (dataKey != null)
                {
                    ...
                    this.drpdwnCommon1.Items.FindByText(row.Cells[1].Text).Selected = true;
                    txtbxSaddleNo.Text = row.Cells[2].Text;
        ...
        protected void btnFileUpload_Click(object sender, EventArgs e)
        {
            try
            {
                if (flupload.HasFile)
                {
                    ...
                    if (Path.GetFileNameWithoutExtension(flupload.PostedFile.FileName).Equals("Horse_SaddleNo"))
                    {
        ...
        private void Import_To_Grid(string FilePath, string Extension)
        {
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    var dtErrorResult = new MasterHorseBL().Import30(dt, "HorseSaddleNo");
                    if (dtErrorResult.Rows.Count > 0)
                    {
                        using (ExcelPackage xp = new ExcelPackage())
                        {
                            ExcelWorksheet ws = xp.Workbook.Worksheets.Add("Horse_SaddleNo");
                            int rowstart = 1;
                            int colstart = 1;
                            int rowend = rowstart;
                            ...
                            Response.AddHeader("content-disposition", "attachment;filename=Horse_SaddleNo.xlsx");
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            try
            {
                using (DataSet ds = new MasterHorseBL().GetExport(_horseId, "HorseSaddleNo"))
                {
