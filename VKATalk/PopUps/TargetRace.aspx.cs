...
namespace VKATalk.PopUps
{
    ...
    public partial class TargetRace : System.Web.UI.Page
    {
        int _horseId = 0;
        int _userId = 1;
            ...
            try
            {
                ...
                if (!IsPostBack)
                {
                    if (_horseId != 0)
                    {
                       ...
                       // BindDropDown(this.drpdwnGender, "MasterCenter", "CenterName", "CenterID");
                        //BindDropDown(this.drpdwnGeneralName, "ProspectusGeneralRaceName", "GeneralRaceName", "GeneralRaceNameID");
        ...
        {
            DataTable dt = new ProspectusBL().GetprospectusAutoFiller("ProspectusGeneralName", prefixText);
        ...
        public void BindData()
        {
                ...
                DataTable dt = Bl.GetHorseDetail(Convert.ToString(_horseId), "TargetRace");
                if (dt.Rows.Count > 0)
                {
        ...
        {
            DataTable dt = new MasterHorseBL().GetHorseNameAutoFiller("TargetRace", prefixText);
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                //{
                //    ds = new MasterHorseBL().GetHorseTillDateValidation(_horseId, "TargetRace", txtbxFromDate.Text, btnSave.Text);
                //}
                //else
                //{
                //    ds = new MasterHorseBL().GetHorseTillDateValidation((int)ViewState["horseStatusID"], "TargetRace", txtbxFromDate.Text, btnSave.Text);
                //}

                ////if (btnSave.Text.Equals("Add"))
                ////{
                ////    ds = new MasterHorseBL().GetHorseTillDateValidationMultiple(_horseId, "DistancePerformanceGroup", txtbxFromDate.Text, btnSave.Text, drpdwnHomeDistance.SelectedItem.Value, drpdwnSupportType.SelectedItem.Value);
                ////}
                ////else
                ////{
                ////    ds = new MasterHorseBL().GetHorseTillDateValidationMultiple((int)ViewState["horseRecordID"], "DistancePerformanceGroup", txtbxFromDate.Text, btnSave.Text, drpdwnHomeDistance.SelectedItem.Value, drpdwnSupportType.SelectedItem.Value);
                ////}
                ...
                if (btnSave.Text.Equals("Add"))
                {
                    status = new MasterHorseBL().HorseTargetRace(
                        _horseId,
                        0,
                        "",
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    new MasterHorseBL().HorseTargetRace((int)ViewState["horseStatusID"], 0,"",0, txtbxFromDate.Text, txtbxTillDate.Text,txtbxComment.Text, 1, "Delete");
                    ClearAllSelection(this);
                    BindData();
                    btnSave.Text = "Add";
        ...
        protected void GvGlobal_OnSelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                if (dataKey != null)
                {
                    ...
                    hdnfieldMasterProsectusId.Value = hdnfieldGeneralRaceID.Value;
                    txtbxProspectusMasterName.Text = hdnval.Value;
                    txtbxFromDate.Text = row.Cells[4].Text;
                    if (!row.Cells[5].Text.Contains("&nbsp;"))
                    {
                        txtbxTillDate.Text = row.Cells[5].Text;
                    }
        ...
        protected void btnFileUpload_Click(object sender, EventArgs e)
        {
            try
            {
                if (flupload.HasFile)
                {
                    ...
                    if (Path.GetFileNameWithoutExtension(flupload.PostedFile.FileName).Equals("Horse_TargetRace"))
                    {
        ...
        private void Import_To_Grid(string FilePath, string Extension)
        {
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    var dtErrorResult = new MasterHorseBL().Import30(dt, "HorseTargetRace");
                    if (dtErrorResult.Rows.Count > 0)
                    {
                        ...
                        {
                            ExcelWorksheet ws = xp.Workbook.Worksheets.Add("Horse_TargetRace");
                            int rowstart = 1;
                            int colstart = 1;
                            int rowend = rowstart;
                            ...
                            Response.AddHeader("content-disposition", "attachment;filename=Horse_TargetRace.xlsx");
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            try
            {
                //using (DataSet ds = new MasterHorseBL().GetHorseNameWithCombination(_horseId, "HorseTargetRace"))
                using (DataSet ds = new MasterHorseBL().GetExport(_horseId, "HorseTargetRace"))
                {
                    DataTable dt = ds.Tables[0];
                    if (dt.Rows.Count > 0)
                    {
                        dt.Columns.Remove("GeneralRaceID_FK");
                        using (ExcelPackage xp = new ExcelPackage())
                        {
                            ExcelWorksheet ws = xp.Workbook.Worksheets.Add("Horse_TargetRace");
                            int rowstart = 1;
