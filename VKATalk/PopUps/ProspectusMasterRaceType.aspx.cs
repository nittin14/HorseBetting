...
namespace VKATalk.PopUps
{
    ...
    public partial class ProspectusMasterRaceType : System.Web.UI.Page
    {
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            ...
            {
                int status = 0;
                //DataSet ds = new ProspectusBL().GetProspectusTillDateValidation(_masterraceId, "MasterRaceType", this.btnSave.Text);
                DataSet ds = null;
                if (btnSave.Text.Equals("Add"))
                {
                    ds = new ProspectusBL().GetProspectusTillDateValidation(_masterraceId, "MasterRaceType", drpdwnFromYear.SelectedItem.Text, btnSave.Text);
                }
                else
                {
                    ds = new ProspectusBL().GetProspectusTillDateValidation((int)ViewState["MasterRaceID"], "MasterRaceType", drpdwnFromYear.SelectedItem.Text, btnSave.Text);
                }
                ...
                else
                {
                    if (btnSave.Text.Equals("Add"))
                    {
                        status = new ProspectusBL().MasterRaceType(
                            _masterraceId,
                            rdbtnRaceType.SelectedItem.Text,
                            Convert.ToInt32(drpdwnFromYear.SelectedItem.Value),
                            Convert.ToInt32(drpdwnTillYear.SelectedItem.Value),
                            txtbxComment.Text,
                            1,
                            "Insert",
                            drpdwnCategory.SelectedItem.Text,
                            txtbxTillDate.Text);
                    }
                    else if (btnSave.Text.Equals("Update"))
                    {
                        status = new ProspectusBL().MasterRaceType(
                            ...
                            1,
                            "Update",
                            drpdwnCategory.SelectedItem.Text, txtbxTillDate.Text);
                    //    btnSave.Text = "Add";
                    }
                    if (status == 1)
                    {
        ...
        {
            DataTable dt = new ProspectusBL().GetprospectusAutoFiller("ProspectusMasterRaceType", prefixText);
        ...
        private void BindData()
        {
            try
            {
                ...
                DataSet ds = new ProspectusBL().GetProspectusNameWithCombination(_masterraceId, "MasterRaceType");
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    var status = new ProspectusBL().MasterRaceType((int)ViewState["MasterRaceID"], "",0,0,txtbxComment.Text, 1, "Delete","","__-__-____");
                    ClearAllSelection(this);
                    BindData();
                    btnSave.Text = "Add";
        ...
        protected void GvHorseStatus_OnSelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                if (dataKey != null)
                {
                    ...
                    {
                        if (row.Cells[1].Text.Equals("Hyderabad"))
                                drpdwnCategory.Items.FindByText("--Please select--").Selected = true;
                    }
                    else
                    {
                        drpdwnCategory.Items.FindByText(row.Cells[1].Text).Selected = true;
                    }
        ...
        protected void btnFileUpload_Click(object sender, EventArgs e)
        {
            try
            {
                if (flupload.HasFile)
                {
                    ...
                    if (Path.GetFileNameWithoutExtension(flupload.PostedFile.FileName).Equals("Prospectus_Master_RaceType"))
                    {
        ...
        private void Import_To_Grid(string FilePath, string Extension)
        {
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    var dtErrorResult = new ProspectusBL().Import30(dt, "ProspectMasterRaceType", _masterraceId);
                    if (dtErrorResult.Rows.Count > 0)
                    {
                        using (ExcelPackage xp = new ExcelPackage())
                        {
                            ExcelWorksheet ws = xp.Workbook.Worksheets.Add("ProspectMasterRaceType");
                            int rowstart = 1;
                            int colstart = 1;
                            int rowend = rowstart;
                            ...
                            Response.AddHeader("content-disposition", "attachment;filename=Prospectus_Master_RaceType.xlsx");
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                //using (DataSet ds = new ProspectusBL().GetProspectusNameWithCombination(_masterraceId, "MasterRaceType"))
                using (DataSet ds = new ProspectusBL().GetExport(_masterraceId, "MasterRaceType"))
                {
                    DataTable dt = ds.Tables[0];
                    if (dt.Rows.Count > 0)
                    {
                       // dt.Columns.Remove("MasterRaceTypeID");
                        using (ExcelPackage xp = new ExcelPackage())
                        {
                            ExcelWorksheet ws = xp.Workbook.Worksheets.Add("ProspectMasterRaceType");
                            int rowstart = 1;
