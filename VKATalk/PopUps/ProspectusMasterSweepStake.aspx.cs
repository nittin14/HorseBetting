...
namespace VKATalk.PopUps
{
    ...
    public partial class ProspectusMasterSweepStake : System.Web.UI.Page
    {
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            ...
            {
                int status = 0;
                //DataSet ds = new ProspectusBL().GetProspectusTillDateValidation(_masterraceId, "MasterSweepStake", this.btnSave.Text);
                DataSet ds = null;
                if (!chkbxSweepStake.Checked)
                {
                    var message = "Please select Sweep Stake.";
                    Page.ClientScript.RegisterStartupScript(this.GetType(), "Popup", "ShowPopup('" + message + "');", true);
                }
                else
                {
                    ...
                    {
                        ds = new ProspectusBL().GetProspectusTillDateValidation(_masterraceId, "MasterSweepStake", drpdwnFromYear.SelectedItem.Text, btnSave.Text);
                    }
                    else
                    {
                        ds = new ProspectusBL().GetProspectusTillDateValidation((int)ViewState["MasterRaceID"], "MasterSweepStake", drpdwnFromYear.SelectedItem.Text, btnSave.Text);
                    }
                    ...
                    else
                    {
                        if (btnSave.Text.Equals("Add"))
                        {
                            status = new ProspectusBL().MasterSweepStake(
                                _masterraceId,
                                "Yes",
        ...
        private void BindData()
        {
            try
            {
                ...
                DataSet ds = new ProspectusBL().GetProspectusNameWithCombination(_masterraceId, "ProspectusMasterSweepStake");
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                if (!ViewState["MasterRaceID"].Equals(""))
                {
                    ...
                    var status = new ProspectusBL().MasterSweepStake((int)ViewState["MasterRaceID"], "", 0,0, txtbxComment.Text, 1, "Delete");
            ...
            try
            {
                ...
                if (dataKey != null)
                {
                   ...
                   // rdbtnMillion.Items.FindByText(hdnval.Value).Selected = true;
                    if (hdnval.Value.Equals("Sweep Stake"))
                    {
        ...
        protected void btnFileUpload_Click(object sender, EventArgs e)
        {
            try
            {
                if (flupload.HasFile)
                {
                    ...
                    if (Path.GetFileNameWithoutExtension(flupload.PostedFile.FileName).Equals("Prospectus_Master_SweepStake"))
                    {
        ...
        private void Import_To_Grid(string FilePath, string Extension)
        {
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    var dtErrorResult = new ProspectusBL().Import30(dt, "MasterSweepStake", _masterraceId);
                    if (dtErrorResult.Rows.Count > 0)
                    {
                       // Page.ClientScript.RegisterStartupScript(this.GetType(), "Popup", "alert('Issue in few record. Please check the XL sheet');", true);
                        ...
                        {
                            ExcelWorksheet ws = xp.Workbook.Worksheets.Add("MasterSweepStake");
                            int rowstart = 1;
                            int colstart = 1;
                            int rowend = rowstart;
                            ...
                            Response.AddHeader("content-disposition", "attachment;filename=Prospectus_Master_SweepStake.xlsx");
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                using (DataSet ds = new ProspectusBL().GetExport(_masterraceId, "ProspectusMasterSweepStake"))
                {
