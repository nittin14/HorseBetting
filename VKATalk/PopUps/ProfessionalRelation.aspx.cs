...
namespace VKATalk.PopUps
{
    ...
    public partial class ProfessionalRelation : System.Web.UI.Page
    {
        int _userId = 1;
                ...
                if (!IsPostBack)
                {
                    try
                    {
                        ...
                        if (_professionalId != 0)
                        {
                            ...
                            BindData();
                            BindDropDown(drpdwnChildRelation, "ProfessionalChildRelation", "Relation", "RelationChildID");
                            BindDropDown(this.drpdwnRelationType, "RelationType", "RelationType", "RelationTypeID");
                            drpdwnChildRelation.Items.Insert(0, new ListItem("-- Please select --", "-1"));
                            drpdwnRelationType.Items.Insert(0, new ListItem("-- Please select --", "-1"));
                            //drpdwnRelationType.Items.FindByText("Business Partner").Selected = true;
                        }
                    }
                    catch (Exception ex)
                    {
        ...
        {
            DataTable dt = new ProfessionalBL().GetProfessionalNameAutoFiller("ProfessionalTrainee", prefixText);
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            ...
            {
                int status = 0;
                //DataSet ds = new ProfessionalBL().GetProfessionalTillDateValidation(_professionalId, "Relation", this.btnSave.Text);
                DataSet ds = null;
				//if (btnSave.Text.Equals("Add"))
				//{
				//    ds = new ProfessionalBL().GetProfessionalTillDateValidation(_professionalId, "Relation", txtbxFromDate.Text, btnSave.Text);
				//}
				//else
				//{
				//    ds = new ProfessionalBL().GetProfessionalTillDateValidation((int)ViewState["ProfessionalRecordID"], "Relation", txtbxFromDate.Text, btnSave.Text);
				//}
				//if (btnSave.Text.Equals("Add"))
				//{
				//    ds = new ProfessionalBL().GetProfessionalTillDateValidationProfile(_professionalId, "ProfessionalRelation", txtbxFromDate.Text, btnSave.Text, Convert.ToInt32(hdnfieldProfessionalnameid.Value));
				//}
				//else
				//{
				//    ds = new ProfessionalBL().GetProfessionalTillDateValidationProfile((int)ViewState["ProfessionalRecordID"], "ProfessionalRelation", txtbxFromDate.Text, btnSave.Text, Convert.ToInt32(hdnfieldProfessionalnameid.Value));
				//}
                ...
                else
                {
					...
					if (ViewState["GroupID"] == null)
						...
						groupid = ViewState["GroupID"].ToString();
					if (btnSave.Text.Equals("Add"))
                    {
                        status = new ProfessionalBL().ProfessionalRelation(
                            _professionalId,
                            Convert.ToInt32(this.hdnfieldProfessionalnameid.Value),
                            Convert.ToInt32(this.drpdwnRelationType.SelectedItem.Value),
                            Convert.ToInt32(this.drpdwnChildRelation.SelectedItem.Value),
                            this.txtbxFromDate.Text,
                            this.txtbxTillDate.Text,
                            txtbxRelationBreak.Text,
        ...
        {
            DataTable dt = new ProfessionalBL().GetProfessionalNameAutoFiller("RelationComment", prefixText);
        ...
        [System.Web.Script.Services.ScriptMethod()]
        [System.Web.Services.WebMethod]
        public static List<string> AddRelationList(string prefixText, int count)
        {
            DataTable dt = new ProfessionalBL().GetProfessionalNameAutoFiller("RelationBreak", prefixText);
            List<string> commentList = new List<string>();
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                commentList.Add(dt.Rows[i][0].ToString());
        ...
        public void ClearAllSelection(Control parent)
        {
            ...
            drpdwnRelationType.ClearSelection();
            //drpdwnRelationType.Items.FindByText("Business Partner").Selected = true;
        }
         /// <summary>
        ...
        {
                DataSet ds = new ProfessionalBL().GetProfessionalNameWithCombination(_professionalId, "Relation");
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    var status = new ProfessionalBL().ProfessionalRelation((int)ViewState["ProfessionalRecordID"], 0, 0, 0, "__-__-____", "__-__-____", "", "", this._userId, "Delete", groupid);
                    if (status > 0)
                    {
                        ClearAllSelection(this);
                        BindData();
        ...
        protected void GvHomeDistance_OnSelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                if (dataKey != null)
                {
                    ...
                    HiddenField hdnfieldRelatedPersonID = (HiddenField)row.FindControl("hdnfieldRelatedPersonID");
					HiddenField hdnfieldGroupID = (HiddenField)row.FindControl("hdnfieldGroupID");
					ViewState["GroupID"] = hdnfieldGroupID.Value;
					txtbxSourceName.Text = hdnfieldProfessionalName.Value;
                    hdnfieldProfessionalnameid.Value = hdnfieldRelatedPersonID.Value;
                    drpdwnRelationType.ClearSelection();
                    this.drpdwnRelationType.Items.FindByText(row.Cells[1].Text).Selected = true;
                    if (!row.Cells[2].Text.Contains("&nbsp;"))
                    {
                        drpdwnChildRelation.ClearSelection();
                        this.drpdwnChildRelation.Items.FindByText(row.Cells[2].Text).Selected = true;
                    }
                    ...
                    {
                        txtbxRelationBreak.Text = row.Cells[5].Text.Equals("&nbsp;") ? "" : row.Cells[5].Text.Contains("&quot;") ? row.Cells[5].Text.Replace("&quot;", "\"") : row.Cells[5].Text;
                    }
                    else
                    {
                        txtbxRelationBreak.Text = row.Cells[5].Text.Equals("&nbsp;") ? "" : row.Cells[5].Text.Contains("&#39;") ? row.Cells[5].Text.Replace("&#39;", "'") : row.Cells[5].Text;
        ...
        protected void btnClose_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (professionalId.Value != "")
                {
                    ...
                    Session["ProfessionalBaseCenter"] = hdnfieldProfessionalName.Value;
                }
        ...
        protected void btnFileUpload_Click(object sender, EventArgs e)
        {
            try
            {
                if (flupload.HasFile)
                {
                    ...
                    if (Path.GetFileNameWithoutExtension(flupload.PostedFile.FileName).Equals("Professional_Relation"))
                    {
        ...
        private void Import_To_Grid(string FilePath, string Extension)
        {
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    var dtErrorResult = new ProfessionalBL().Import30(dt, "ProfessionalRelation");
                    if (dtErrorResult.Rows.Count > 0)
                    {
                        ...
                        {
                            ExcelWorksheet ws = xp.Workbook.Worksheets.Add("Professional_Relation");
                            int rowstart = 1;
                            int colstart = 1;
                            int rowend = rowstart;
                            ...
                            Response.AddHeader("content-disposition", "attachment;filename=Professional_Relation.xlsx");
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                using (DataSet ds = new ProfessionalBL().GetExport(_professionalId, "Relation"))
                {
