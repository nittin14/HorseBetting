...
namespace VKATalk.PopUps
{
    ...
    public partial class HomeClass: System.Web.UI.Page
    {
        int _userId = 1;
        int _horseId = 0;
            ...
            if (!IsPostBack)
            {
                try
                {
                    ...
                    if (_horseId != 0)
                    {
                        ...
                        BindDropDown(drpdwnHomeDistance, "HomeClass", "ClassType", "ClassTypeID");
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                //{
                //    ds = new MasterHorseBL().GetHorseTillDateValidation(_horseId, "HomeClass", txtbxFromDate.Text, btnSave.Text);
                //}
                //else
                //{
                //    ds = new MasterHorseBL().GetHorseTillDateValidation((int)ViewState["horseRecordID"], "HomeClass", txtbxFromDate.Text, btnSave.Text);
                //}
                if (btnSave.Text.Equals("Add"))
                {
                    ds = new MasterHorseBL().GetHorseTillDateValidationMultiple(_horseId, "HomeClass", txtbxFromDate.Text, btnSave.Text, drpdwnHomeDistance.SelectedItem.Value, drpdwnSupportType.SelectedItem.Value);
                }
                else
                {
                    ds = new MasterHorseBL().GetHorseTillDateValidationMultiple((int)ViewState["horseRecordID"], "HomeClass", txtbxFromDate.Text, btnSave.Text, drpdwnHomeDistance.SelectedItem.Value, drpdwnSupportType.SelectedItem.Value);
                }
                ...
                else
                {
                    if (btnSave.Text.Equals("Add"))
                    {
                        status = new MasterHorseBL().HomeClass(
                            _horseId,
        ...
        {
            DataTable dt = new MasterHorseBL().GetHorseNameAutoFiller("HomeClass", prefixText);
        ...
        private void BindData()
        {
            try
            {
                DataSet ds = new MasterHorseBL().GetHorseNameWithCombination(_horseId, "HomeClass");
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                if (!ViewState["horseRecordID"].Equals(""))
                {
                    ...
                    var status = new MasterHorseBL().HomeClass((int)ViewState["horseRecordID"],0,0,"","__-__-____","__-__-____","",this._userId,"Delete");
                    if (status > 0)
                    {
                        ClearAllSelection(this);
                        BindData();
        ...
        protected void btnFileUpload_Click(object sender, EventArgs e)
        {
            try
            {
                if (flupload.HasFile)
                {
                    ...
                    if (Path.GetFileNameWithoutExtension(flupload.PostedFile.FileName).Equals("Horse_HomeClass"))
                    {
        ...
        private void Import_To_Grid(string FilePath, string Extension)
        {
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    //var dtErrorResult = new MasterHorseBL().UploadExcelRecordBulkMinimumColumns(dt, "HorseHomeClass");
                    var dtErrorResult = new MasterHorseBL().Import30(dt, "HorseHomeClass");
                    if (dtErrorResult.Rows.Count > 0)
                    {
                        ...
                        {
                            ExcelWorksheet ws = xp.Workbook.Worksheets.Add("Horse Class");
                            int rowstart = 1;
                            int colstart = 1;
                            int rowend = rowstart;
                            ...
                            Response.AddHeader("content-disposition", "attachment;filename=Horse_HomeClass.xlsx");
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            try
            {
                //using (DataSet ds = new MasterHorseBL().GetHorseNameWithCombination(_horseId, "HorseHomeClass"))
                using (DataSet ds = new MasterHorseBL().GetExport(_horseId, "HorseHomeClass"))
                {
                    ...
                    if (dt.Rows.Count > 0)
                    {
                       // dt.Columns.Remove("HorseOwnerStudID");
                        using (ExcelPackage xp = new ExcelPackage())
                        {
                            ExcelWorksheet ws = xp.Workbook.Worksheets.Add("Horse_HomeClass");
                            int rowstart = 1;
                            int colstart = 1;
                            int rowend = rowstart;
