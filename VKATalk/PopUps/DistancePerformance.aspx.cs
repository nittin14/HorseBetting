...
namespace VKATalk.PopUps
{
    ...
    public partial class DistancePerformance : System.Web.UI.Page
    {
        int _userId = 1;
        int _horseId = 0;
            ...
            if (!IsPostBack)
            {
                try
                {
                    ...
                    if (_horseId != 0)
                    {
                        ...
                        BindDropDown(drpdwnHomeDistance, "DistancePerformance", "Distance", "DistanceID");
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                //{
                //    ds = new MasterHorseBL().GetHorseTillDateValidation(_horseId, "DistancePerformanceGroup", txtbxFromDate.Text, btnSave.Text);
                //}
                //else
                //{
                //    ds = new MasterHorseBL().GetHorseTillDateValidation((int)ViewState["horseRecordID"], "DistancePerformanceGroup", txtbxFromDate.Text, btnSave.Text);
                //}
                if (btnSave.Text.Equals("Add"))
                {
                ...
                else
                {
                    ...
                    if (btnSave.Text.Equals("Add"))
                    {
                        status = new MasterHorseBL().DistancePerformance(
                            _horseId,
        ...
        {
            DataTable dt = new MasterHorseBL().GetHorseNameAutoFiller("DistancePerformance", prefixText);
        ...
        private void BindData()
        {
            try
            {
                DataSet ds = new MasterHorseBL().GetHorseNameWithCombination(_horseId, "DistancePerformance");
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                if (!ViewState["horseRecordID"].Equals(""))
                {
                    ...
                    var status = new MasterHorseBL().DistancePerformance((int)ViewState["horseRecordID"], 0, 0, "", "__-__-____", "__-__-____", "", this._userId, "Delete");
                    if (status > 0)
                    {
                        ClearAllSelection(this);
                        BindData();
        ...
        protected void btnFileUpload_Click(object sender, EventArgs e)
        {
            try
            {
                if (flupload.HasFile)
                {
                    ...
                    if (Path.GetFileNameWithoutExtension(flupload.PostedFile.FileName).Equals("Horse_Distance"))
                    {
        ...
        private void Import_To_Grid(string FilePath, string Extension)
        {
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    //var dtErrorResult = new MasterHorseBL().UploadExcelRecordBulkMinimumColumns(dt, "HorseDistancePerformance");
                            var dtErrorResult = new MasterHorseBL().Import30(dt, "HorseDistancePerformance");
                    if (dtErrorResult.Rows.Count > 0)
                    {
                        using (ExcelPackage xp = new ExcelPackage())
                        {
                            ExcelWorksheet ws = xp.Workbook.Worksheets.Add("Horse DistancePerformance");
                            int rowstart = 1;
                            int colstart = 1;
                            int rowend = rowstart;
                            ...
                            Response.AddHeader("content-disposition", "attachment;filename=Horse_Distance.xlsx");
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            try
            {
                using (DataSet ds = new MasterHorseBL().GetExport(_horseId, "HorseDistancePerformance"))
                {
                    DataTable dt = ds.Tables[0];
                    if (dt.Rows.Count > 0)
                    {
                        //dt.Columns.Remove("DistancePerformanceID");
                        using (ExcelPackage xp = new ExcelPackage())
                        {
                            ExcelWorksheet ws = xp.Workbook.Worksheets.Add("HorseDistancePerformance");
                            int rowstart = 1;
                            int colstart = 1;
                            int rowend = rowstart;
