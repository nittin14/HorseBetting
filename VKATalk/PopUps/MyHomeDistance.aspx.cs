...
namespace VKATalk.PopUps
{
    ...
    public partial class MyHomeDistance : System.Web.UI.Page
    {
        int _userId = 1;
        int _horseId = 0;
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                //{
                //    ds = new MasterHorseBL().GetHorseTillDateValidation(_horseId, "MyHomeDistance", txtbxFromDate.Text, btnSave.Text);
                //}
                //else
                //{
                //    ds = new MasterHorseBL().GetHorseTillDateValidation((int)ViewState["horseRecordID"], "MyHomeDistance", txtbxFromDate.Text, btnSave.Text);
                //}
                if (btnSave.Text.Equals("Add"))
                {
                    ds = new MasterHorseBL().GetHorseTillDateValidationMultiple(_horseId, "MyHomeDistance", txtbxFromDate.Text, btnSave.Text, drpdwnHomeDistance.SelectedItem.Value, drpdwnSupportType.SelectedItem.Value);
                }
                else
                {
                    ds = new MasterHorseBL().GetHorseTillDateValidationMultiple((int)ViewState["horseRecordID"], "MyHomeDistance", txtbxFromDate.Text, btnSave.Text, drpdwnHomeDistance.SelectedItem.Value, drpdwnSupportType.SelectedItem.Value);
                }
                ...
                else
                {
                    if (btnSave.Text.Equals("Add"))
                    {
                        status = new MasterHorseBL().MyHomeDistance(
                            _horseId,
        ...
        {
            DataTable dt = new MasterHorseBL().GetHorseNameAutoFiller("MyHomeDistanceComments", prefixText);
        ...
        {
               DataSet ds = new MasterHorseBL().GetHorseNameWithCombination(_horseId, "MyHomeDistance");
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    var status = new MasterHorseBL().MyHomeDistance((int)ViewState["horseRecordID"], 0, 0, "", "__-__-____", "__-__-____", "", this._userId, "Delete");
                    if (status > 0)
                    {
                        ClearAllSelection(this);
                        BindData();
        ...
        protected void btnFileUpload_Click(object sender, EventArgs e)
        {
            try
            {
                if (flupload.HasFile)
                {
                    ...
                    if (Path.GetFileNameWithoutExtension(flupload.PostedFile.FileName).Equals("Horse_MyHomeDistance"))
                    {
        ...
        private void Import_To_Grid(string FilePath, string Extension)
        {
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    var dtErrorResult = new MasterHorseBL().Import30(dt, "HorseMyHomeDistance");
                    if (dtErrorResult.Rows.Count > 0)
                    {
                        ...
                        {
                            ExcelWorksheet ws = xp.Workbook.Worksheets.Add("Horse MyHomeDistance");
                            int rowstart = 1;
                            int colstart = 1;
                            int rowend = rowstart;
                            ...
                            Response.AddHeader("content-disposition", "attachment;filename=Horse_MyHomeDistance.xlsx");
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            try
            {
                //using (DataSet ds = new MasterHorseBL().GetHorseNameWithCombination(_horseId, "HorseMyHomeDistance"))
                using (DataSet ds = new MasterHorseBL().GetExport(_horseId, "HorseMyHomeDistance"))
                {
                    DataTable dt = ds.Tables[0];
                    if (dt.Rows.Count > 0)
                    {
                     //   dt.Columns.Remove("MyHomeDistanceID");
                        using (ExcelPackage xp = new ExcelPackage())
                        {
                            ExcelWorksheet ws = xp.Workbook.Worksheets.Add("Horse_MyHomeDistance");
                            int rowstart = 1;
                            int colstart = 1;
                            int rowend = rowstart;
