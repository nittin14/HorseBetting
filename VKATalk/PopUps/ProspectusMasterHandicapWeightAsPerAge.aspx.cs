...
namespace VKATalk.PopUps
{
    ...
    public partial class ProspectusMasterHandicapWeightAsPerAge : System.Web.UI.Page
    {
            ...
            if (!IsPostBack)
            {
                try
                {
                    if (_masterraceId != 0)
                    {
                        ...
                        BindDropDown(drpdwnAge, "AgeCondition", "AgeCondition", "AgeConditionID");
                        drpdwnAge.Items.Insert(0, new ListItem("-- Please select --", "-1"));

                        BindDropDown(drpdwnGender, "Horse_HorseSex", "HorseSex", "HorseSexID");
            ...
            try
            {
				...
				Int64 checkTillDate = 0;
                    ...
                    else
                    {
                        ...
                        {
                            status = new ProspectusBL().HandicapWeightAsPerAge(
                                _masterraceId,
                                Convert.ToInt32(drpdwnAge.SelectedItem.Value),
                                txtbxhandicapWeight.Text,
                                Convert.ToInt32(drpdwnFromYear.SelectedItem.Value),
                                Convert.ToInt32(drpdwnTillYear.SelectedItem.Value),
                                txtbxComment.Text,
                                1,
                                "Insert",
                                drpdwnGender.SelectedItem.Value);
                        }
        ...
        {
            DataTable dt = new ProspectusBL().GetprospectusAutoFiller("ProspectusMasterSweepStake", prefixText);
        ...
        private void BindData()
        {
            try
            {
                ...
                DataSet ds = new ProspectusBL().GetProspectusNameWithCombination(_masterraceId, "ProspectusMasterHandicapWeightAsPerAge");
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                if (!ViewState["MasterRaceID"].Equals(""))
                {
                    ...
                    var status = new ProspectusBL().HandicapWeightAsPerAge((int)ViewState["MasterRaceID"],0, "", 0, 0, txtbxComment.Text, 1, "Delete",string.Empty);
        ...
        protected void GvHorseStatus_OnSelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                if (dataKey != null)
                {
                    ...
                    drpdwnAge.Items.FindByText(hdnval.Value).Selected = true;
                    drpdwnGender.Items.FindByText(row.Cells[0].Text).Selected = true;
                    txtbxhandicapWeight.Text = row.Cells[2].Text;
        ...
        protected void btnFileUpload_Click(object sender, EventArgs e)
        {
            try
            {
                if (flupload.HasFile)
                {
                    ...
                    if (Path.GetFileNameWithoutExtension(flupload.PostedFile.FileName).Equals("Prospectus_Master_HWAPAgeCondition"))
                    {
        ...
        private void Import_To_Grid(string FilePath, string Extension)
        {
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    var dtErrorResult = new ProspectusBL().Import30(dt, "WeightAsperAGe", _masterraceId);
                    if (dtErrorResult.Rows.Count > 0)
                    {
                       // Page.ClientScript.RegisterStartupScript(this.GetType(), "Popup", "alert('Issue in few record. Please check the XL sheet');", true);
                        ...
                        {
                            ExcelWorksheet ws = xp.Workbook.Worksheets.Add("MasterAsPerAge");
                            int rowstart = 1;
                            int colstart = 1;
                            int rowend = rowstart;
                            ...
                            Response.AddHeader("content-disposition", "attachment;filename=Prospectus_Master_HWAPAgeCondition.xlsx");
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                using (DataSet ds = new ProspectusBL().GetExport(_masterraceId, "ProspectusMasterWeightAsPerAge"))
                {
