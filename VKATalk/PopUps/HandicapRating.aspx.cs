...
namespace VKATalk.PopUps
{
    ...
    public partial class HandicapRating : System.Web.UI.Page
    {
        int _horseId = 0;
        int _userId = 1;
       ...
       /// <summary>
        ...
        public void BindData()
        {
            try
            {
                ...
                DataTable dt = Bl.GetHorseDetail(Convert.ToString(_horseId), "HandicapRating");
                if (dt.Rows.Count > 0)
                {
        ...
        {
            DataTable dt = new MasterHorseBL().GetHorseNameAutoFiller("HandicapRating", prefixText);
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                  //  ds = new MasterHorseBL().GetHorseTillDateValidation(_horseId, "HandicapRating", txtbxFromDate.Text, btnSave.Text);
                }
                else
                {
                    //ds = new MasterHorseBL().GetHorseTillDateValidation((int)ViewState["horseStatusID"], "HandicapRating", txtbxFromDate.Text, btnSave.Text);
                }
                ...
                else
                {
                    if (btnSave.Text.Equals("Add"))
                    {
                        status = new MasterHorseBL().HandicapRating(
                            _horseId,
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    new MasterHorseBL().HandicapRating((int)ViewState["horseStatusID"], "", txtbxFromDate.Text, txtbxTillDate.Text,txtbxComment.Text, 1, "Delete");
                    ClearAllSelection(this);
                    BindData();
                    btnSave.Text = "Add";
        ...
        protected void btnFileUpload_Click(object sender, EventArgs e)
        {
            try
            {
                if (flupload.HasFile)
                {
                    ...
                    if (Path.GetFileNameWithoutExtension(flupload.PostedFile.FileName).Equals("Horse_HandicapRating"))
                    {
        ...
        private void Import_To_Grid(string FilePath, string Extension)
        {
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    var dtErrorResult = new MasterHorseBL().Import30(dt, "HorseHandicapRating");
                    if (dtErrorResult.Rows.Count > 0)
                    {
                        ...
                        {
                            ExcelWorksheet ws = xp.Workbook.Worksheets.Add("Horse HandicapRating");
                            int rowstart = 1;
                            int colstart = 1;
                            int rowend = rowstart;
                            ...
                            Response.AddHeader("content-disposition", "attachment;filename=Horse_HandicapRating.xlsx");
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                using (DataSet ds = new MasterHorseBL().GetExport(_horseId, "HorseHandicapRating"))
                {
                    ...
                    if (dt.Rows.Count > 0)
                    {
                       // dt.Columns.Remove("HorseOwnerStudID");
                        using (ExcelPackage xp = new ExcelPackage())
                        {
                            ExcelWorksheet ws = xp.Workbook.Worksheets.Add("HorseHandicapRating");
                            int rowstart = 1;
                            int colstart = 1;
                            int rowend = rowstart;
