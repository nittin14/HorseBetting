...
namespace VKATalk.PopUps
{
    ...
    public partial class ProfessionalStatus : System.Web.UI.Page
    {
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            ...
            {
                int status = 0;
                //DataSet ds = Bl.GetProfessionalTillDateValidation(_professionalId, "ProfessionalStatus", this.btnSave.Text);
                DataSet ds = null;
                if (btnSave.Text.Equals("Add"))
                {
                    ds = new ProfessionalBL().GetProfessionalTillDateValidation(_professionalId, "ProfessionalStatus", txtbxFromDate.Text, btnSave.Text);
                }
                else
                {
                    ds = new ProfessionalBL().GetProfessionalTillDateValidation((int)ViewState["ProfessionalRecordID"], "ProfessionalStatus", txtbxFromDate.Text, btnSave.Text);
                }
                ...
                else
                {
                    if (btnSave.Text.Equals("Add"))
                    {
                        status = Bl.InsertProfessionalStatus(
                            _professionalId,
                            rdbtnStatus.SelectedItem.Text,
                            txtbxFromDate.Text,
                            txtbxTillDate.Text,
                            txtbxComment.Text,
                            1,
        ...
        {
            DataTable dt = new ProfessionalBL().GetProfessionalNameAutoFiller("ProfessionalStatusComments", prefixText);
        ...
        {
                DataSet ds= Bl.GetProfessionalNameWithCombination(_professionalId, "ProfessionalStatus");
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    var status = Bl.InsertProfessionalStatus((int)ViewState["ProfessionalRecordID"], "", txtbxFromDate.Text, txtbxTillDate.Text,
        ...
        protected void btnFileUpload_Click(object sender, EventArgs e)
        {
            try
            {
                if (flupload.HasFile)
                {
                    ...
                    if (Path.GetFileNameWithoutExtension(flupload.PostedFile.FileName).Equals("Professional_PermanentStatus"))
                    {
        ...
        private void Import_To_Grid(string FilePath, string Extension)
        {
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    var dtErrorResult = new ProfessionalBL().Import30(dt, "ProfessionalStatus");
                    if (dtErrorResult.Rows.Count > 0)
                    {
                        ...
                        {
                            ExcelWorksheet ws = xp.Workbook.Worksheets.Add("ProfessionalStatus");
                            int rowstart = 1;
                            int colstart = 1;
                            int rowend = rowstart;
                            ...
                            Response.AddHeader("content-disposition", "attachment;filename=Professional_PermanentStatus.xlsx");
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                using (DataSet ds = new ProfessionalBL().GetExport(_professionalId, "ProfessionalStatus"))
                {
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        using (ExcelPackage xp = new ExcelPackage())
                        {
                            ExcelWorksheet ws = xp.Workbook.Worksheets.Add("Professional_PermanentStatus");
                            int rowstart = 1;
                            int colstart = 1;
                            int rowend = rowstart;
