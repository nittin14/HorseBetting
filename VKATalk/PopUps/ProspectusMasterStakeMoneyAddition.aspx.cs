...
namespace VKATalk.PopUps
{
    ...
    public partial class ProspectusMasterStakeMoneyAddition : System.Web.UI.Page
    {
            ...
            if (!IsPostBack)
            {
                try
                {
                    if (_masterraceId != 0)
                    {
                        ...
                        {}
                        BindDropDown(drpdwnAdditionType, "Master_StakeMoneyAddition", "StakeMoneyAddition", "StakeMoneyAdditionMID");
                        drpdwnAdditionType.Items.Insert(0, new ListItem("-- Please select --", "-1"));
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
				...
				DataSet ds = new ProspectusBL().GetProspectusNameWithCombination(_masterraceId, "StakeMoneyAdditional");
				var checkstatus = 0;
				//if (ds.Tables[0].Rows.Count > 0)
				//{
				//	for (int count = 0; count <= ds.Tables[0].Rows.Count - 1; count++)
				//	{
				//		if (ds.Tables[0].Rows[count][1].ToString().Equals(drpdwnAdditionType.SelectedItem.Text) &&
				//					ds.Tables[0].Rows[count][4].ToString().Equals(""))
				//		{
				//			if ((hdnfieldstakemoneyadditionID.Value.ToString() != null) || (hdnfieldstakemoneyadditionID.Value.ToString() != ""))
				//			{
				//				if (ds.Tables[0].Rows[count][0].ToString().Equals(hdnfieldstakemoneyadditionID.Value.ToString()))
				//				{
				//					checkstatus = 0;
				//				}
				//				else
				...
				//	}
				//}
				//hdnfieldstakemoneyadditionID.Value = string.Empty;
				if (checkstatus == 1)
				{
                ...
                else
                {
                    ...
                    {
                        status = new ProspectusBL().StakeMoneyAddition(
                            _masterraceId,
                            Convert.ToInt32(drpdwnAdditionType.SelectedItem.Value),
                            txtbxAmount.Text,
        ...
        {
            DataTable dt = new ProspectusBL().GetprospectusAutoFiller("StakeMoneyAdditional", prefixText);
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    var status = new ProspectusBL().StakeMoneyAddition((int)ViewState["MasterRaceID"], 0, "", 0, 0, txtbxComment.Text, 1, "Delete");
        ...
        protected void GvHorseStatus_OnSelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                if (dataKey != null)
                {
					...
					hdnfieldstakemoneyadditionID.Value = dataKey.Value.ToString();
					drpdwnAdditionType.Items.FindByText(hdnval.Value).Selected = true;
                    if (!row.Cells[1].Text.Contains("&nbsp;"))
                    {
                        txtbxAmount.Text = row.Cells[1].Text;
                    }
        ...
        protected void btnFileUpload_Click(object sender, EventArgs e)
        {
            try
            {
                if (flupload.HasFile)
                {
                    ...
                    if (Path.GetFileNameWithoutExtension(flupload.PostedFile.FileName).Equals("Prospectus_Master_StakeMoneyAddition"))
                    {
        ...
        private void Import_To_Grid(string FilePath, string Extension)
        {
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    var dtErrorResult = new ProspectusBL().Import30(dt, "StakeMoneyAddition", _masterraceId);
                    if (dtErrorResult.Rows.Count > 0)
                    {
                        using (ExcelPackage xp = new ExcelPackage())
                        {

                            ExcelWorksheet ws = xp.Workbook.Worksheets.Add("StakeMoneyAddition");
                            ...
                            Response.AddHeader("content-disposition", "attachment;filename=Prospectus_Master_StakeMoneyAddition.xlsx");
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                using (DataSet ds = new ProspectusBL().GetExport(_masterraceId, "StakeMoneyAddition"))
                {
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        using (ExcelPackage xp = new ExcelPackage())
                        {
                            ...
                            ExcelWorksheet ws = xp.Workbook.Worksheets.Add("ProspectMasterStakeMoneyAddition");
