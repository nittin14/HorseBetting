...
namespace VKATalk.PopUps
{
    ...
    public partial class ProfessionalHabit : System.Web.UI.Page
    {
        int _userId = 1;
                ...
                if (!IsPostBack)
                {
                    try
                    {
                        ...
                        if (_professionalId != 0)
                        {
							...
							BindDropDown(this.drpdwnHabitCharacteristics, "ProfessionalHabitCharacteristic", "ProfessionalCharacter", "ProfessionalCharacterMID");
							drpdwnHabitCharacteristics.Items.Insert(0, new ListItem("-- Please select --", "-1"));
							BindData();
                        }
                    }
                    catch (Exception ex)
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            ...
            {
                int status = 0;
                //DataSet ds = new ProfessionalBL().GetProfessionalTillDateValidation(_professionalId, "Habit", this.btnSave.Text);
                DataSet ds = null;
                //if (btnSave.Text.Equals("Add"))
                //{
                //    ds = new ProfessionalBL().GetProfessionalTillDateValidation(_professionalId, "Habit", txtbxFromDate.Text, btnSave.Text);
                //}
                //else
                //{
                //    ds = new ProfessionalBL().GetProfessionalTillDateValidation((int)ViewState["ProfessionalRecordID"], "Habit", txtbxFromDate.Text, btnSave.Text);
                //}
                ...
                if (btnSave.Text.Equals("Add"))
                {
                    ds = new ProfessionalBL().GetProfessionalMultipleTillDateString(_professionalId, "ProfessionalHabit", txtbxFromDate.Text, btnSave.Text, drpdwnHabitCharacteristics.SelectedItem.Value);
                }
                else
                {
                    ds = new ProfessionalBL().GetProfessionalMultipleTillDateString((int)ViewState["ProfessionalRecordID"], "ProfessionalHabit", txtbxFromDate.Text, btnSave.Text, drpdwnHabitCharacteristics.SelectedItem.Value);
                }
                Int64 checkTillDate = Convert.ToInt64(ds.Tables[0].Rows[0][0]);
                if (_professionalId == 0)
                {
                ...
                else
                {
                    if (btnSave.Text.Equals("Add"))
                    {
                        status = new ProfessionalBL().ProfessionalHabit(
                            _professionalId,
							drpdwnHabitCharacteristics.SelectedItem.Value,
                            this.txtbxFromDate.Text,
                            this.txtbxTillDate.Text,
                            txtbxHabitDetail.Text,
        ...
        {
            DataTable dt = new ProfessionalBL().GetProfessionalNameAutoFiller("ProfessionalHabitComment", prefixText);
        ...
        [System.Web.Script.Services.ScriptMethod()]
        [System.Web.Services.WebMethod]
        public static List<string> AddHabit(string prefixText, int count)
        {
            DataTable dt = new ProfessionalBL().GetProfessionalNameAutoFiller("ProfessionalHabit", prefixText);
            List<string> commentList = new List<string>();
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                commentList.Add(dt.Rows[i][0].ToString());
        ...
        [System.Web.Script.Services.ScriptMethod()]
        [System.Web.Services.WebMethod]
        public static List<string> AddHabitList(string prefixText, int count)
        {
            DataTable dt = new ProfessionalBL().GetProfessionalNameAutoFiller("ProfessionalHabitDetails", prefixText);
            List<string> commentList = new List<string>();
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                commentList.Add(dt.Rows[i][0].ToString());
        ...
        {
                DataSet ds = new ProfessionalBL().GetProfessionalNameWithCombination(_professionalId, "Habit");
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    var status = new ProfessionalBL().ProfessionalHabit((int)ViewState["ProfessionalRecordID"], "", "__-__-____", "__-__-____", "", "", this._userId, "Delete");
                    if (status > 0)
                    {
                        ClearAllSelection(this);
                        BindData();
        ...
        protected void GvHomeDistance_OnSelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                if (dataKey != null)
                {
					...
					drpdwnHabitCharacteristics.Items.FindByText(hdnval.Value).Selected = true;
					//txtbxHabitDetail.Text = row.Cells[1].Text;
					if (row.Cells[1].Text.Contains("&quot;"))
					{
						txtbxHabitDetail.Text = row.Cells[1].Text.Equals("&nbsp;") ? "" : row.Cells[1].Text.Contains("&quot;") ? row.Cells[1].Text.Replace("&quot;", "\"") : row.Cells[1].Text;
					}
					else
					{
						txtbxHabitDetail.Text = row.Cells[1].Text.Equals("&nbsp;") ? "" : row.Cells[1].Text.Contains("&#39;") ? row.Cells[1].Text.Replace("&#39;", "'") : row.Cells[1].Text;
        ...
        protected void btnFileUpload_Click(object sender, EventArgs e)
        {
            try
            {
                if (flupload.HasFile)
                {
                    ...
                    if (Path.GetFileNameWithoutExtension(flupload.PostedFile.FileName).Equals("Professional_Character"))
                    {
        ...
        private void Import_To_Grid(string FilePath, string Extension)
        {
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    var dtErrorResult = new ProfessionalBL().Import30(dt, "ProfessionalHabit");
                    if (dtErrorResult.Rows.Count > 0)
                    {
                        ...
                        {
                            ExcelWorksheet ws = xp.Workbook.Worksheets.Add("Professional_Character");
                            int rowstart = 1;
                            int colstart = 1;
                            int rowend = rowstart;
                            ...
                            Response.AddHeader("content-disposition", "attachment;filename=Professional_Character.xlsx");
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                using (DataSet ds = new ProfessionalBL().GetExport(_professionalId, "Habit"))
                {
