...
namespace VKATalk.PopUps
{
    ...
    public partial class CurrentMissionPopup : System.Web.UI.Page
    {
        int _horseId = 0;
        int _userId = 1;
		...
		public void horseinformation()
		{
			...
			BindDropDown(drpdwnCurrentMission, "CurrentMission", "CurrentMission", "CurrentMissionMID");
			drpdwnCurrentMission.Items.Insert(0, new ListItem("-- Please select --", "-1"));
			BindData();
        ...
        public void BindData()
        {
            try
            {
                ...
                DataTable dt = Bl.GetHorseDetail(Convert.ToString(_horseId), "CurrentMission");
                if (dt.Rows.Count > 0)
                {
                    GvCurrentMission.DataSource = dt;
                    GvCurrentMission.DataBind();
                }
                else
                {
                    GvCurrentMission.DataSource = new DataTable();
                    GvCurrentMission.DataBind();
                }
        ...
        [System.Web.Script.Services.ScriptMethod()]
        [System.Web.Services.WebMethod]
        public static List<string> SearchCurrentMission(string prefixText, int count)
        {
            DataTable dt = new MasterHorseBL().GetHorseNameAutoFiller("CurrentMission", prefixText);
            List<string> currentMission = new List<string>();
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                currentMission.Add(dt.Rows[i][0].ToString());
            }
        ...
        {
            DataTable dt = new MasterHorseBL().GetHorseNameAutoFiller("CurrentMissionComments", prefixText);
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
				...
				//{
				//    ds = new MasterHorseBL().GetHorseTillDateValidation(_horseId, "CurrentMission", txtbxFromDate.Text, btnSave.Text);
				//}
				//else
				//{
				//    ds = new MasterHorseBL().GetHorseTillDateValidation((int)ViewState["horseStatusID"], "CurrentMission", txtbxFromDate.Text, btnSave.Text);
				//}
                ...
                else
                {
                    if (btnSave.Text.Equals("Add"))
                    {
                       status= new MasterHorseBL().CurrentMission(
                            _horseId,
                            0,
                            drpdwnCurrentMission.SelectedItem.Value,
                            txtbxFromDate.Text,
                            txtbxTillDate.Text,
                            txtbxComment.Text,
                            1,
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    new MasterHorseBL().CurrentMission((int)ViewState["horseStatusID"], 0,
        ...
        protected void GvCurrentMission_OnSelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                btnSave.Text = "Update";
                GridViewRow row = GvCurrentMission.SelectedRow;
                HiddenField hdnval = (HiddenField)row.FindControl("hdnfieldStatus");
                var dataKey = GvCurrentMission.DataKeys[row.RowIndex];
                if (dataKey != null)
                {
                    ClearAllSelection(this);
                    ViewState["horseStatusID"] = dataKey.Value;
                    drpdwnCurrentMission.ClearSelection();
                    drpdwnCurrentMission.Items.FindByText(hdnval.Value).Selected = true;
        ...
        protected void btnFileUpload_Click(object sender, EventArgs e)
        {
            try
            {
                if (flupload.HasFile)
                {
                    ...
                    if (Path.GetFileNameWithoutExtension(flupload.PostedFile.FileName).Equals("Horse_CurrentMission"))
                    {
        ...
        private void Import_To_Grid(string FilePath, string Extension)
        {
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    //ar dtErrorResult = new MasterHorseBL().UploadExcelRecordBulkMinimumColumns(dt, "HorseCurrentMission");
                    var dtErrorResult = new MasterHorseBL().Import30(dt, "HorseCurrentMission");
                    if (dtErrorResult.Rows.Count > 0)
                    {
                      //  Page.ClientScript.RegisterStartupScript(this.GetType(), "Popup", "alert('Issue in few record. Please check the XL sheet');", true);
                        ...
                        {
                            ExcelWorksheet ws = xp.Workbook.Worksheets.Add("Horse CurrentMission");
                            int rowstart = 1;
                            int colstart = 1;
                            int rowend = rowstart;
                            ...
                            Response.AddHeader("content-disposition", "attachment;filename=Horse_CurrentMission.xlsx");
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            try
            {
                //using (DataSet ds = new MasterHorseBL().GetHorseNameWithCombination(_horseId, "HorseCurrentMission"))
                using (DataSet ds = new MasterHorseBL().GetExport(_horseId, "HorseCurrentMission"))
                {
                    DataTable dt = ds.Tables[0];
                    if (dt.Rows.Count > 0)
                    {
                       // dt.Columns.Remove("CurrentMissionID");
                        using (ExcelPackage xp = new ExcelPackage())
                        {
                            ExcelWorksheet ws = xp.Workbook.Worksheets.Add("HorseCurrentMission");
                            int rowstart = 1;
                            int colstart = 1;
                            int rowend = rowstart;
