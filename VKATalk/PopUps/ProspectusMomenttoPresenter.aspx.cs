...
namespace VKATalk.PopUps
{
    ...
    public partial class ProspectusMomenttoPresenter : System.Web.UI.Page
    {
        int _masterraceId = 0;
            ...
            if (!IsPostBack)
            {
                try
                {
                    if (_masterraceId != 0)
                    {
                        ...
                        //BindDropDown(drpndwnProfessional, "ProfessionalName", "ProfessionalName", "ProfessionalNameID");
                        //drpndwnProfessional.Items.Insert(0, new ListItem("-- Please select --", "-1"));
        ...
        public static List<string> AddSponcerList(string prefixText, int count)
        {
            DataTable dt = new ProspectusBL().GetprospectusAutoFiller("SponcerName", prefixText);
            List<string> commentList = new List<string>();
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                commentList.Add(AjaxControlToolkit.AutoCompleteExtender.CreateAutoCompleteItem(
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
				...
				//{
				//    ds = new ProspectusBL().GetProspectusTillDateValidation(_masterraceId, "Presenter", drpdwnFromYear.SelectedItem.Text, btnSave.Text);
				//}
				//else
				//{
				//    ds = new ProspectusBL().GetProspectusTillDateValidation((int)ViewState["MasterRaceID"], "Presenter", drpdwnFromYear.SelectedItem.Text, btnSave.Text);
				//}
                ...
                else
                {
                    if (btnSave.Text.Equals("Add"))
                    {
                        status = new ProspectusBL().Presenter(
                            _masterraceId,
                            Convert.ToInt32(hdnfieldSponcerNameID.Value),
                            Convert.ToInt32(drpdwnFromYear.SelectedItem.Value),
                            Convert.ToInt32(drpdwnTillYear.SelectedItem.Value),
                            txtbxOtherDetails.Text,
                            txtbxComment.Text,
                            1,
                            "Insert");
                    }
                    ...
                    if (status == 1)
                    {
                        ...
                        GvProfessionalProfile.DataSource = new DataTable();
                        GvProfessionalProfile.DataBind();
                    }
                    else if (status == 2)
                    {
        ...
        {
            DataTable dt = new ProspectusBL().GetprospectusAutoFiller("PresenterCommentList", prefixText);
            List<string> commentList = new List<string>();
            for (int i = 0; i < dt.Rows.Count; i++)
            {
        ...
        [System.Web.Script.Services.ScriptMethod()]
        [System.Web.Services.WebMethod]
        public static List<string> AddOtherDetails(string prefixText, int count)
        {
            DataTable dt = new ProspectusBL().GetprospectusAutoFiller("PresenterOtherList", prefixText);
            List<string> commentList = new List<string>();
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                commentList.Add(dt.Rows[i][0].ToString());
        ...
        private void BindData()
        {
            try
            {
                ...
                DataSet ds = new ProspectusBL().GetProspectusNameWithCombination(_masterraceId, "Presenter");
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    var status = new ProspectusBL().Presenter((int)ViewState["MasterRaceID"], 0,0,0,"",txtbxComment.Text, 1, "Delete");
        ...
        protected void GvHorseStatus_OnSelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                HiddenField hdnfieldGridViewProfessionalNameID = (HiddenField)row.FindControl("hdnfieldGridViewProfessionalNameID");
                var dataKey = GvProspectus.DataKeys[row.RowIndex];
                if (dataKey != null)
                {
                    ClearAllSelection(this);
                    ViewState["MasterRaceID"] = dataKey.Value;
                   // ViewState["GridViewProfessionalNameID"] = hdnfieldGridViewProfessionalNameID.Value;
                    hdnfieldSponcerNameID.Value = hdnfieldGridViewProfessionalNameID.Value;
                    //drpndwnProfessional.Items.FindByText(hdnval.Value).Selected = true;
                    txtbxSponcerName.Text = hdnval.Value;
                    ...
                    {
                        txtbxOtherDetails.Text = row.Cells[3].Text.Equals("&nbsp;") ? "" : row.Cells[3].Text.Contains("&quot;") ? row.Cells[3].Text.Replace("&quot;", "\"") : row.Cells[3].Text;
                    }
                    else
                    {
                        txtbxOtherDetails.Text = row.Cells[3].Text.Equals("&nbsp;") ? "" : row.Cells[3].Text.Contains("&#39;") ? row.Cells[3].Text.Replace("&#39;", "'") : row.Cells[3].Text;
        ...
        protected void btnFileUpload_Click(object sender, EventArgs e)
        {
            try
            {
                if (flupload.HasFile)
                {
                    ...
                    if (Path.GetFileNameWithoutExtension(flupload.PostedFile.FileName).Equals("Prospectus_Master_MomenttoPresenter"))
                    {
        ...
        private void Import_To_Grid(string FilePath, string Extension)
        {
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    var dtErrorResult = new ProspectusBL().Import30(dt, "Presenter", _masterraceId);
                    if (dtErrorResult.Rows.Count > 0)
                    {
                        ...
                        {
                            ExcelWorksheet ws = xp.Workbook.Worksheets.Add("Presenter");
                            int rowstart = 1;
                            int colstart = 1;
                            int rowend = rowstart;
                            ...
                            Response.AddHeader("content-disposition", "attachment;filename=Prospectus_Master_MomenttoPresenter.xlsx");
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                //using (DataSet ds = new ProspectusBL().GetProspectusNameWithCombination(_masterraceId, "Presenter"))
                using (DataSet ds = new ProspectusBL().GetExport(_masterraceId, "Presenter"))
                {
                    DataTable dt = ds.Tables[0];
                    if (dt.Rows.Count > 0)
                    {
                        //dt.Columns.Remove("PresenterID");
                        using (ExcelPackage xp = new ExcelPackage())
                        {
        ...
        protected void txtbxSponcerName_OnTextChanged(object sender, EventArgs e)
        {
            if (txtbxSponcerName.Text != "")
            {
                DataSet ds = new ProspectusBL().GetProfessionalProfileDetail(Convert.ToInt32(hdnfieldSponcerNameID.Value));
                if (ds.Tables[0].Rows.Count > 0)
                {
                    GvProfessionalProfile.DataSource = ds.Tables[0];
                    GvProfessionalProfile.DataBind();
                }
