...
namespace VKATalk.PopUps
{
    ...
    public partial class ExpectedClass: System.Web.UI.Page
    {
        int _userId = 1;
        int _horseId = 0;
            ...
            if (!IsPostBack)
            {
                try
                {
                    ...
                    if (_horseId != 0)
                    {
						...
						BindDropDown(drpdwnHomeDistance, "MasterClassDummy", "ClassDummy", "ClassDummyMID");
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    ds = new MasterHorseBL().GetHorseTillDateValidationMultiple(_horseId, "ExpectedHomeClass", txtbxFromDate.Text, btnSave.Text, drpdwnHomeDistance.SelectedItem.Value, drpdwnSupportType.SelectedItem.Value);
                }
                else
                {
                    ds = new MasterHorseBL().GetHorseTillDateValidationMultiple((int)ViewState["horseRecordID"], "ExpectedHomeClass", txtbxFromDate.Text, btnSave.Text, drpdwnHomeDistance.SelectedItem.Value, drpdwnSupportType.SelectedItem.Value);
                }
                ...
                else
                {
                    if (btnSave.Text.Equals("Add"))
                    {
                        status = new MasterHorseBL().ExpectedClass(
                            _horseId,
        ...
        {
            DataTable dt = new MasterHorseBL().GetHorseNameAutoFiller("ExpectedHomeClass", prefixText);
        ...
        private void BindData()
        {
            try
            {
                DataSet ds = new MasterHorseBL().GetHorseNameWithCombination(_horseId, "ExpectedHomeClass");
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    var status = new MasterHorseBL().ExpectedClass((int)ViewState["horseRecordID"], 0, 0, "", "__-__-____", "__-__-____", "", this._userId, "Delete");
                    if (status > 0)
                    {
                        ClearAllSelection(this);
                        BindData();
                        btnSave.Text = "Add";
                        string message = "Reord deleted successfully.";
                        ClientScript.RegisterStartupScript(this.GetType(), "Popup", "ShowPopup('" + message + "');", true);
        ...
        protected void btnFileUpload_Click(object sender, EventArgs e)
        {
            try
            {
                if (flupload.HasFile)
                {
                    ...
                    if (Path.GetFileNameWithoutExtension(flupload.PostedFile.FileName).Equals("Horse_ExpectedClass"))
                    {
        ...
        private void Import_To_Grid(string FilePath, string Extension)
        {
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    //var dtErrorResult = new MasterHorseBL().UploadExcelRecordBulkMinimumColumns(dt, "HorseExpectedHomeClass");
                    var dtErrorResult = new MasterHorseBL().Import30(dt, "HorseExpectedHomeClass");
                    if (dtErrorResult.Rows.Count > 0)
                    {
                       // Page.ClientScript.RegisterStartupScript(this.GetType(), "Popup", "alert('Issue in few record. Please check the XL sheet');", true);
                        ...
                        {
                            ExcelWorksheet ws = xp.Workbook.Worksheets.Add("Horse_ExpectedClass");
                            int rowstart = 1;
                            int colstart = 1;
                            int rowend = rowstart;
                            ...
                            Response.AddHeader("content-disposition", "attachment;filename=Horse_ExpectedClass.xlsx");
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            try
            {
               // using (DataSet ds = new MasterHorseBL().GetHorseNameWithCombination(_horseId, "HorseExpectedHomeClass"))
                using (DataSet ds = new MasterHorseBL().GetExport(_horseId, "HorseExpectedHomeClass"))
                {
