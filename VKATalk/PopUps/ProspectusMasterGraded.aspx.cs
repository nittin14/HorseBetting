...
namespace VKATalk.PopUps
{
    ...
    public partial class ProspectusMasterGraded : System.Web.UI.Page
    {
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            ...
            {
                int status = 0;
               // DataSet ds = new ProspectusBL().GetProspectusTillDateValidation(_masterraceId, "ProspectusMasterGraded", this.btnSave.Text);
                DataSet ds = null;
                if (!chkbxGrade.Checked)
                {
                    var message = "Please select Grade.";
                    Page.ClientScript.RegisterStartupScript(this.GetType(), "Popup", "ShowPopup('" + message + "');", true);
                }
                else
                {
                    ...
                    {
                        ds = new ProspectusBL().GetProspectusTillDateValidation(_masterraceId, "ProspectusMasterGraded", drpdwnFromYear.SelectedItem.Text, btnSave.Text);
                    }
                    else
                    {
                        ds = new ProspectusBL().GetProspectusTillDateValidation((int)ViewState["MasterRaceID"], "ProspectusMasterGraded", drpdwnFromYear.SelectedItem.Text, btnSave.Text);
                    }
                    ...
                    else
                    {
                        if (btnSave.Text.Equals("Add"))
                        {
                            status = new ProspectusBL().MasterGraded(
                                _masterraceId,
                                "Yes",
        ...
        {
            DataTable dt = new ProspectusBL().GetprospectusAutoFiller("ProspectusMasterGraded", prefixText);
        ...
        private void BindData()
        {
            try
            {
                ...
                DataSet ds = new ProspectusBL().GetProspectusNameWithCombination(_masterraceId, "ProspectusMasterGraded");
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                if (!ViewState["MasterRaceID"].Equals(""))
                {
                    ...
                    var status = new ProspectusBL().MasterGraded((int)ViewState["MasterRaceID"], "", 0,0, txtbxComment.Text, 1, "Delete");
        ...
        protected void btnFileUpload_Click(object sender, EventArgs e)
        {
            try
            {
                if (flupload.HasFile)
                {
                    ...
                    if (Path.GetFileNameWithoutExtension(flupload.PostedFile.FileName).Equals("Prospectus_Master_Grade"))
                    {
        ...
        private void Import_To_Grid(string FilePath, string Extension)
        {
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    var dtErrorResult = new ProspectusBL().Import30(dt, "ProspectusMasterGraded", _masterraceId);
                    if (dtErrorResult.Rows.Count > 0)
                    {
                        using (ExcelPackage xp = new ExcelPackage())
                        {
                            ExcelWorksheet ws = xp.Workbook.Worksheets.Add("ProspectusMasterGraded");
                            int rowstart = 1;
                            int colstart = 1;
                            int rowend = rowstart;
                            ...
                            Response.AddHeader("content-disposition", "attachment;filename=Prospectus_Master_Grade.xlsx");
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                using (DataSet ds = new ProspectusBL().GetExport(_masterraceId, "ProspectusMasterGraded"))
                {
