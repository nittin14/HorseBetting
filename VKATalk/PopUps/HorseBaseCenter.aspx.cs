...
namespace VKATalk.PopUps
{
    ...
    public partial class HorseBaseCenter : System.Web.UI.Page
    {
        int _horseId = 0;
        int _userId = 1;
        ...
        public void BindData()
        {
                ...
                DataTable dt = Bl.GetHorseDetail(Convert.ToString(_horseId), "MasterCenter");
                if (dt.Rows.Count > 0)
                {
        ...
        {
            DataTable dt = new MasterHorseBL().GetHorseNameAutoFiller("HorseBaseCenter", prefixText);
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (btnSave.Text.Equals("Add"))
                    {
                        ds = new MasterHorseBL().GetHorseTillDateValidation(_horseId, "HorseBaseCenter", txtbxFromDate.Text, btnSave.Text);
                    }
                    else
                    {
                        ds = new MasterHorseBL().GetHorseTillDateValidation((int)ViewState["horseStatusID"], "HorseBaseCenter", txtbxFromDate.Text, btnSave.Text);
                    }
                    ...
                    else
                    {
                        if (btnSave.Text.Equals("Add"))
                        {
                            status = new MasterHorseBL().HorseBaseCenter(
                                _horseId,
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    new MasterHorseBL().HorseBaseCenter((int)ViewState["horseStatusID"], Convert.ToInt32(this.drpdwnGender.SelectedItem.Value), txtbxFromDate.Text, txtbxTillDate.Text,
        ...
        protected void btnFileUpload_Click(object sender, EventArgs e)
        {
            try
            {
                if (flupload.HasFile)
                {
                    ...
                    if (Path.GetFileNameWithoutExtension(flupload.PostedFile.FileName).Equals("Horse_BaseCenter"))
                    {
        ...
        private void Import_To_Grid(string FilePath, string Extension)
        {
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    var dtErrorResult = new MasterHorseBL().Import30(dt, "HorseBaseCenter");
                    if (dtErrorResult.Rows.Count > 0)
                    {
                        ...
                        {
                            ExcelWorksheet ws = xp.Workbook.Worksheets.Add("Horse_BaseCenter");
                            int rowstart = 1;
                            int colstart = 1;
                            int rowend = rowstart;
                            ...
                            Response.AddHeader("content-disposition", "attachment;filename=Horse_BaseCenter.xlsx");
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            try
            {
              //  using (DataSet ds = new MasterHorseBL().GetHorseNameWithCombination(_horseId, "HorseBaseCenter"))
                using (DataSet ds = new MasterHorseBL().GetExport(_horseId, "HorseBaseCenter"))
                {
                    DataTable dt = ds.Tables[0];
                    if (dt.Rows.Count > 0)
                    {
                      //  dt.Columns.Remove("BaseCenterID");
                        using (ExcelPackage xp = new ExcelPackage())
                        {
                            ExcelWorksheet ws = xp.Workbook.Worksheets.Add("Horse_BaseCenter");
                            int rowstart = 1;
