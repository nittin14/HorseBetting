...
namespace VKATalk.PopUps
{
    ...
    public partial class HorseRunningCenter : System.Web.UI.Page
    {
        int _horseId = 0;
        int _userId = 1;
            ...
            try
            {
                ...
                if (!IsPostBack)
                {
                    if (_horseId != 0)
                    {
                        ...
                        drpdwnGender.Items.FindByText("Madras").Selected = true;
                        BindData();
                    }
                }
            }
        ...
        public void BindData()
        {
                ...
                DataTable dt = Bl.GetHorseDetail(Convert.ToString(_horseId), "MasterStationCenter");
                if (dt.Rows.Count > 0)
                {
                    this.GvHorseBaseCenter.DataSource = dt;
                    this.GvHorseBaseCenter.DataBind();
                }
                else
                {
                    this.GvHorseBaseCenter.DataSource = new DataTable();
                    this.GvHorseBaseCenter.DataBind();
                }
        }
        ...
        {
            DataTable dt = new MasterHorseBL().GetHorseNameAutoFiller("HorseStationCenter", prefixText);
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                //if (conditioncheck == 0)
                //{
                //    string message = "Please select Till Date, Reason of Change & Status.";
                //    ClientScript.RegisterStartupScript(this.GetType(), "Popup", "ShowPopup('" + message + "');", true);
                //}
                //else
                //{
                    ...
                    {
                        ds = new MasterHorseBL().GetHorseTillDateValidation(_horseId, "HorseStationCenter", txtbxFromDate.Text, btnSave.Text);
                    }
                    else
                    {
                        ds = new MasterHorseBL().GetHorseTillDateValidation((int)ViewState["horseStatusID"], "HorseStationCenter", txtbxFromDate.Text, btnSave.Text);
                    }
                    ...
                    else
                    {
                        if (btnSave.Text.Equals("Add"))
                        {
                            status = new MasterHorseBL().HorseStationCenter(
                                _horseId,
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    new MasterHorseBL().HorseStationCenter((int)ViewState["horseStatusID"], Convert.ToInt32(this.drpdwnGender.SelectedItem.Value), txtbxFromDate.Text, txtbxTillDate.Text,
        ...
        protected void GvHorseBaseCenter_OnSelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                btnSave.Text = "Update";
                GridViewRow row = this.GvHorseBaseCenter.SelectedRow;
                HiddenField hdnval = (HiddenField)row.FindControl("hdnfieldStatus");
                var dataKey = GvHorseBaseCenter.DataKeys[row.RowIndex];
                if (dataKey != null)
                {
                    ClearAllSelection(this);
        ...
        protected void btnFileUpload_Click(object sender, EventArgs e)
        {
            try
            {
                if (flupload.HasFile)
                {
                    ...
                    if (Path.GetFileNameWithoutExtension(flupload.PostedFile.FileName).Equals("Horse_RunningCenter"))
                    {
        ...
        private void Import_To_Grid(string FilePath, string Extension)
        {
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    var dtErrorResult = new MasterHorseBL().Import30(dt, "HorseStationCenter");
                    if (dtErrorResult.Rows.Count > 0)
                    {
                        ...
                        {
                            ExcelWorksheet ws = xp.Workbook.Worksheets.Add("Horse_RunningCenter");
                            int rowstart = 1;
                            int colstart = 1;
                            int rowend = rowstart;
                            ...
                            Response.AddHeader("content-disposition", "attachment;filename=Horse_RunningCenter.xlsx");
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            try
            {
               // using (DataSet ds = new MasterHorseBL().GetHorseNameWithCombination(_horseId, "HorseStationCenter"))
                using (DataSet ds = new MasterHorseBL().GetExport(_horseId, "HorseStationCenter"))
                {
                    DataTable dt = ds.Tables[0];
                    if (dt.Rows.Count > 0)
                    {
                       // dt.Columns.Remove("RunningCenterID");
                        using (ExcelPackage xp = new ExcelPackage())
                        {
                            ExcelWorksheet ws = xp.Workbook.Worksheets.Add("Horse_RunningCenter");
                            int rowstart = 1;
