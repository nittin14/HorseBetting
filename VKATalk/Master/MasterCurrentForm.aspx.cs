...
{
    public partial class MasterCurrentForm : System.Web.UI.Page
    {
        ...
        protected void BtnSubmit_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    status = Bl.CurrentForm(0, txtbxCurrentForm.Text, 1, "Insert");
                }
                else
                {
                    status = Bl.CurrentForm((int)ViewState["GridViewRowID"], txtbxCurrentForm.Text, 1, "Update");
                }
        ...
        private void BindData()
        {
            ...
            {
                DataTable dt;
                dt = Bl.GetMasterData("CurrentForm");
        ...
        protected void gridvvGlobal_OnSelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                HiddenField hdnval = (HiddenField)row.FindControl("hdnfieldCurrentForm");
                var dataKey = gridvvGlobal.DataKeys[row.RowIndex];
                if (dataKey != null)
                {
                    ClearAllSelection(this);
                    ViewState["GridViewRowID"] = dataKey.Value;
                    txtbxCurrentForm.Text = hdnval.Value;
                }
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                if (!ViewState["GridViewRowID"].Equals(""))
                {
                    ...
                    var status = Bl.CurrentForm((int)ViewState["GridViewRowID"], "", 1, "Delete");
                    ClearAllSelection(this);
                    BindData();
                    BtnSubmit.Text = "Add";
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                using (DataTable dt = Bl.GetMasterData("CurrentForm"))
                {
                    if (dt.Rows.Count > 0)
                    {
                        dt.Columns.Remove("RowCount");
                        dt.Columns.Remove("CurrentFormID");
                        using (ExcelPackage xp = new ExcelPackage())
                        {

                            ExcelWorksheet ws = xp.Workbook.Worksheets.Add("CurrentForm");
                            ...
                            //  }
                            Response.AddHeader("content-disposition", "attachment;filename=Master_CurrentForm.xlsx");
        ...
        protected void btnFileUpload_Click(object sender, EventArgs e)
        {
            try
            {
                if (flupload.HasFile)
                {
                    ...
                    if (Path.GetFileNameWithoutExtension(flupload.PostedFile.FileName).Equals("Master_CurrentForm"))
                    {
        ...
        private void Import_To_Grid(string FilePath, string Extension)
        {
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    var dtErrorResult = new MasterBL().ImportExcel30(dt, "CurrentForm");
