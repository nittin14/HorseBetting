...
{
    public partial class MasterHandicapRatingRange : System.Web.UI.Page
    {
        ...
        protected void BtnSubmit_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    status = Bl.HandicapRatingRange(0, txtbxmasterhandicapratingrange.Text, Convert.ToInt32(txtbxMinRating.Text), Convert.ToInt32(txtbxMaxRating.Text), 1, "Insert");
                }
                else
                {
                    status = Bl.HandicapRatingRange((int)ViewState["GridViewRowID"], txtbxmasterhandicapratingrange.Text, Convert.ToInt32(txtbxMinRating.Text), Convert.ToInt32(txtbxMaxRating.Text), 1, "Update");
                }
        ...
        private void BindData()
        {
            ...
            {
                DataTable dt;
                dt = Bl.GetMasterData("HandicapRatingRange");
        ...
        protected void gridvvGlobal_OnSelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                HiddenField hdnval = (HiddenField)row.FindControl("hdnfieldHandicapRatingRange");
                var dataKey = gridvvGlobal.DataKeys[row.RowIndex];
                if (dataKey != null)
                {
                    ClearAllSelection(this);
                    ViewState["GridViewRowID"] = dataKey.Value;
                    txtbxmasterhandicapratingrange.Text = hdnval.Value;
                    txtbxMinRating.Text = row.Cells[2].Text;
                    txtbxMaxRating.Text = row.Cells[3].Text;
                }
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                if (!ViewState["GridViewRowID"].Equals(""))
                {
                    ...
                    var status = Bl.HandicapRatingRange((int)ViewState["GridViewRowID"], "",0,0, 1, "Delete");
                    ClearAllSelection(this);
                    BindData();
                    BtnSubmit.Text = "Add";
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                using (DataTable dt = Bl.GetMasterData("HandicapRatingRange"))
                {
                    if (dt.Rows.Count > 0)
                    {
                        dt.Columns.Remove("RowCount");
                        dt.Columns.Remove("HandicapRatingRangeID");
                        using (ExcelPackage xp = new ExcelPackage())
                        {

                            ExcelWorksheet ws = xp.Workbook.Worksheets.Add("HandicapRatingRange");
                            ...
                            //  }
                            Response.AddHeader("content-disposition", "attachment;filename=Master_HandicapRatingRange.xlsx");
        ...
        protected void btnFileUpload_Click(object sender, EventArgs e)
        {
            try
            {
                if (flupload.HasFile)
                {
                    ...
                    if (Path.GetFileNameWithoutExtension(flupload.PostedFile.FileName).Equals("Master_HandicapRatingRange"))
                    {
        ...
        private void Import_To_Grid(string FilePath, string Extension)
        {
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    var dtErrorResult = new MasterBL().ImportExcel30(dt, "HandicapRatingRange");
