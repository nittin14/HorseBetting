...
{
    public partial class MasterRaceCardCondition : System.Web.UI.Page
    {
        ...
        protected void BtnSubmit_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    status = Bl.RaceCardCondition(0, txtbxRaceCardCondition.Text, 1, "Insert");
                }
                else
                {
                    status = Bl.RaceCardCondition((int)ViewState["GridViewRowID"], txtbxRaceCardCondition.Text, 1, "Update");
                }
        ...
        private void BindData()
        {
            ...
            {
                DataTable dt;
                dt = Bl.GetMasterData("RaceCardCondition");
        ...
        protected void gridvvGlobal_OnSelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                HiddenField hdnval = (HiddenField)row.FindControl("hdnfieldRaceCardCondition");
                var dataKey = gridvvGlobal.DataKeys[row.RowIndex];
                if (dataKey != null)
                {
                    ClearAllSelection(this);
                    ViewState["GridViewRowID"] = dataKey.Value;
                    txtbxRaceCardCondition.Text = hdnval.Value;
                }
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                if (!ViewState["GridViewRowID"].Equals(""))
                {
                    ...
                    var status = Bl.RaceCardCondition((int)ViewState["GridViewRowID"], "", 1, "Delete");
                    ClearAllSelection(this);
                    BindData();
                    BtnSubmit.Text = "Add";
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                using (DataTable dt = Bl.GetMasterData("RaceCardCondition"))
                {
                    if (dt.Rows.Count > 0)
                    {
                        dt.Columns.Remove("RowCount");
                        dt.Columns.Remove("RaceCardConditionID");
                        using (ExcelPackage xp = new ExcelPackage())
                        {

                            ExcelWorksheet ws = xp.Workbook.Worksheets.Add("RaceCardCondition");
                            ...
                            //  }
                            Response.AddHeader("content-disposition", "attachment;filename=RaceCard_Condition.xlsx");
        ...
        protected void btnFileUpload_Click(object sender, EventArgs e)
        {
            try
            {
                if (flupload.HasFile)
                {
                    ...
                    if (Path.GetFileNameWithoutExtension(flupload.PostedFile.FileName).Equals("RaceCard_Condition"))
                    {
        ...
        private void Import_To_Grid(string FilePath, string Extension)
        {
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    var dtErrorResult = new MasterBL().ImportExcel30(dt, "RaceCardCondition");
