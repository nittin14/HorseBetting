    ...
    public partial class ProspectusMasterPermanentCondition : System.Web.UI.Page
    {
        ...
        protected void BtnSubmit_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    status = Bl.PermanentCondition(0,txtbxPermanentCondition.Text,1,"Insert");
                }
                else
                {
                    status = Bl.PermanentCondition((int)ViewState["GridViewRowID"], txtbxPermanentCondition.Text, 1, "Update");
                }
        ...
        private void BindData()
        {
            ...
            {
                DataTable dt;
                dt = Bl.GetMasterData("PermanentCondition");
        ...
        protected void gridvvGlobal_OnSelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                HiddenField hdnval = (HiddenField)row.FindControl("hdnfieldPermanentCondition");
                var dataKey = gridvvGlobal.DataKeys[row.RowIndex];
                if (dataKey != null)
                {
                    ClearAllSelection(this);
                    ViewState["GridViewRowID"] = dataKey.Value;
                    txtbxPermanentCondition.Text = hdnval.Value;
                }
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                if (!ViewState["GridViewRowID"].Equals(""))
                {
                    ...
                    var status = Bl.PermanentCondition((int)ViewState["GridViewRowID"], "", 1, "Delete");
                    ClearAllSelection(this);
                    BindData();
                    BtnSubmit.Text = "Add";
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                using (DataTable dt = Bl.GetMasterData("PermanentCondition"))
                {
                    if (dt.Rows.Count > 0)
                    {
                        dt.Columns.Remove("RowCount");
                        dt.Columns.Remove("PermanentConditionID");
                        using (ExcelPackage xp = new ExcelPackage())
                        {
                            ...
                            //  }
                            Response.AddHeader("content-disposition", "attachment;filename=Permanent_Condition.xlsx");
        ...
        protected void btnFileUpload_Click(object sender, EventArgs e)
        {
            try
            {
                if (flupload.HasFile)
                {
                    ...
                    if (Path.GetFileNameWithoutExtension(flupload.PostedFile.FileName).Equals("Permanent_Condition"))
                    {
        ...
        private void Import_To_Grid(string FilePath, string Extension)
        {
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    var dtErrorResult = new MasterBL().ImportExcel30(dt, "PermanentCondition");
