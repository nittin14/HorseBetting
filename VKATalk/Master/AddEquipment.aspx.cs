    ...
    public partial class AddEquipment : System.Web.UI.Page
    {
        ...
        protected void BtnSubmit_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    status = Bl.InsertMasterPagesData("Equipment", txtbxEquipment.Text, txtbxAlias.Text, 1);
                    checkcondition = 1;
                }
                else
                {
                    status = Bl.UpdateMasterPagesData("Equipment", (int)ViewState["GridViewRowID"], txtbxEquipment.Text, txtbxAlias.Text, 1, "Update");
                    checkcondition = 2;
                }
        ...
        {
                DataTable  dt = Bl.GetMasterData("Equipment");
                //dvgridview.Visible = true;
                if (dt.Rows.Count > 0)
                {
                    gridvwEquipment.DataSource = dt;
                    gridvwEquipment.DataBind();
                }
                else
                {
                    gridvwEquipment.DataSource = new DataTable();
                    gridvwEquipment.DataBind();
                }
        }
        ...
        protected void gridvwEquipment_OnSelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                BtnSubmit.Text = "Update";
                GridViewRow row = gridvwEquipment.SelectedRow;
                HiddenField hdnval = (HiddenField)row.FindControl("hdnfieldStatus");
                var dataKey = gridvwEquipment.DataKeys[row.RowIndex];
                if (dataKey != null)
                {
                    ClearAllSelection(this);
                    ViewState["GridViewRowID"] = dataKey.Value;
                    txtbxEquipment.Text = hdnval.Value;
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    var status = Bl.UpdateMasterPagesData("Equipment", (int)ViewState["GridViewRowID"], "", "", 1, "Delete");
                    ClearAllSelection(this);
                    BindData();
                    BtnSubmit.Text = "Add";
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                using (DataTable dt = Bl.GetMasterData("Equipment"))
                {
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        using (ExcelPackage xp = new ExcelPackage())
                        {
                            ...
                            ExcelWorksheet ws = xp.Workbook.Worksheets.Add("Equipment");
                            ...
                            //  }
                            Response.AddHeader("content-disposition", "attachment;filename=Equipment.xlsx");
        ...
        protected void btnFileUpload_Click(object sender, EventArgs e)
        {
            try
            {
                if (flupload.HasFile)
                {
                    ...
                    if (Path.GetFileNameWithoutExtension(flupload.PostedFile.FileName).Equals("Equipment"))
                    {
        ...
        private void Import_To_Grid(string FilePath, string Extension)
        {
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    var dtErrorResult = new MasterBL().ImportExcel30(dt, "Equipment");
