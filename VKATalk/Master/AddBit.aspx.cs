...
{
    public partial class AddBit : System.Web.UI.Page
    {
        ...
        protected void BtnSubmit_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    status = Bl.InsertMasterPagesData("Bit", txtbxBit.Text, txtbxAlias.Text, 1);
                    checkcondition = 1;
                }
                else
                {
                    status = Bl.UpdateMasterPagesData("Bit", (int)ViewState["GridViewRowID"], txtbxBit.Text, txtbxAlias.Text, 1, "Update");
                    checkcondition = 2;
                }
        ...
        private void BindData()
        {
            ...
            {
                DataTable dt;
                dt = Bl.GetMasterData("Bit");
               // dvgridview.Visible = true;
                if (dt.Rows.Count > 0)
                {

                    gridvwBit.DataSource = dt;
                    gridvwBit.DataBind();
        ...
        protected void gridvwBit_OnSelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                BtnSubmit.Text = "Update";
                GridViewRow row = gridvwBit.SelectedRow;
                HiddenField hdnval = (HiddenField)row.FindControl("hdnfieldStatus");
                var dataKey = gridvwBit.DataKeys[row.RowIndex];
                if (dataKey != null)
                {
                    ClearAllSelection(this);
                    ViewState["GridViewRowID"] = dataKey.Value;
                    txtbxBit.Text = hdnval.Value;
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                using (DataTable dt = Bl.GetMasterData("Bit"))
                {
                    if (dt.Rows.Count > 0)
                    {
                        dt.Columns.Remove("RowCount");
                        dt.Columns.Remove("BitID");
                        using (ExcelPackage xp = new ExcelPackage())
                        {
                            ...
                            //  }
                            Response.AddHeader("content-disposition", "attachment;filename=Bit.xlsx");
        ...
        protected void btnFileUpload_Click(object sender, EventArgs e)
        {
            try
            {
                if (flupload.HasFile)
                {
                    ...
                    if (Path.GetFileNameWithoutExtension(flupload.PostedFile.FileName).Equals("Bit"))
                    {
        ...
        private void Import_To_Grid(string FilePath, string Extension)
        {
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    var dtErrorResult = new MasterBL().ImportExcel30(dt, "Bit");
