...
namespace VKATalk.PopUps
{
    ...
    public partial class RaceDayIncident : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                if (!IsPostBack)
                {
                    hdnfieldDivisionRaceDate.Value = Request.QueryString["DataEntryDate"];
                    hdnfieldCenterID.Value = Request.QueryString["CenterID"];
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    if(!hdnfieldIncidentID.Value.Equals(""))
                    {
                        status = new CardsBL().ResultRaceIncident(
                        0,
                        Convert.ToInt32(hdnfieldOwnerStudID.Value),
                        Convert.ToInt32(hdnfieldCommentator.Value),
                        Convert.ToInt32(drpdwntoteVariant.SelectedItem.Value),
        ...
        {
            DataTable dt = new CardsBL().GetToteDividentDetail(Convert.ToInt32(hdnfieldOwnerStudID.Value), "RaceDayIncident", hdnfieldDivisionRaceDate.Value, hdnfieldCenterID.Value);
            if (dt.Rows.Count > 0)
            {
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    var status = new CardsBL().ResultRaceIncident(
        ...
        protected void GvHorseGlobal_OnSelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                HiddenField hdnfdIncidentID = (HiddenField)row.FindControl("hdnfdIncidentID");
                ...
                if (dataKey != null)
                {
                    ...
                    if (row.Cells[1].Text.Contains("&quot;"))
                    {
                        ...
                        hdnfieldCommentator.Value = hdnfdProfessionalNameID.Value;
                    }
                    else
                    {
                        txtbxCommentator.Text = row.Cells[1].Text.Equals("&nbsp;") ? "" : row.Cells[1].Text.Contains("&#39;") ? row.Cells[1].Text.Replace("&#39;", "'") : row.Cells[1].Text;
                    ...
                    {
                        drpdwntoteVariant.Items.FindByText(row.Cells[2].Text.Replace("&#39;","'")).Selected = true;
                    }
                    else
                    {
                        drpdwntoteVariant.Items.FindByText(row.Cells[2].Text).Selected = true;
                    }

                    
                    if (row.Cells[3].Text.Contains("&quot;"))
                    {
                        txtbxIncident.Text = row.Cells[3].Text.Equals("&nbsp;") ? "" : row.Cells[3].Text.Contains("&quot;") ? row.Cells[3].Text.Replace("&quot;", "\"") : row.Cells[3].Text;
                        hdnfieldIncidentID.Value = hdnfdIncidentID.Value;
                    }
                    else
                    {
                        txtbxIncident.Text = row.Cells[3].Text.Equals("&nbsp;") ? "" : row.Cells[3].Text.Contains("&#39;") ? row.Cells[3].Text.Replace("&#39;", "'") : row.Cells[3].Text;
        ...
        {
            if (chkbx3.Checked.Equals(true))
            {
                //drpdwntoteVariant.ClearSelection();
                txtbxIncident.Text = string.Empty;
                hdnfieldIncidentID.Value = string.Empty;
                chkbxHighlight.Checked = false;
                //GvHorseGlobal.DataSource = new DataTable();
                //GvHorseGlobal.DataBind();
            }
        ...
        protected void btnFileUpload_Click(object sender, EventArgs e)
        {
            try
            {
                if (flupload.HasFile)
                {
                    ...
                    if (Path.GetFileNameWithoutExtension(flupload.PostedFile.FileName).Equals("Card_Result_RaceIncident"))
                    {
        ...
        private void Import_To_Grid(string FilePath, string Extension)
        {
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    var dtErrorResult = Bl.Import30(dt, "Card_Result_RaceIncident");
                    if (dtErrorResult.Rows.Count > 0)
                    {
                        using (ExcelPackage xp = new ExcelPackage())
                        {

                            ExcelWorksheet ws = xp.Workbook.Worksheets.Add("Card_Result_RaceIncident");
                            ...
                            Response.AddHeader("content-disposition", "attachment;filename=Card_Result_RaceIncident.xlsx");
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                using (DataSet ds = new MasterHorseBL().GetExport(_horseId, "Card_Result_RaceIncident"))
                {
        ...
        protected void txtbxOwnerStud_TextChanged(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    drpdwntoteVariant.DataSource = dt;
                    drpdwntoteVariant.DataTextField = "HorseName";
                    drpdwntoteVariant.DataValueField = "HorseNameID";
                    drpdwntoteVariant.DataBind();
