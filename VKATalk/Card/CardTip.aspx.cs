...
namespace VKATalk.Card
{
    ...
    public partial class CardTip : System.Web.UI.Page
    {
        ...
        {
            DataTable dt = new CardsBL().GetCardAutoFiller("AddTipsterList", prefixText);
        ...
        protected void dvgrdviewHorseDetail_OnSelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                if (dataKey != null)
                {
                    ...
                    HiddenField hdnfieldtiptypeid = (HiddenField)row.FindControl("hdnfieldtiptypeid");
                    hdnfieldTipID.Value = hdnfieldtiptypeid.Value;
        ...
        private void AcceptanceShow()
        {
            DataSet ds = new CardsBL().GetAcceptanceDivisionDetailMultipleReturn(
                    txtbxRaceDate.Text,
                    Convert.ToInt32(drpdwnCenterName.SelectedItem.Value), "CardTip");
        ...
        protected void btnAdd_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    dt = new CardsBL().Tip(0, txtbxRaceDate.Text, Convert.ToInt32(drpdwnCenterName.SelectedItem.Value), Convert.ToInt32(hdnfieldhotlinerid.Value),
                        Convert.ToInt32(hdnfieldHorseID.Value), Convert.ToInt32(hdnfieldTipID.Value), 1, "Insert");
                }
                else
                {
                    dt = new CardsBL().Tip(Convert.ToInt32(ViewState["HotlineCID"]), txtbxRaceDate.Text, Convert.ToInt32(drpdwnCenterName.SelectedItem.Value), Convert.ToInt32(hdnfieldhotlinerid.Value),
                        Convert.ToInt32(hdnfieldHorseID.Value), Convert.ToInt32(hdnfieldTipID.Value), 1, "Update");
                    btnAdd.Text = "Add";
                }
                if (dt.Rows.Count > 0)
                {
        ...
        protected void btnFileUpload_Click(object sender, EventArgs e)
        {
            try
            {
                if (flupload.HasFile)
                {
                    ...
                    if (Path.GetFileNameWithoutExtension(flupload.PostedFile.FileName).Equals("Card_Tip"))
                    {
        ...
        private void Import_To_Grid(string FilePath, string Extension)
        {
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    var dtErrorResult = new CardsBL().Import30(dt, "Card_Tip", 0);
                    if (dtErrorResult.Rows.Count > 0)
                    {
                        using (ExcelPackage xp = new ExcelPackage())
                        {

                            ExcelWorksheet ws = xp.Workbook.Worksheets.Add("Card_Tip");
                            ...
                            Response.AddHeader("content-disposition", "attachment;filename=Card_Tip.xlsx");
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            try
            {
                using (DataSet ds = new CardsBL().GetExport(txtbxRaceDate.Text, Convert.ToInt32(drpdwnCenterName.SelectedItem.Value), "Card_Tip"))
                {
